...
**残課題 / 次の方向**
- **実機確認**（web は GitHub Pages 即反映）＝① 全期間で代表サムネが出るか ② 自宅周辺のように写真が密な所は寄っても `dot` のままになる（600枚が狭い範囲＝z15 でも点）。代表60枚で足りるか、`MAP_SAMPLE_THUMB` を上げるか下げるかは**実機の手触りで決める**。
- native（iOS 1.7 / Android vc5）は次ビルドで乗る。
