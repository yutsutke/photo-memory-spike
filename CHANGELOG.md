...
- あちらへは今回こちらから**構造を移植**した（コア一行のリトマス §1① / park・un-park の二重ゲート §1③ / 転換の力学 §5 / アンチパターン表 §4）＝**判断の移植は実装でなく判断の理由を写す (§2.10-4) の実演**。あちらのコア一行は「自分の生活の事実を、信頼できる形で残して、読み返せること」で起草（本人の確定待ち）。
