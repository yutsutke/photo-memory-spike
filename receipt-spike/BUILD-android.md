...
**残っている疑い＝「パッケージは登録済みでも、鍵が全部は登録されていない」**

- エラーの文言が「アプリを登録して」ではなく「**すべての鍵を登録して**」である
- レシートだけ **キーが3つ**（あの日1・ボイスカレンダー2）＝レシートは
  **debug 鍵で APK を直接配った時期がある**（このファイルの下の方の運用）ので、
  Google が見た署名鍵が **debug 鍵／アップロード鍵／Play の配信用鍵**の3つある可能性が高い
- → **パッケージ名の行の → を開いて、鍵ごとのステータスを見る**のが次の一手

**アップロード鍵 `receipt-upload` の指紋（フォームが求めてきた時のため）**
...
- 指紋の出し方（PowerShell・パスワードを画面に出さない書き方）:
  ```powershell
  ...
  ```
- ⚠ **debug 鍵**の指紋が要る時は `~\.android\debug.keystore`（別名 `androiddebugkey`・パスワードは `android`）
