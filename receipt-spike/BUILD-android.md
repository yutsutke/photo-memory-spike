...
**次の一手（未検証の見立て）＝この画面の「鍵の追加」でアップロード鍵を足す**

- 根拠＝リマインダーメールに「**Google Play アプリに対し Google Play 以外での署名に使用する別の鍵を追加する**」とある。
  アップロード鍵はまさに「Play の外でも署名に使っている鍵」（`receipt-1.0-vcN.apk` を直接配る道）
- 追加するのは **`A7:91:…:4B`**（上の表のアップロード鍵）
- ⚠ **これで通るかは未確認**。通らなければ、debug 鍵（`74:80:…:78`）も足すか、リリースを作り直して再確認する
