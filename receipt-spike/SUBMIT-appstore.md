...
>
> ⚠ **3の「まとめて」は道で中身が違う**（実装を確認済み）＝📈 簡易は レシート＋食事＋🩺健診＋💊薬（**🫀血圧・⚖体重は未接続**）／📤 書き出しは **🫀血圧・体重まで全部入る**。下の文章は**その通りに書き分けてある**。📈 に 🫀 を足したら、書き分けをやめて1行にできる。
...
・簡易はワンタップ — 期間を選んで型を押すだけ。
  ...
  レシートに加えて、食事・健康診断・薬もいっしょに渡せます。
・じっくりは、ご自身の AI チャットで — 📤 で期間と種類を選んで書き出し、共有シートから
  Claude や ChatGPT にそのまま手渡し。Google ドライブに置いておけば「あのファイルを見て」で済みます。
  こちらは レシート・食事・健康診断・薬 に加えて 🫀 血圧・体重 まで、まとめて渡して解析できます。
