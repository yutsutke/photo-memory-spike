# 🍎 App Store に審査を出す — レシート（撮ってAIで読む）

> **これは何か**: TestFlight まで来ているアプリを、**App Store の審査に出すまで**の手順書＋**そのまま貼れる文章**。
> ビルドの作り方は [BUILD-ios.md](BUILD-ios.md)、Play 側は [BUILD-android.md](BUILD-android.md)。
> ⚠ **ASC（App Store Connect＝アップルの管理画面）の操作と最終の提出ボタンはユーザー（ゆう）が押す**。ここは「何をどう答えるか」の台本。

- appId: `io.github.yutsutke.receipt`
- ホーム画面の表示名: **レシート**
- プライバシーポリシー: https://yutsutke.github.io/photo-memory-spike/receipt-spike/privacy.html
- サポート: https://yutsutke.github.io/photo-memory-spike/receipt-spike/support.html
- 連絡先: anohiapp@gmail.com（あの日と共通）

---

## 0. いまの状態（2026-08-23）

> 🎉 **2026-08-23 12:00 頃、1.0 を審査に提出した（審査待ち）。** 以下は「何をどう答えたか」の記録＝**次の版（1.1）でもそのまま使える台本**。

| もの | いま |
|---|---|
| **審査** | 🎉 **1.0 提出ずみ（審査待ち）**・リリースは**手動** |
| web（GitHub Pages） | **r104** |
| 提出したビルド | **1.0 (11)**＝iPhone 専用・r103・r104 入り |
| 配信地域 | **日本のみ**（中身が日本専用＝§5 の理由） |
| 年齢制限 | **9+**（§6・健康／ウェルネス関連＝はい が効いた） |
| スクリーンショット | ✅ 5枚・**6.5インチ枠 1284×2778** |
| Google Play | vc4 ＝ r94（内部テスト）＝**置いていかれている**（次は vc5 で r104 へ） |

---

## ✅ 1. 決めた2つ（2026-08-23）

### ① iPad は外す＝**iPhone 専用で出す**（決定・変更済み）

`TARGETED_DEVICE_FAMILY` を `"1,2"`（iPhone と iPad）から **`"1"`（iPhone だけ）へ変更した**（`ios/App/App.xcodeproj/project.pbxproj` の2か所）。**あの日と同じ形**。

iPad を含めたままだと、こうなるので外した:
- ASC が **iPad 13インチのスクリーンショットを必須で要求する**（iPhone のぶんだけでは提出できない）
- 審査担当者が **iPad で開いて崩れを見る**（この画面は 375px 幅を基準に作ってある＝広い画面での確認は一度もしていない）

✅ 焼き直しずみ＝**提出に使うのは 1.0(11)**（iPhone 専用・r103・r104 入り）。1.0(8) 以前は iPad も対象のままなので使わない。
iPad を永久に捨てるわけではなく、**後から広げるのは自由**（狭く出して広げるのは簡単・逆は面倒）。

### ② ストア上の名前は Play に寄せる（決定）

| どこ | いまの名前 |
|---|---|
| ホーム画面（`CFBundleDisplayName`） | **レシート** |
| App Store（ASC） | **レシート — 撮ってAIで読む** |
| Google Play | **レシートと健康— 撮ってAIで読む** |

**ASC も「レシートと健康 — 撮ってAIで読む」に寄せる**（18字・上限30字）。
理由＝r83〜r97 で 🫀 血圧・⚖ 体重と体組成計・📸 からだの写真・💊 薬 が増え、**健康はもうレシートと並ぶ柱**（トップの3タブのうち1つ）。名前に入っていないと「家計簿アプリ」としてしか探されない。
⚠ ASC の名前は**審査に出す前なら何度でも変えられる**。ホーム画面の「レシート」は短いままでよい（アイコンの下は短い方が読める）。

---

## ✅ 1-b. 配信地域＝**日本のみ**（2026-08-23 決定）

新規アプリは「価格および配信状況」で**配信地域を設定しないと提出できない**（あの日は設定済みだったので気付かなかった）。

**日本のみにした理由＝中身が日本専用だから**（コードで確認した）:
- **UI も掲載文も日本語だけ**（あの日と違い `i18n` の仕組みが無い）
- **読み取りが日本の制度に合わせてある**＝適格請求書の登録番号（T+13桁）・8%/10% の税率
- **💴 支出マップが国土地理院**（日本の住所に強い）を使う

**おまけの効き目**: EU に配信しなければ、**DSA の「取引業者（trader）の住所公開」問題が丸ごと消える**（あの日は EU を外すことで自宅住所の公開を避けている）。
**広げるのは公開後にいつでもできる**（審査不要）。順路は あの日と同じ＝英語対応 → EU を除く地域へ。

---

## 🔴 2. いちばんの関門＝審査担当者が「試せない」問題（Guideline 2.1）

このアプリは **BYOK（本人の APIキーを登録して使う）**。**キーが無いと読み取りが1回も動かない**＝審査担当者はアプリの中心機能を試せない。
Apple は「**レビュー担当者が全機能を試せること**」を求めるので、**ここを外すとほぼ確実に差し戻される**。

**対処＝審査用のキーを1本用意して、審査メモに書く。**

- [ ] **審査専用の APIキーを発行する**（⚠ ゆうの作業。私は代行しません）
  - おすすめ＝**OpenRouter**（鍵1本で複数のモデルが使える・**クレジットを $5〜10 だけ入れておけば被害はそこで止まる**）
  - 名前を `appstore-review-2026-08` のようにしておくと、あとで失効させやすい
- [ ] **審査メモにキーと手順を書く**（§5 の文面に貼る欄がある）
- [ ] **審査が終わったら失効させる**（承認・却下どちらでも。⚠ 忘れると誰かに使われ続ける）

> 💡 「キーが無くても撮って残せる」は事実だが、**それだけでは中心機能の説明にならない**。キーを渡すのがいちばん短い道。

---

## 3. やることの一本道（この順に進む）

**A. 出す前に直すもの（コード・文書）**
- [x] ① iPad を外す＝`TARGETED_DEVICE_FAMILY = "1"` に変更（2026-08-23 済み）→ ✅ 焼き直して **1.0(11)**
- [x] ② プライバシーポリシーを r102 の中身に合わせる（🫀 血圧・脈／⚖ 体重と体組成計の5つ／📸 からだの写真／⭐ いつものメニュー／🔁 繰り返し を追記）＝**2026-08-23 更新済み**
- [ ] ③ 新しいビルドが TestFlight に着いたら、**実機で一通り触る**（キー登録 → 🧾 レシート1枚 → 🍽 → 🩺 の5タブ → 📤 書き出し → 🧹 まとめて削除）

**B. ASC に入れるもの（§4〜§7 のコピペ）** — ✅ 2026-08-23 に全部やった
- [x] ④ **App 情報**（左サイドバー「App 情報」）: 名前・サブタイトル・カテゴリ（第1＝ファイナンス／第2＝仕事効率化）・コンテンツ配信権＝サードパーティ無し・年齢レーティング（§6）
- [x] ⑤ **価格および配信状況**: 無料（¥0）／アプリ内課金なし／**配信地域＝日本のみ**（§5）／プライバシーポリシーURL（必須・空だと出せない）
- [x] ⑥ **App のプライバシー**（§7）＝データ収集の申告 ⚠ 最後に**「公開」**を押さないと反映されない
- [x] ⑦ **バージョン情報**: プロモーションテキスト・説明文・キーワード・サポートURL・マーケティングURL（空でよい）・著作権
- [x] ⑧ **スクリーンショット**（§8）＝⚠ **キーを入れて中身が入った状態**で撮る
- [x] ⑨ **ビルドを選ぶ**: **1.0 (11)**（iPhone 専用化＋r103・r104 入り）。⚠ 1.0 (8) 以前は iPad も対象のままなので使わない（⚠ 一覧に古いビルドしか出ない時は**保存 → 再読み込み**で全部出る＝あの日でハマった罠）
- [x] ⑩ **App Review 情報**: サインイン不要／連絡先（氏名・電話・anohiapp@gmail.com）／**審査メモ（§5）に審査用キーを貼る**
- [x] ⑪ **リリース方法**: 「手動でリリース」を選ぶ（承認されてもすぐ公開されない＝落ち着いて確認できる）
- [x] ⑫ **提出**＝🎉 **2026-08-23 完了（審査待ち）**

**C. 出したあと**
- [ ] ⑬ 審査用キーを**失効させる**
- [ ] ⑭ Play 側も同じ版（r104）へ上げる＝vc5 を焼く（[BUILD-android.md](BUILD-android.md)・一発50秒）
- [ ] ⑮ 承認されたら「このバージョンをリリース」を押す

---

## 4. コピペ用の文章（ASC のそれぞれの欄へ）

> 🗣 **元になった「伝えたいこと」（ゆう・2026-08-23）**。以下の文章はこの順・この重みで書いてある。
> 1. レシートを撮るだけで、詳細が書き起こされる
> 2. 食事の写真
> 3. 定期健康診断の情報を入れておくと、**食事やレシートの内容から AI がアドバイスをしてくれる**
>    ・簡易はワンタップ ／ ・じっくりはご自身の AI チャットに情報を送って分析
>    ・食事とレシート＋健診・血圧・体重・薬 を**まとめて渡して解析もできる**
> 4. 血圧・体重計・薬の記録機能もある
> 5. **解析精度を上げられる**（AI モデルを変える／食事の写真にレシートや成分表示の写真を一緒に足す）
>
> ✅ **3の「まとめて」は r103（2026-08-23）で両方そろった**＝📈 簡易も 📤 書き出しも、レシート・食事・🩺健診・💊薬・**🫀血圧と体重**を渡せる。さらに **渡すもの5種を1つずつ選べる**（🧾 レシートを外すこともできる）。以前あった書き分けは不要になった。

### App 名（上限30字）

```
レシートと健康 — 撮ってAIで読む
```

### サブタイトル（上限30字）

```
撮るだけ。AIが家計と健康をつなぐ
```

### プロモーションテキスト（上限170字・⚠ **審査なしでいつでも差し替えられる**）

```
レシートも食事も、撮るだけで AI が書き起こします。健康診断や血圧・体重を入れておけば、「何を買って何を食べているか」と合わせて AI が助言。サーバーはありません。記録はあなたの端末の中だけです。
```

### 説明文（上限4000字）

```
レシートを撮るだけ。AI が店名・日付・金額・品目まで書き起こします。
食事も撮るだけ。そして健康診断・血圧・体重・薬を入れておくと、
「何を買って、何を食べているか」と合わせて、AI が助言してくれます。

■ 1. 撮るだけで、書き起こされる

🧾 レシート — 撮る／写真から選ぶ → 数秒で、店名・日付・金額・品目・支払方法まで表になります。
1枚に複数のレシートが写っていても、AI が分けて読みます。
🍽 食事 — 料理を撮るだけ。手作り／中食（買って帰る）／外食も AI が見分けます。
よく食べる献立は ⭐ で覚えて次から1タップ。毎日の定番は 🔁 繰り返しで自動的に入ります。

■ 2. 健康も、同じ「撮るだけ・入れるだけ」

🩺 健康診断の結果／💊 薬（いつから何を飲んでいるか）／🫀 血圧・脈（1日に何回でも）／
⚖ 体重と体組成計の数値（体脂肪率・骨格筋率・基礎代謝・BMI・内臓脂肪レベル）／
📸 からだの写真（同じ角度で撮って並べる）。
体組成計や血圧計の表示も、撮れば AI が数字を書き写します。

■ 3. AI が、家計と健康をつないで見てくれる

・簡易はワンタップ — 期間を選んで型を押すだけ。
  📊 ふりかえり／💡 節約のヒント／🩺 健康の面から／🏪 お店のくせ／💰 予算と照らす／✍️ 自由に書く。
・じっくりは、ご自身の AI チャットで — 📤 で書き出して、共有シートから Claude や ChatGPT に
  そのまま手渡し。Google ドライブに置いておけば「あのファイルを見て」で済みます。
・どちらも 🧾 レシート・🍽 食事・🩺 健康診断・💊 薬・🫀 血圧と体重 を、まとめて渡して解析できます。
  渡すものは1つずつ選べます（今日は食事と体の数字だけ、という渡し方もできます）。

■ 4. 解析の精度は、自分で上げられる

・使う AI モデルを選べます（読み取り・分析・健診・薬 で別々に。写真は安いモデル、分析は賢いモデル、という組み合わせも）
・食事の写真に「手がかり」を足せます — 🧾 レシート・伝票／🏷 成分表示・パッケージ／🍽 別の角度の写真。
  何を食べたかが写真だけでは分からない時、印字された商品名や内容量が精度を上げます。

■ 5. 貯まったものを眺める

📊 統計／📈 推移（週・月・年）／💴 支出マップ（レシートに印字された住所から地図に置きます）／
🔍 レシートも食事も健康もまとめて検索／💰 予算（統計・推移・マップ・書き出しに反映されます）。

■ サーバーはありません

記録はあなたの端末の中だけに貯まります。アカウント登録も不要です。
AI に読ませる部分は、あなた自身の APIキー（Claude / ChatGPT / OpenRouter）を ⚙ 設定に登録して使います。
写真は端末から、あなたが選んだ AI へ直接送られます。開発者はそれを受け取りません。

■ ご注意

・ご利用には AI 事業者の APIキーが必要です（料金はあなたと事業者の間で発生します）
・本アプリは医療機器ではありません。診断も医学的な助言もしません。AI の要約は参考です。判断は医師にご相談ください
```

### キーワード（上限100字・カンマ区切り・⚠ スペースを入れない）

```
レシート,家計簿,支出,経費,OCR,読み取り,AI,分析,節約,食事,栄養,健康管理,健康診断,薬,血圧,体重
```

### サポートURL

```
https://yutsutke.github.io/photo-memory-spike/receipt-spike/support.html
```

### プライバシーポリシーURL

```
https://yutsutke.github.io/photo-memory-spike/receipt-spike/privacy.html
```

### 著作権（Copyright）

```
2026 yutsutke
```

⚠ あの日で入れたものと同じ表記に揃える（ASC の「販売元名」と食い違わせない）。

### このバージョンの新機能

⚠ **1.0（初回）では出てこない欄**。1.1 以降で使う。

---

## 5. 審査メモ（App Review 情報 ▸ メモ欄へそのまま貼る）

> ⚠ `__________` に、§2 で発行した**審査用キー**を貼ってから提出する。

```
[English]
Thank you for reviewing.

HOW TO TEST THE MAIN FEATURE (please read first)
This app has no server and no account. It reads photos using AI through the
user's OWN API key (BYOK). Without a key, photos are only stored on the device
and no reading happens. For review, please use this temporary key:

  Provider: OpenRouter
  API key:  __________

  Steps:
   1. Open the app and tap the gear icon (settings) at the top right.
   2. Under "AI for reading receipts", choose "OpenRouter".
   3. Paste the key into the field below it.
   4. Tap "Fetch the list of usable models" and pick any model from the list.
   5. Tap "Connection test" (free, not billed) - it should say OK.
   6. Close settings, tap the camera button at the bottom and photograph any
      receipt, or pick a photo of a receipt from the library.
   The store name, date, total and line items appear within a few seconds.

  The same flow works for meals, blood pressure, weight, medication and health
  check-up results from the three tabs at the top (Receipts / Meals / Health).

PRIVACY
- We (the developer) operate no server and receive nothing. Photos are sent
  DIRECTLY from the device to the AI provider the user chose, authenticated
  with the user's own API key. That provider's privacy policy applies, and this
  is stated in our privacy policy and in the settings screen.
- We declared Photos, Purchase History, Health and Other User Content as
  "collected" for app functionality, not linked to identity, no tracking,
  because those items do leave the device when the user runs a reading.
- No location permission is requested and photo GPS is never read. The
  coordinates on the spending map come from the printed store ADDRESS TEXT
  (geocoded via GSI Japan / OpenStreetMap), not from the user's location.
- No ads, no analytics, no tracking SDKs.

HEALTH
The app only transcribes and organises the user's own documents and numbers.
It gives no diagnosis and no medical advice; a disclaimer to that effect is
shown in the Health tab and in the privacy policy. HealthKit is not used.

DATA DELETION
Settings > bottom > "Delete in bulk" removes data by period and by kind
(receipts / meals / check-ups / medication / body numbers / body photos /
analysis history / store locations / repeat rules / saved menus).
Deleting the app removes everything.

[日本語]
審査ありがとうございます。

■ 中心機能の試し方（BYOK のため、先にお読みください）
本アプリはサーバーもアカウントも持たず、写真の読み取りは「利用者ご自身の
APIキー」で行います（BYOK）。キーが無いと写真は端末に保存されるだけで読み取り
は動きません。審査用に一時キーをご用意しました。

  事業者: OpenRouter
  APIキー: __________

  手順:
   1. アプリを開き、右上の ⚙（設定）を開く
   2. 「📷 レシートの読み取りに使うAI」で「OpenRouter」を選ぶ
   3. その下の欄にキーを貼る
   4. 「🔄 使えるモデルの一覧を取得」を押し、一覧から1つ選ぶ
   5. 「🔌 接続テスト（無料・課金されません）」を押して OK を確認
   6. 設定を閉じ、下の 📷 でレシートを撮る（または 🖼 から写真を選ぶ）
   数秒で店名・日付・金額・品目が表示されます。
  料理・血圧・体重・薬・健康診断も、上の3つのタブ（🧾 レシート／🍽 食事／
  🩺 健康）から同じ流れで使えます。

■ プライバシー
・開発者はサーバーを持たず、何も受け取りません。写真は端末から、利用者が選んだ
  AI 事業者へ直接送られます（利用者自身のキーで認証）。その事業者の方針が適用
  される旨は、プライバシーポリシーと設定画面に明記しています。
・写真・購入履歴・健康・その他のユーザーコンテンツを「収集する（アプリの機能・
  本人に紐付けない・追跡しない）」として申告しています。読み取り時に実際に端末
  の外へ出るためです。
・位置情報の権限は要求せず、写真の GPS も読みません。支出マップの座標はレシート
  に印字された「住所の文字」から引いたもの（国土地理院／OpenStreetMap）で、
  利用者の居場所ではありません。
・広告・アナリティクス・トラッキング SDK はありません。

■ 健康について
本アプリは利用者本人の書類や数値を書き写して整理するだけで、診断も医学的助言も
行いません。その旨を 🩺 タブとプライバシーポリシーに明記しています。HealthKit は
使用していません。

■ データの削除
⚙ 設定の一番下「🧹 まとめて削除」で、期間と種類（レシート／料理／健診／薬／
からだの数字／からだの写真／分析の履歴／店の場所／繰り返しの決まり／いつもの
メニュー）を選んで削除できます。アプリの削除ですべて消えます。
```

**審査連絡先（同じ画面の上の方）**

- サインインが必要ですか → **いいえ**
- 連絡先: 氏名・電話番号（あの日と同じ）・メール `anohiapp@gmail.com`

---

## 6. 年齢レーティング → **結果 9+**（2026-08-23 実際に答えた記録）

⚠ 2026年の画面は**7ステップ**。ほとんど「なし／いいえ」だが、**考えるのは ステップ3 の2問だけ**。

### ステップ3「医療／ウェルネス」＝ここだけ本気で読む

⚠ **2問あって、性質が違う**（片方は程度、片方は有無）。**画面に書いてある定義文で決まる**。

| 質問（定義） | 答え | 根拠 |
|---|---|---|
| **医療または治療に関する情報**（なし／まれ／頻繁）<br>＝「病状や健康とウェルネスの管理に関する**診断やガイダンス**を提供するコンテンツ」 | **なし** | 本人の書類と数値を**書き写して整理するだけ**で、病状の判断も治療の指示もしない。AI にも「医者の代わりではないので断定しない・気になる点は受診の目安として書く」と指示してある（AN_PRESETS）。🩺 タブとプライバシーポリシーに「医療機器ではない」と明記（r72） |
| **健康またはウェルネス関連**（いいえ／はい）<br>＝「**セルフケアや生活習慣に関する推奨事項**を提供するコンテンツ」 | **はい** | 🩺「健康の面から」で AI が食生活の気づきを書く＝まさにこれ。⚠ **説明文に「AI が助言してくれます」と自分で書いている**＝いいえ にすると掲載文と申告が食い違う（審査で見つかりやすい種類の矛盾） |

### 残りは全部「なし／いいえ」

暴力／性的表現／不適切な言葉／恐怖・ホラー／ギャンブル／コンテスト／無制限のウェブアクセス／チャット・ユーザー生成コンテンツの共有／広告。

**引っかかりやすい4つ**（全部「なし・いいえ」でよい理由）:

| 項目 | 迷う理由 | なぜ該当しないか |
|---|---|---|
| アルコール・タバコ・**薬物** | 💊 薬がある | この項目が指すのは**嗜好品・違法薬物の描写**。処方薬の記録は該当しない |
| **無制限のウェブアクセス** | 中身が WebView | 見られるのは**自分のアプリの画面だけ**。ポリシーは Safari で外に開く＝ブラウザ機能ではない |
| **ユーザー生成コンテンツの共有・チャット** | 📤 書き出しがある | あれは**本人が自分のデータを外へ持ち出す**だけ。利用者どうしが見せ合う仕組みではない |
| **広告** | — | v1 は広告なし |

### ステップ7「追加情報」

- **算出された年齢制限＝9+**（健康／ウェルネス関連＝はい が効いた）
- **年齢カテゴリと上書き → 「該当なし」**
  - ⚠ **「子ども向け」は絶対に選ばない**＝Kids Category に入り、外部リンク禁止など**格段に厳しい審査**になる
  - 「より高い年齢制限に上書き」も不要
- **年齢適合性URL → 空**（任意）

**9+ で問題ない**＝配信地域も機能も制限されない。実態と違う低い年齢を出す方がずっと危険（公開後に変更を求められる）。**提出前なら何度でも変えられる**。

---

## 7. App のプライバシー（データ収集の申告）＝「収集する」で出す

**方針（2026-08-18 決定）**: 実態より**広めに申告する**。理由＝BYOK でも**写真は実際に端末の外へ出る**（本人が選んだ AI へ）。
狭すぎる申告は差し戻しの定番だが、広めの申告で落とされることはほぼ無い。

ASC ▸ App のプライバシー で、**すべて「アプリの機能」・「ユーザーに紐付けられていない」・「トラッキングに使用しない」**として申告する。
（⚠「紐付けられていない」の根拠＝**アカウントも識別子も無い**。アプリは誰の記録かを一切知らない）

| 申告するデータ型 | 何が該当するか | 用途 | 紐付け | 追跡 |
|---|---|---|---|---|
| **写真またはビデオ** | 読み取りのために送るレシート・料理・健診・薬・からだの写真 | アプリの機能 | しない | しない |
| **購入履歴**（Financial Info ▸ Purchase History） | レシートから読み取った店名・金額・品目 | アプリの機能 | しない | しない |
| **健康**（Health & Fitness ▸ Health） | 健康診断の結果・薬・血圧・体重・体組成計の数値 | アプリの機能 | しない | しない |
| **その他のユーザーコンテンツ** | 本人が書いたメモ・AI への依頼文 | アプリの機能 | しない | しない |

**申告しないもの（理由は審査メモにも書いてある）**

- **位置情報**: 位置情報の権限を要求しない。写真の GPS も読まない。💴 支出マップの座標は「**店の住所の文字**」から引いたもので、本人の居場所ではない
- **連絡先・識別子・使用状況データ・診断データ**: アナリティクスも広告 SDK も入っていない

---

## 8. スクリーンショット ✅ 済み（2026-08-23・5枚）

**枠は 6.5インチ 1284×2778**。⚠ **機種の解像度ではなく、ASC の画面で開いているスロットに合わせる**。
6.9インチ（1290×2796）で作って **「1個以上のスクリーンショットの寸法が正しくありません」** で弾かれた（ASC が挙げたのは 1242×2688 / 1284×2778＝どちらも 6.5インチ）。あの日も 6.5インチのスロットで出している。

**出した5枚**（順番＝§4 の「伝えたいこと」と同じ物語）

| 順 | 画面 | なぜこの順か |
|---|---|---|
| 01 | **レシート1件の詳細**（店名・日付・金額・品目の表） | 「**撮るだけで書き起こされる**」の証拠。1枚目がいちばん効く |
| 02 | **🍽 食事**（料理の写真＋下の帯の 📷） | 2つめの柱。**📷「撮る」が写っている唯一の絵**＝どうやって入れるのかが分かる |
| 03 | **📈 健康の面から**（AI の助言が本文で出ている） | **このアプリの中心**。他の家計簿と違う所なので必ず入れる |
| 04 | **⚖ 体重**（体重＋体脂肪・骨格筋のグラフ） | 健康が柱であることを見せる |
| 05 | **📊 統計**（分類バー・お店・日別） | 貯めた先に何が見えるか |

**撮る時のコツ（実際にやり直した点）**
- ⚠ **iOS の通知が写り込む**（「背面タップ ダブルタップが検出されま…」）→ 撮り直し
- ⚠ **TestFlight から開くと左上に「◀ TestFlight」が残る** → ホーム画面のアイコンから開き直す
- ⚠ **上のタブが途中でスクロールして切れている**と壊れて見える → いちばん上まで戻してから撮る
- ⚠ **下の帯が中身に重なる** → 行の境目に来る位置までスクロールする
- ⚠ **中身が入った状態**で撮る（空の画面ばかりだと魅力が伝わらない）
- ⚠ 実際の店名・金額・体重が写る＝**人に見せたくないものが無いか**確認してから出す

**🛠 大きさをそろえる道具**（1px でも違うと ASC は弾く）

```bash
python receipt-spike/scripts/asc-shots.py <スクショを入れたフォルダ>
```

やること3つ＝① **1284×2778 ちょうど**にする（既定＝6.5インチ／`--69` で 6.9インチ。比が違う時は**はみ出すぶんを切る**＝黒帯を足さない）② **透過を外す** ③ **ファイル名順に 01_ から番号**（`1_receipt.png` のように番号付きで渡すと `01_receipt.png` になる）。出力は入力フォルダの中の `asc/`。

⚠ **iPhone から PC へ送る経路で写真は縮む**。今回届いたのは **960×2079**（元は 1290×2796 のはず）＝1.34倍に引き伸ばすことになり、細かい数字の輪郭が少し眠くなった。**次はケーブル（エクスプローラーの「Apple iPhone ▸ Internal Storage ▸ DCIM」）から直接コピーする**。メールなら「実際のサイズ」、Google ドライブなら元のまま上がる。

---

## 9. 提出後 / 承認後

- [ ] 審査用の APIキーを**失効させる**（⚠ いちばん忘れやすい）
- [ ] 承認 → ASC で「**このバージョンをリリース**」（手動リリースを選んでいるため、押すまで公開されない）
- [ ] Play 側も r104 へ（vc5・[BUILD-android.md](BUILD-android.md)）＝web・iOS・Android の3つを同じ版に揃える
