...
⚠ **この変更の前に回したビルドは使えない**＝**Codemagic を回し直す**（iPhone 専用になった 1.0(9) 以降を提出する）。
...
**A. 出す前に直すもの（コード・文書）**
- [x] ① iPad を外す＝`TARGETED_DEVICE_FAMILY = "1"` に変更（2026-08-23 済み）→ ⚠ **Codemagic を回し直す**（この変更を含んだビルドを提出する）
...
- [ ] ⑨ **ビルドを選ぶ**: ⚠ **iPhone 専用に変えたあとのビルド**（1.0 (9) 以降）。1.0 (8) は iPad も対象のままなので使わない（⚠ 一覧に古いビルドしか出ない時は**保存 → 再読み込み**で全部出る＝あの日でハマった罠）
