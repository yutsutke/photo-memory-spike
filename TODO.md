...
## 現在地 — BUILD: phase3.22 (📅から飛び飛びの日/期間を追加)

> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.22`。git clean & push 済み。**v49〜v71 の地図周回は全て preview E2E green。ただし v56〜v71 はまとめて実機 (iPhone Safari) 未確認**。
> - **いまの地図モデル（v64〜v71 で確定。旧 v62 の seed/意味echo は撤去済み）**:
>   - 表示は**入口で決まる**。状態は `mapState.pickDays`（表示する日の集合）。**🗺ヘッダ＝ランダム3日**（各日の全GPSを別色の軌跡に・🎲もう一度引く・軌跡1本タップで他を暗く）/ **📷写真入口＝その写真の日**。
>   - **非連続の複数日は「全期間（ブラウズ）」で**。入口は2つに揃った（v71）：①タイムラインの日付タップで出し入れ ②**📅パネルの「＋ 追加」**で日／期間をカレンダーから足す（数年前の日もスクロール不要）。両方とも `pickDays`/`days` モードに集約＝相互に出し入れ可。各日が別色で重なる＝2旅行の重ね合わせ。📅パネルの「適用」＝連続範囲だけ表示、は別モード（排他）。
>   - **タイムライン折りたたみ**（入口モードのみ）：日見出しタップ＝その日の写真を畳む/開く（▾/▸）。**畳んだ日は地図からも線・マーカーごと消える**（残った日の色 index は安定）。
>   - 線の色: 複数日=パレット（昔→今, v58）/ 単一日=時刻→空の色（v63, `skyColor`）。markercluster クラッシュは `removeOutsideVisibleBounds:false` で根治（v66）。
>   - 設計判断・ハマり詳細は CHANGELOG v49〜v71 と memory [[map-view-unparked]] に集約。
> - **最有力の次手 = 実機 (iPhone Safari) で地図周回（v56〜v71）を触る**（手元作業）。観察: ①ランダム3日の軌跡から想起できるか／3日が離れすぎないか②**📅「＋追加」で数年前の日に一発で届く快適さ／多日範囲が各日別色の独立軌跡で旅として読めるか**③折りたたみの手触り④（v56-61 積み残し）封印の範囲×期間・位置/日付編集の操作感。
> - **大きく動くなら**: プロダクト化 = HOW Phase 0-1（Capacitor 実機 + ネイティブ写真ライブラリ全件アクセスの de-risk）。足場リポ `madeleine` は parked。
> - **将来案（要望が固まれば）**: 📅「＋追加」で足した期間を unit（trip 単位）でラベル表示・削除／多日範囲を v58 風の1本連続線で（`pickRanges` 化）／畳んだ日に枚数「(N枚)」表示／days・browse でも折りたたみ。
