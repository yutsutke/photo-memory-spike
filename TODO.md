...
>   - **Phase 0 登録・準備**: [x] Apple Developer 登録($99)＝**申込・支払い済み 2026-06-18**(注文 W1884878174・承認/有効化待ち) [ ] Small Business Program(手数料15%・**有効化後に申請**) [~] Node+Capacitor 確認(Node v24/npm OK・Capacitor CLI 未導入) [~] プライバシーポリシー **ドラフト作成済み**(`privacy.html`・日英トグル・自己完結=外部依存なし／公開URL=yutsutke.github.io/photo-memory-spike/privacy.html／**要: 氏名+連絡先メール記入**・広告/IAP 節は実装時に最終確定)
