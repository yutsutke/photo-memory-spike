...
- **取り込み準備が長い時のガイド**(v34) — iPad 第8世代 (古い) で 10枚超の取り込みが「準備中」スピナーで 30分以上 stall (iOS 側の HEIC→JPEG 準備が古い端末で激遅、web 不可侵)。→ `preparing` が 25秒超で「少なめ(10〜20枚)に分けて」とガイド表示。回避策は古い端末では小分け取り込み (dedup で安全)。**要追加調査: 小分けでも stall するなら importOne に per-photo timeout (app側 wedge 対策)**
