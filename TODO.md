...
>
> ## 状態（2026-07-04 セッション19 終了時）
> - **web** = GitHub Pages `phase3.101`（v148 まで・思い出の場所改名/マルチ地点/タイムライン統一 反映済み）。
> - **iOS** = **1.3 を TestFlight にアップロード成功**（MARKETING_VERSION 1.2→1.3）。1.2(29) は審査提出/承認済み（train 締切）＝**1.2 を release するか / 1.3 を次に審査提出するかは次セッションで判断**（Gmail で 1.2 の状態確認）。
