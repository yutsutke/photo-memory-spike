...
> 3. **web**: GitHub Pages `phase3.128` 全 push 済（**v173＝🎞️撮り重ねるカメラ：基準写真をスワイプでフォルダー内の別写真に切替（ゴースト/2画面とも・‹›or左右スワイプ）＋📌でそのまま基準写真に確定＋2画面が横持ちで左右分割（縦=上下/横=左右・回転で自動）**。ほか v168-172）＝**実機の手触り FB 待ち**（スワイプしきい値 48px/横優勢 1.4× が構図合わせと干渉しないか・横持ち分割・回転の再レイアウト）。
