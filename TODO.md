...
>   2. **v119-v127 を実機確認**（v120-v127 は **GitHub Pages で今すぐ web 実機可**・v119 は native）: v127=①ログ逆時系列 ②間隔で時間を感じる ③一覧→ログ ④ログ写真の拡大スワイプ ⑤ログ↔一覧/比較/足跡の地図 の相互移動／v126=popup日時・一覧見た目・ログ(コメント/代表写真/未来の予定)／v125以前=地図から登録・🎲1日・比較スワイプ 等／v119=起動プロンプト・差分・電池。
>   3. **記念日が刺さったら → native フェーズで 未来半分＝iOS カレンダー書き出し**（EventKit 橋・`createEvent`・**write-only** `NSCalendarsWriteOnlyAccessUsageDescription`・fire-and-forget・イベント中身に歴代の足跡＝「過去が未来の燃料」・iOS 先行/Google は将来）。**planned entry から「🗓 カレンダーに追加」で接続できる形にした**。
>   4. **承認後 v1.1**: 英語名 A Past Day／iPad／位置ロガー復活／広告（[[monetization-v1-adfree]]）。v111-127 も承認後の次版に。
>   - **状態**: web=GitHub Pages `phase3.80`（v119-v127 反映）／native=ビルド16 が審査中（✅16 のまま）・17/18 は TestFlight 済（差し替えない）。**Gmail 監視継続**。受信確認リマインド＝anohiapp@gmail.com。**📝 Notion: WHY に curation 境界／HOW にカレンダー書き出し方針＋planned entry は pull のみ を記載済(v121/v126)**。
>
