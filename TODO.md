...
## 現在地 — BUILD: phase3.34 (トップに「過去の今日」フィルタチップを追加＝v84)
...
> **v84 = トップ画面に「過去の今日」フィルタチップ**（全期間→過去の今日→この時季→…）。`pickPool('today')`＝今日とちょうど同じ月日（年問わず・地図 🗺 today と同条件）。チップから引く／🗺 で重ねて地図化、の対。preview E2E green・**チップから引く実機手触りは未確認**。詳細 CHANGELOG v79–v84・memory [[on-this-day-daily-entry]]。
