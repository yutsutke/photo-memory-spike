...
## 現在地 — BUILD: phase3.67（v114：地図の足取りの線を見やすく＝暗い縁取り(casing)＋太め。preview 構文green・**見た目は GPS 写真のある実機/web で要確認**。｜🎨UI改善ロードマップ（実機フィードバック起点）：**塊A 地図線=✅** ／塊B 足跡・タイムラインのスワイプ拡大（showFullImage をリスト対応に）=▶次 ／塊C 連想ウォーク6枚→1枚スワイプ（deck/makeBigCard 流用）。｜v111-113＝トップ1枚スワイプ＋微調整＋ヘッダ整理（実機YES「完璧」「すっきり」）。｜v110：✅再提出済み＝「審査待ち（2回目）」→**審査結果待ち**→承認後 ASC で手動公開。英語名 A Past Day — Photos and Footprints・v1.1。📝次段＝起動時の自動差分取り込み(native)。）
