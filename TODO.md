...
- 🔬 **取り込み18秒の切り分け計測中** (v8) — 200枚で☑→次画面が18秒。仮説は「iOS ピッカーが準備中 (`change` 前、web から手出し不能)」。`+押下→取込開始` と `取込処理` を画面に分離表示して確定待ち。**実機の数値が出たら計測コードを外す**
- ▶ **計測結果次第の分岐**:
  - iOS 側確定なら → 分割取り込み最適化 (50枚ずつを滑らかに、v7 キュー合流) or 「ファイル」アプリ経由で HEIC 原本受け取り→背景パイプラインで自前変換 (iOS 変換を回避)
  - 我々の処理側なら → fast-track 枚数 (6) / dbGetAll の全 blob ロード軽量化
- ▶ **その他の選択肢**:
