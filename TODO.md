...
>   - **A＝本命: 残りストア素材** ① **スクショ**（実機キャプチャ・6.7"／6.9" 1セット〜＝内部テストで撮る）★ユーザー作業（撮る画面は提案済: 連想ウォーク/過去の今日/地図の足跡/全画面）② 掲載テキスト＝ドラフト済→微修正で確定 ③ **ASC プライバシーラベル＝「データを収集していません」**（v1 広告なしで確定・manifest と一致）④ 年齢レーティング 4+。← ✅ アイコン(v107)・✅ プライバシーポリシー URL・✅ カテゴリ/主言語(ja) は済。ASC 操作は Chrome 拡張で一緒に。
