...
## 現在地 — BUILD: phase3.60 (v108：版表示をヘッダ→ℹ️情報モーダル＋consoleに移動（スクショ/本番すっきり）＋?shot で web を v1見え（位置UIオフ）にするスクショ用モード。v107=アイコン差し替え。✅ 1.0(11) で ITMS-90683/91053 警告ゼロ確認済。案B＝コア先行・ストア素材フェーズ（次はスクショ撮影）)
