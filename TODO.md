...
- **(30k 弱点) 起動時の全件読み込み** — 今は起動時に全レコードをメモリに読む。数万枚で起動がもっさりしたら → thumb を別ストアに分け、起動はメタデータだけ読み、サムネは表示分だけ都度読むレイジー化 (schema 移行を伴う中規模改修)。間引きでは消えない別軸
