...
>   - **v225 🌀web背景取り込みの適応ペース**: 固定220ms→`clamp(処理時間×0.35, 16, 250)`＝速い端末≈5倍速・遅い端末は自動で息継ぎ。preview E2E ✅（hidden タブ=遅い端末相当で work1025→rest250 clamp）。
>   - **v226 📊区間計測**: importOne/importOneNative に区間タイマー→終了時「📊 N枚 平均Xms/枚（内訳）」logInfo＋直近を localStorage→⚙情報末尾に常設。**古い端末FBは「⚙情報の📊行を教えて」で数字になる**。preview E2E ✅（hidden の遅さを「縮小1018ms」と正しく指した）。
>   - **v227 🌅fast-trackを全期間サンプリング**: 最新6枚→`spreadIndices` で両端含む等間隔6枚＝数秒で昔↔今に灯り。ユニット ✅。
>   - **v228 🚀nativeサムネのバッチ×並列（本丸）**: Kotlin/Swift に `thumbnails(ids,size)` 同契約バッチ（Android=`thumbPool`(コア数-1, 2..4) 並列デコード・従来は**単一スレッド直列がボトルネック**だった）。JS `processNativeRest` をバッチ16＋**パイプライン**（次バッチを先に投げてから今の分を JS 処理＝native と JS が重なる）に。失敗時は1枚ずつへ自動フォールバック（withTimeout 付き＝従来より頑丈）。Kotlin コンパイル ✅ EXIT=0・**iframe に Capacitor モック注入で本物の importNativeLibrary を preview E2E ✅**（45枚→fast6が2019-2025に分散・バッチ[16,16,7]・プリフェッチgap 0ms・EXIF格上げ/GPS同乗 ✅・故障→単発30回フォールバック ✅・📊「サムネ取得18・色3…」）。
> - **🎉 Android エミュレータ実 E2E 完走（v228+v229）**: debug APK 手元ビルド→Pixel_7 エミュに 40枚 push→自動差分取り込みが logcat で `thumbnails` バッチ **[16,16,8]**（単発なし）→「✓ 40 new」。**📊 行が出ない発見→v229＝runAutoImport に impTimeReset 入れ忘れ（3つ目の入口）を1行修正**→25枚追加で **📊「25 photos, avg 47ms/photo（サムネ取得61・色18・保存17・変換7・重複4）」が実機点灯＝壁47<合計107ms/枚＝パイプラインの重なりを実測確認**。ハマり＝emulator offline 張り付き→スナップショット破損＝`-no-snapshot` コールドブートで復旧／adb push だけでは MediaStore に入らない→MEDIA_SCANNER_SCAN_FILE broadcast で1枚ずつスキャン。テスト後清掃済（v228/v229 ファイル削除・debug アプリはアンインストール）。
> - **▶ 次の一手**: ① **vc4 を作って Pixel 7a 実機確認**（v220-229 全部入り。⚙情報末尾=phase3.180 で確認・📚取り込みの体感と📊行・地図上部コントロール崩れの watch は継続）② iOS も次ビルド（1.6 審査結果次第で 1.7 train）③ 古い端末FBが来たら「⚙情報の📊行」を回収→v228 の効果を数字で確認（バッチ16は決め打ち＝bridge支配が続くなら24-32も）。
>
> **🔧 セッション38（2026-07-15）＝v215〜v220。phase3.171。vc3（署名付き・v215+v216）Play 内部テスト公開→🎉Android 実機で「写真アクセス/トップ重なり/位置ロガー 完璧」＝main 機能が揃った。残FB2点を v220 で修正済み（次ビルド vc4 で反映）。**
> - **🎉 vc3 Android 実機FB（2026-07-15）**: ✅写真アクセス完璧・✅トップページ重なり完璧（v216 効いた）・✅🛰️位置ロガー完璧。🔧拡大表示が電池と重なる／タイムラインをもっと上まで → **v220 で両方修正**。
> - **v221（web FB）**: タイムラインを開いたら地図コントロール（過去の今日/🎲/⋯/⛶）を隠す＝上まで来た時に被って見づらい→`.map-overlay.sheet-open` で display:none（✕は残す・畳めば戻る）。overlay に sheet-open を JS 同期（開閉3経路）。preview 検証済（開でctrlbar/⛶=none・✕残る・畳みで復帰・console 0）。web 即反映。
> - **v222（実機FB③）＝✏️手書き 🎯起点→終点で「任意の場所」も端点に**。近くに点があればスナップ／無ければ ✚と同じ自動時刻（`handAutoTime` に抽出・共通化）で実点を作り端点に。preview E2E 検証済。
> - **v223（実機FB③続き・モックつき）＝🎯を「起点/通過/滞在/終点」を選んで置く方式に**。点種セレクタ（`.hand-seg-types`＝4チップ・現在値表示・アクティブ強調）を追加、`handSegTap` を `sg.segType` 分岐＋自動送り（起点→終点→通過）。確定「√線を引く」（起点<終点で有効）＋「×やめる」（下書き破棄）。逆順/同時刻は確定不可＋案内、finalizeSegDraft も 起点<終点 の時だけ焼き込み。preview 実データ注入 E2E 検証済（起点10:00→終点10:10→通過按分10:03→滞在按分10:06☕・チップ状態逐次更新・×やめる・✚モード復帰・console 0）。web 即反映／iOS/Android native は次ビルド（vc4）。
> - **v220（Android 実機FB2点）**: ①拡大表示＝`html.cap-android .full-deck{top:var(--sat)}` で写真をステータスバー直下へ（電池が黒帯に乗る・iOS/web 無影響）②タイムラインシート＝`calc(100dvh - var(--sat) - 8px)` で status bar 直下まで最大化（開くと地図コントロールを覆う→ハンドルで畳む）。preview 検証済（sheet 776px・上端36px・full-deck top 28px/web 0・console 0）。web 即反映／**iOS/Android native は vc4 で実機再確認**。
> - **v218/v219**: ℹ️使い方「連想の歩き方」を現行スワイプUIに更新／🫧囲んで見るを「地図タップで中心→スライダー」に（📍いまの場所と同UI 流用）。両方 web 即反映。
> - **v218**: ⚙情報「🎲 連想の歩き方」が旧「3枚引く」のまま→v111 スワイプ化に合わせ「1枚ずつ横スワイプ」に更新（ユーザー「過去の話」）。web 即反映。
> - **v219（囲んで見る・実機FBでドラッグが難しい→ユーザーがスライダー選択）**: 「地図タップで中心→スライダーで半径」に作り替え。📍いまの場所と同じ `.here-panel` UI／`hereSliderToRadius` を流用（出荷済み here コードは不変）。円 interactive:false でタップ透過＝中心を置き直せる・地図パン/ズーム有効のまま（Leaflet click はドラッグで非発火）。attach 経路（別の場所）保持。preview 検証済（タップ→円+パネル+スライダー・501m→4km・中心移動・やめるで清掃・console 0）。go≥2枚経路は openEncircleTimeline 不変ゆえロジックのみ→実機で最終確認。**web 即反映**。
...
> - **✅ vc3 は署名付きビルド完成・検証済み**（`madeleine-1.0-vc3.aab` 4017KB／署名 MADELEIN.SF・RSA／versionCode 3／phase3.167＝v215カメラ+v216ステータスバー入り）。手順のハマり所は [[android-local-signing-build]] に集約（複数行ペースト割れ・Gradleデーモンの env キャッシュ・ダブルクォート・**未署名で一度 Play に弾かれた**→CM_* 4変数を `Get-Content|Invoke-Expression` で入れ `--no-daemon` で解決）。
> - **▶ 次の一手＝Play 内部テストへ vc3 アップロード→Pixel 7a 実機で確認**: ①⚙情報末尾が **phase3.167**（vc3 が入った確認）②トップ header のボタンが時計/電池の下＝押せるか（本命）③地図の ←/📅 がバーに重ならないか（本命）④🎞️カメラ＝許可ダイアログ→許可→映る（「⚙設定を開く」が出たら過去の拒否が残っていた証拠）⑤🛰️位置ロガーが距離モード表示。②③が直っていれば「端末で挙動が割れる」真因診断（WebView≥140 パススルー）が的中。
>
...
> - **✅ vc2 は作成→Play 内部テスト公開済み**（v209 カメラ/ステータスバー入り・手元 Windows で `BUILD_NUMBER=2`＋`gradlew bundleRelease`・JAVA_HOME=Android Studio jbr／控え `madeleine-signing\madeleine-1.0-vc2.aab`）。Pixel 7a の更新ボタンは反映待ちだった。
> - **▶ 次の一手**: 1) **vc3 を作って実機確認**（`$env:BUILD_NUMBER="3"`→`npm run sync:web; npx cap sync android; cd android; .\gradlew.bat bundleRelease`。JAVA_HOME は恒久化済み・署名 env は CREDENTIALS.txt）＝**v209 カメラ/ステータスバー ＋ v211 位置ロガー背景（50/150/500m）**をまとめて実機で（vc3 が vc2 の上位版）。iOS も vc3 相当を Codemagic で。2) ③**取り込み速度**（古い端末）＝web の残課題（進捗の見せ方/チャンク/メモリ解放。既に 50枚バッチ＋停止/差分再開はある）。3) 「アプリのコンテンツ」入力（プライバシー/データセーフティ/レーティング/対象年齢＝Android 製品版公開の必須）。
>
> **🎉 セッション33（2026-07-13）＝Android が Google Play 内部テストで実機インストール成功（ロードマップ③ Android デビュー）。**
> - ✅ アップロード鍵生成（keytool・**リポ外** `Documents/madeleine-signing/madeleine-upload.jks`・自己署名2053まで・Play App Signing 用）→ **手元 Windows で署名付き AAB ビルド**（Codemagic 不要・JDK21/SDK36・`gradlew bundleRelease` ＋ env `CM_KEYSTORE_*`）→ `app-release.aab` 3.94MB/vc1 → Play Console 内部テスト → **Pixel 7a インストール成功**（ユーザー「インストールできました！！！」）。
> - 🔧 途中修正: `android/app/build.gradle` の versionCode 行の Groovy パース罠（ローカルで `versionCode(null)`）を曖昧さのない形に（**CI 不変**）。gradlew はフルパス＋cwd=android で実行。
> - **▶ 次の一手**: ①**実機FBの「小さな修正」**（ユーザーが触って気づいた点・要具体化）②「アプリのコンテンツ」入力（プライバシー/データセーフティ/レーティング/対象年齢＝**製品版公開の必須**）③ストア掲載文（日英）＋スクショ（café流用・Android サイズ）。次AABは **versionCode を増やす**。
> - iOS は 1.5 審査完了（配信可能・自動リリース）／手元は 1.6(47/48) を TestFlight で回している。挨拶時は [[session-start-gmail-check]]（Apple＋Play Console 通知）。

---

## 現在地 — BUILD: phase3.156（**🎞️v204＝基準確定を📌アイコンのみ＋右上へ（実機FB「上部中央だと今度は上の写真にかぶる・📌アイコンだけにして🖼基準の下あたりに」）＝テキスト帯（画面幅pill）→📌アイコンのみ円ボタン(42px・テキストはtitleへ)で被り面積を最小化・位置は上部中央→右上（✕の下 top62/right12・✕と10pxギャップ）＝写真の中央/上を避け全モードでタグ(📷いま左/🖼基準中央-split)や⚙(左)と非競合。※基準ピンは📌統一（📍でなく）。⚙説明も「右上に📌が出る→タップで確定」に更新。表示条件（基準バー非表示かつ基準以外表示中）とpinCurrentは v202/203 のまま。preview E2E green（375px：アイコン📌のみ・top62/right12・✕の下gap10・シャッター上でない・タップでrpBasePhotoId=R2永続／2画面でも📷/🖼タグと非重複・console0）。教訓＝「写真にかぶらない」の最終解は位置だけでなく面積を減らす（テキスト→アイコン）。｜🎞️v203＝基準確定チップを写真にかぶらない位置へ（実機FB「挙動は完璧・分かりやすいが邪魔・写真にかぶらないように」）＝下部コントロール（シャッター上＝写真の下寄り中央にかぶる）から上部中央（⚙左/✕右の間の UI 帯）の absolute 要素へ（オーバーレイ直下・z7・max-width で⚙/✕回避）。あわせて基準バー表示中はその📌があるのでチップを出さない（重複＋横持ち上部中央の競合回避）＝チップは「基準バー非表示 かつ 基準以外を表示中」の時だけ。preview E2E green（375px：チップ top10-47・中心x188・シャッター上でない・⚙/✕非重複／基準バーONでチップ消え📌が出る/OFFで復活／新位置でもタップ確定rpBasePhotoId永続・console0）。教訓＝一時アクションチップは操作対象（写真）の主要部を避け chrome 帯へ・同機能の別UIと同時に出さない。｜🎞️v202＝基準写真の📌ピン止めを見つけやすく（実機FB「基準バー非表示だと📌が出ず撮影時の基準写真ピン止め方法がわからない／スワイプしても基準は変わってない」）＝原因＝スワイプ(ステージ全面)は基準バー表示ON/OFFに関係なく効くのに📌は基準バーの中だけ→OFFだと確定できなかった。→📌を独立させ「📌 この写真を基準にする」チップ新設（.repho-setbase・スワイプで基準以外を表示中は常に出る・基準バー表示と独立・タップでrpBasePhotoId確定rpPersist・基準に戻すと消える）。pinCurrent に共通化。⚙ヒントもチップ言及に更新。挙動整理＝スワイプは下敷き(候補)を変えるだけ・確定は📌チップ・ピン止めしなければrpBasePhotoId不変(次回は基準から)。preview E2E green（基準バーoffでスワイプ→チップ出現→確定rpBasePhotoId=R2→再オープンでR2から・DB永続・console0）。教訓＝操作が効く条件と確定手段の可視条件を一致させる。｜🐛v201＝細かい修正2点（実機FB）＝①トップの⚙メニュー/モーダルを開いている間、背景（本体ページ）のタッチスクロールが抜けて後ろが動く/傾け直すと一緒に動く→document の touchmove を捕まえ、メニュー(.menu-dropdown)/モーダル(.ai-modal)が開いていて中身(.ai-modal-card/.menu-dropdown)の外の touchmove を preventDefault（position:fixed ロックは sticky ヘッダーで画面ジャンプ→不採用・全面オーバーレイは自前で覆うので早期return）②重ね撮りカメラ⚙パネルに基準写真スワイプの説明「💡画面を左右にスワイプで下敷き（基準）を選べる・📌で確定」＝フォルダーに2枚以上の時だけ表示・基準バー表示ON/OFFと独立。preview E2E green（touchmove 6ケース分岐一致・A2枚ヒント表示/B1枚非表示・console0）。教訓＝iOSのモーダル背後スクロール抜けは position:fixed だと sticky ヘッダーでジャンプ→レイアウト不変の touchmove 抑止（中身の外だけ）が素直。｜✨v200＝遷移のちらつき解消＋🤝地図トグルを「写真・足あと」まとめてに（実機FB2点）＝①拡大→「地図でこの場所を見る」で本体画面が一瞬見えるちらつきを解消＝先に地図(z150)を作り拡大(z2000)を上に残したまま下地を用意→最後に拡大/フォルダーを外す（v199の「後で開く」は openMapView の loadLeaflet 待ちで隙間が空いていた・特に思い出の場所で顕著）。計測で closeFullImage の瞬間に map-overlay が既にDOMにあることを確認。②「🖼写真を表示/隠し」→「🗺 地図に表示中/非表示（写真・足あと）」＝写真だけでなく相手の足あと（線/ドット/☕）もまとめてON/OFF（peerPackPhotosShown→peerPackShown・drawPeerLines/refreshTrack の peer 描画を gate・pref key は photos のまま後方互換・文言は中身に追従＝写真だけ/足あとだけ/両方・トグルは写真か足あとがあれば表示）。「共有」でなく「表示/非表示」に（受け手目線）。preview E2E green（ちらつき=map先・OFFで写真＋足あと全部0→ON復帰・rephotoは（写真）表示・console0）。教訓＝覆いを絶やさないなら「先に開いてから閉じる」／受け手UIは送り手目線の語を避ける。｜🐛v199＝拡大表示→「地図でこの場所を見る」で地図に飛ばずフォルダーに戻る不具合を修正（実機FB・重ね撮り/思い出の場所の両方）＝原因は z-index（地図 z150 へ移る時に下のフォルダー＝重ね撮り food-overlay z1900 / 思い出の場所ログ atl-overlay z5000 が畳まれず手前に残る）＋思い出の場所は openAnnivPhotoZoom の onClose=reopen が closeFullImage で発火しログ再表示していた二重要因。→ .to-map ハンドラで onClose 無効化＋closeRephotoSeries()＋closeAnnivTimeline() してから openMapView。preview E2E green（重ね撮り/思い出の場所とも写真拡大→地図が最前面・思い出の場所はログ再表示されない・回帰=普通に閉じたらログ戻る/通常写真の地図遷移OK・console0）。教訓＝「戻る」系は下レイヤー残留＋別経路の再オープン(onClose)の二重を疑う。次＝「写真表示中/隠し中」トグルの説明を分かりやすく（実機FB・別途）。｜🤝v198＝受け取ったパックが「帰る場所」へ（実機FB「撮り重ね/思い出の場所も地図に飛んじゃう・重ね撮りファイル/思い出の場所ファイルに入ってほしい・渡す時の軌跡は思い出の場所内のみを基本に」）＝①パックに型 ptype(rephoto/place)+title+circles（v:2のまま・型なし=日パック後方互換）②取り込み: rephoto→最古をroot(rpFolder+rpTitle)に子はrephotoOf＝重ね撮り一覧/シリーズ/比べるがそのまま動く（rephotoSeriesOf/AllSeriesにpeer分岐・🖼トグルと独立）／place→annivストアに_peerレコード（円検証・無ければ写真から近似）＝一覧/ログに👤で並び**あなたの同じ場所の写真も同じログに積み重なる**（photosInCircle live）③開く先を型で分岐（✉️手紙の主ボタン🎞️/📖/🗺・取り込み直後・🤝一覧の開く）＝地図に飛ばされない④プライバシー: placeの書き出しは軌跡を「円内だけ」にクリップ（行き帰り=家までの道は渡らない・ダイアログに円内N点を正直表示）・rephotoは写真のみ⑤受け取り側は見るだけ（シリーズの⚙/📤/撮り重ねる・場所の基準日/メモ/代表写真/＋続き/📍別の場所を隠す＝v151 transientゲート再利用・💬裏書きはログに読み取り専用）⑥削除=重ね撮り一覧/場所一覧の行からもパック丸ごと（annivも掃除・相手の端末には影響なし文言）。preview E2E green（rephoto往復=手紙🎞️→シリーズ画面（地図開かず・👤ユキ・アクションなし）・一覧own+peer2行／place往復=ダイアログ円内5点→**パックtrackは円内14時5点のみ=円外12時4点除外**→手紙📖→ログ（[←][🗺]のみ・own3+peer3=6枚混在）／日パック回帰=地図／削除2経路=自分のデータ無傷・console0）＝実機の手触りが次。教訓＝送り側の文脈（フォルダー）を型として運び受け側で同じ器に還元＝既存機構がタダで付いてくる／プライバシー既定は「渡す単位の意味」（日=その日ごと・場所=場所の中だけ）。残=native共有シート・受け取りフォルダーへの自分メモ/引き取りはFB次第。｜📤v197＝「渡す」を撮り重ね/思い出の場所からも（ユーザー要望「この仕組みめっちゃいい・撮り重ね/思い出の場所の各フォルダーも同じ仕組みで送れるように・3モード/タップ分離/保持/記憶 も採用」）＝共有ダイアログを source 一般化（kind=days 日パック[足あとも] / kind=photos 写真リスト元[写真のみ]）。撮り重ねシリーズ画面に [⚙設定][📤渡す][🎞️撮り重ねる]・思い出の場所ログに [←一覧][🗺地図][📤渡す][📍別の場所]→そのフォルダーの写真（rephotoSeriesOf/annivPhotoList）を丸ごと→贈り物ビルダー（全部/選ぶ+3モードグリッド+💬+✉️・記憶は共通）。写真リスト元は⏱窓なし・入れないなし・タイトル「『富士山』を渡す」・サマリ「写真だけを渡します」。buildSharePack を spec 化（写真絞り込みはダイアログが解決してphotos渡す＝両source同じ埋め込みロジック）・足あとは includeTrack（日パック）時だけ。受け手は無改修（普通の peer パック＝地図/TL/囲む/💬裏書き/✉️手紙が乗る・pack.days は写真日から導出＝同エリアの自分の日オーバーレイも効く）。配列渡しは後方互換で日パック（地図⋯📤は無改修）。preview E2E green（撮り重ね「富士山」3枚→📤→窓なし/全部選ぶのみ→listでF1コメント+F3外し→v:2/2枚/track0/コメント/days3・思い出の場所「富士山ビュー」→写真のみ3枚→受け取り実UI=手紙→地図→peer2枚コメント保持GPS維持+自分の2023自動オーバーレイ・日パック後方互換=窓/入れないあり・console0）＝実機の手触りが次。教訓＝日ベース機能を写真リストへ広げる時はデータ生成から絞り込みUIを剥がし呼び出し側に解決させる＝両source同じ生成ロジック共有・分岐最小。｜📤v196＝「選ぶ」画面に見せ方3択（v195直後のユーザー要望）＝①▦小さく（現状4列）②⊡大きく（2列・写真大きくチェックで選ぶ）③☰コメント（写真とコメントを上下に・1列でずらーと＝コメント欄タップで書きながら選ぶ）。v186 の getViewPref で次回も同じモードで開く（surface=sharePick）。small/big はセル共通（正方形+💬ボタン・列数だけCSS）・list は別レイアウト（写真contain+下にコメント欄・写真タップ=選択/コメント欄タップ=編集）・モード横断で選択/コメント保持。preview E2E green（3モード切替=4列/2列/縦並び・list実操作でコメント編集&写真タップ選択・保持・big書き出し→再オープンbig・375px縦積み実測・console0）。ハマり＝トグルdivに share-seg と share-viewseg を重ね付け→写真モードのハンドラがトグルも巻き込み st.mode=undefined→単独クラス化+CSSカンマ結合で解決（教訓＝スタイル用クラス再利用はイベント委譲も付いてくる）。｜📤v195＝「渡す」を贈り物ビルダーに（v194直後のユーザー要望3点）＝①⏱時間帯まで選べる（開始〜終了の1本窓・既定=全範囲・1日なら午後だけ/旅行なら出発〜解散・写真/足あと両方に適用・件数ライブ追従）②🖼写真=全部/選ぶ/入れない 3択（選ぶ=窓内グリッドをタップで入/外・初回全選択=家族に数枚外す/他人に数枚だけ両対応・全選択/全解除・窓に追従・120枚チャンク）③💬写真ごとのひとこと（💬→プレビュー付きミニモーダル200字→受け手は p.peerComment にレコード永続＝popup/拡大/TL行に裏書き表示）＋✉️全体のひとこと（500字→受け手はprefs＝取り込み直後に「✉️手紙」モーダル（名前+本文+🗺地図で見る）＝贈り物を開ける瞬間・無しは地図直行・一覧行に✉️1行）。パックv:2（v:1後方互換=フィールド単位）。選ぶ/書く行為自体が想起=editing-triggers-reminiscence（写真の裏に書く文化）。preview E2E green（窓10:30-11:30=写真1/点5検算一致・グリッド窓追従・選択保持・往復=書き出しv:2（外した1枚なし/cは該当だけ/note改行）→実UI取り込み→手紙モーダル→🗺→💬3箇所+一覧✉️行・v:1互換・console0）＝実機の手触りが次。教訓＝preview の取り込み検証は完了待ち長め（2秒超）＝「モーダル出ない」の正体はタイミング。残＝native共有シート持ち越し・日ごと時間帯/コメント追記（往復書簡）/常設キャプションはFB次第。**）｜🤝v194＝もらった思い出＝家族/友達と「日パック」を渡し合う（シェア un-park 第一手・ユーザー明示要求 2026-07-12・サーバーなし＝JSON手渡し）。ユースケース＝①過去の旅行を家族/友達に渡す②知り合った人と同じ旅行先の道を比べる（時間ズレ×同じ地域＝「重ね歩き」・重ね撮り「N年 apart」と地続き）。📤書き出し＝地図⋯表示「この思い出を渡す」→表示中の日々（pickDays/期間→写真日+足あと日＝v190の写真ゼロ手書き日も渡せる・上限31日）→名前+「🖼写真も入れる」（外すと足あとだけ数十KB＝知り合ったばかりの人用）→共有シート（iOS Safari 15+）/DL（⚠️native WKWebView は navigator.share 無し→正直に「次版で対応」案内＝@capacitor/share を次ビルドで）。📥取り込み＝⚙「🤝もらった思い出」→ファイル→検証（app/kind/id・座標/時刻クランプ・上限 写真3000/点10万・**画像は createThumbnail 再エンコード＝正規化**・名前エスケープ・同idパックは confirm 置き換え）→写真=photos ストアに peerPack/peerName・足あと=track に src:'peer'+pack+name（**新ストアなし・DB移行なし・一覧はレコード走査から導出＝メタ別持ちしない**・🖼トグルだけ localStorage pms-peerPacks）→**取り込み後そのまま地図が開き「同じ地域にある自分の日」（bbox+2km・≤6日）を自動で重ね＋「（1.5年ずれ）」トースト**（7日以上は📅＋追加の案内＝地元洪水の防止）。混ざり先の境界（ユーザー指定）＝相手の写真は**地図・地図タイムライン・囲む（→思い出の場所ログ含む=circle由来）だけ**＝`visible()` に `!p.peerPack` を足して全域除外し、地図系の収集点だけ新設 `mapVisible()`（＝出る方向のミスが構造的に起きない）・**パック単位「🖼写真を表示」トグル＝写真だけ隠せて線は残る**。相手の線＝パック色の破線＋線頭に👤名前ラベル（`drawPeerLines`・自分の時刻順マージ v139 には混ぜない）・マーカー＝パック色の太枠・popup/拡大/タイムライン行は**読み取り専用**（位置直す/削除/日付直す/重ね撮りを出さない）。ガード＝`isPeerPt` で ロガー統計/日別削除/すべて消す/自動確定 autoConfirmTrackPass/推定配置の基盤/手書き骨格 handDaySeq から除外＝**友達の散歩に自分のGPSなし写真がピン留めされる事故を構造的に防ぐ**。一覧＝👤名前・期間・🖼N枚・👣N点・[🗺見る][🖼トグル][🗑パック丸ごと削除＝相手の端末に影響なし]。preview E2E green（書き出し=写真3+点9=11KB・track座標+時刻のみ・peer点は又貸ししない／実UI取り込み（file input に DataTransfer）→👤2枚+8点→**地図自動オープン pickDays=[ユキの2022-11-12, 自分の2024-05-03]＋「1.5年ずれ」トースト**／破線1+👤ラベル+☕+パック色枠マーカー（クラスタ内はDOM非存在＝ズームで出る）／TL6行=自分4行ボタンあり+ユキ2行👤バッジのみ／罠のGPSなし写真が確定されない（lat=null維持）／handDaySeq(ユキの日)=0／ロガー集計 自分9点・peer8点除外／トップのプール4枚に混入なし／トグルOFF=写真消え線残る→ON復帰／削除=写真/点/設定消え自分の4枚無傷・console0）＝**実機の手触りが次（共有シートでLINE/AirDropに乗るか・受け取りの一連・重ね歩きが心に効くか）**。教訓＝混ぜてよい場所が少数なら既定述語を厳しくし例外側を opt-in（プライバシーは「出る」方向のバグが致命傷）／同居+タグ+述語（hand の型）の再利用。残＝native共有（@capacitor/share）・E2E暗号リレーは手渡しの摩擦が実証されてから・「偶然モード」（peer をトップにも・パック単位）は次段候補。**）｜✏️v193＝期間絞り込み（クイック1週間・📅期間指定）からも手書きに入れる（実機FB「一週間を選択→✏️→どの日から直す？モーダルの場所がわからない」＝正体はモーダル不到達：クイックは pickDays でなく range を立てる→✏️入口が pickDays しか見ず「先に日を選んで」で弾かれていた）→range 中は期間内の暦日（写真ゼロ日も一級=v190）を日リスト化してモーダルへ（1週間=8日）・期間10日超は「期間が長すぎます（N日）…期間を短く（1週間など）」警告・モーダル各日に写真枚数「・N枚」（空の日は無印＝手がかり）・「先に日を」文言に期間の絞り込みを追記。preview E2E green（実UIクイック経由：R1 1週間=モーダル8日新しい順→選択で編集入り・チップ8・range保持／R2 1ヶ月=31日警告／R3 無選択=先に日を（期間言及）／R4 ・2枚表示／range中の描画=タップ→handMerge→線・console0）＝実機の手触りが次。教訓＝「一週間を選ぶ」もユーザーには日の選択だが内部は pickDays と range の2系統＝入口が片方しか見ないと「選んだのに弾かれる」。「見つからない」FBは「出ていない」を疑う。｜✏️v192＝複数日の手書き修正で「どの日から直す？」モーダルを復活（実機FB「🎲(単一日)は手書きできる／複数日選んだ時に先に日を選択するモーダルがどこにあるかわからない」＝v191でパネル内チップに統合したら入口が見えなくなった）→複数日で✏️→まず日選択モーダル（全ラベルの日ボタン）→選ぶと編集入り（アクティブ=選んだ日・v191は最新固定だった）＋パネル上部の日切替チップで他日にも切替可（数日まとめて修正は維持・ヒントは「別の日に切り替え」）。単一日/🎲(1日)はモーダル無しで直行・日が10日超は警告。preview E2E green（複数日=編集入らずモーダル3日ボタン→2022選択で編集入り・アクティブ2022・チップ3・pickDays非折り畳み／単一日=直行チップ無し／pickRandom3≤1日・console0）＝実機の手触りが次。教訓＝別画面の明示選択をパネル常設に統合すると省ステップだが入口が消え発見性を失う（合成検証では拾えないUX）→明示モーダル+常設チップの二段構え。｜✏️v191＝手書き修正の「1日固定」を緩和（ユーザー要望「数日の選択でも手書き修正したい・日数が多くて無理なときは押したら注意を」）＝複数日を選択中でも**折り畳まず**、パネルの**日切替チップ**（`.hand-daypick`・横スクロール・アクティブ日を強調＋その日へ地図を寄せる）で数日をまとめて修正できる。日が **HAND_MAX_DAYS(=10) 超**で✏️押下時に警告（📅🎲で絞る誘導）。既存レンダリング（refreshTrack の editDk＝アクティブ日だけ編集委譲・drawTrips は全日描画）にそのまま乗る＝折り畳みを外して日切替UIを足すだけ。旧 openHandDayChooser（別画面で日選択）は日切替チップに統合し撤去。単一日は従来どおり折り畳み（時刻色の線）。realN/フリーハンド/写真ゼロ日の入口(v190)は不変。preview E2E green（D1 単一日=折り畳み・チップ無し／D2 複数日3=折り畳まず・チップ3・アクティブ=最新／D3 日切替でタップが正しい日に（2023→1点/2021→1点/2022→0）／D4 12日=警告で入らない・console0）＝実機の手触りが次。｜✏️v190＝手書きモードを写真の無い日にも開放（ユーザー要望「位置つきが1点以下でも点を打ちたい・写真を撮らなくても振り返りたい日はある」）＝確定点（写真+ロガー）が2つ未満の日もフリーハンドで道を描ける。`realN` で分岐＝**≥2は v188 の按分をバイト単位で無改修（回帰ゼロ）**／≤1はタップ順＝道順で最寄り点の前後に入れ端の外側も許す（描き始め〜描き終わり）・空の日は正午起点。写真ゼロの日の入口＝📅フィルタ「＋追加」で**単日なら写真が無くても足せる**（range 空追加は従来どおり拒否）。線は保存せず点だけ（座標+時刻・src:'hand'）＝データ増加の歯止め不変。preview E2E green（A 空の日=入れる＋タップ順 noon→+1分→+2分＋beforeで前へ／B 確定点2枚=2枚の間に按分＝v188不変／C 📅単日追加→その日で手書き可・console0）＝実機の手触りが次。｜✏️v188＝手書きの移動線 実機FB反映：②☕滞在を分かりやすく（teal塗り+白縁+☕絵文字マーカー・通過は線のみで差が明確）③挿入の向きを明確に（v187「最寄り線分」は折り返す道で誤区間→「最寄り"点"＋前/後トグル」に変更＝パネル「◀近くの点の前／近くの点の後▶」既定=後）。①操作感・④入口の深さ は FB どおり無改修。preview E2E green（同一タップで after=12:39/before=11:45 と区間切替を検算・☕divIcon確認・console 0）＝実機の手触りが次。✏️v187＝手書きの移動線（ユーザー要望「その日の移動線を手書きで引きたい・確定点を起点終点に通過/滞在を簡単に打つ・線はロジックで」）＝地図⋯表示「✏️この日の道を手書き」→日を1日に固定→🚶通過/☕滞在の点をタップで打つ→点は最寄り区間の前後から距離按分した時刻を持ち v139 の時刻順マージがそのまま線を引く（手書き専用の線ロジックなし）。track ストアに src:'hand'(+stay) 同居＝DB移行なし・機種変更で一緒に移る。ロガー🗑削除は手書きスキップ・編集中は自動確定保留→✓終了で描いた道に GPSなし写真が乗る・軌跡トグルOFFでも手書きは線に乗る。preview E2E green（按分検算一致・連鎖挿入・undo・滞在切替・自動終了・console 0）＝実機の手触りが次。🧭v186＝タイムライン系4画面（地図TL/思い出の場所/重ね撮り/食べ物）の「最後に使った表示（一覧⇄写真・大きく/小さく等）」を localStorage 記憶し次回そこから開く（ユーザーFB「最後に使った画面から始めて」）。🎞️v185＝連続撮影ラベル簡素化（説明削除=ユーザー依頼）＋新規フォルダーのカメラに「1枚目を撮ると下敷きになります」ヒント（透過/2画面が最初だけ無い仕様が2度混乱を生んだ→empty-state説明・撮ると消える）。🎉アプリの連続撮影は新1.5ビルドで解消をユーザー確認。v184＝実機FB「iOSで連続撮影が変更できない」→原因はTestFlight 1.5(42)がv177より前＝新1.5ビルドで解消・webに保険3点（🔁を5番目へ/touch-action/overscroll）。🐘v180〜v183＝10万枚スケール対応（①取り込み🌀タップ→⏸停止＋新しい順＋残り時間 ②地図の線/写真を量で自動切替＝色分け/単色/密度タイル・白点・spotlight ③タイムライン窓モード＋写真グリッドチャンク＋全画面デッキ窓化 ④v183=取り込みメモリ解放（500枚ごとに thumb をディスク参照へ差し替え）＋ℹ️使用容量表示）は下の「再開ポイント」＋CHANGELOG v180-183 参照**）｜（以下は v179 までの履歴：v168〜v179＝Android AAB パイプライン／重ね撮りの削除・カメラロール保存・サイズ・スワイプ削除／回帰修正・思い出の場所スワイプ削除／基準写真スワイプ切替＋横持ち左右分割／カメラ重なり解消＋📌アイコン化／⚙パネル収納・全画面復活・前回モード記憶・🔁連続撮影／シャッター音＋連続撮影の増分反映バグ修正）は下の「再開ポイント」＋ CHANGELOG 参照。以下は v167 までの履歴**）｜🗺️**v167：地図タイムラインの「🖼写真」タブに「⊡大きく/⊞小さく」トグル追加（実機FB）**＝思い出の場所/重ね撮りと同じ `.atl-photobar`/`.atl-size` を流用・状態 `mapState.tlBig`・`buildTimelinePhotos` の big 引数へ（`.atl-grid.big`=2列 ⇔ 4列）・絞り込みは `timelineFilteredPhotos` で従来どおり尊重・写真タップ→全画面拡大は既存のまま。preview green（地図→写真タブ→大きく=2列/小さく=4列・文言連動・console0）。｜🎞️**v166++：重ね撮り設定の再描画バグ修正（実機FB）**＝⚙設定/題名✎の変更（位置トグル・基準写真の枠・基準日・タイトル）が実機iOSで**その場で反映されず「閉じて開くと反映」**だった→原因は全部 `await dbPut(root)` の後に再描画＝**iOS Safari は IDBから読んだ Blob(thumb) 入り写真レコードの再put で tx が hang しやすく**再描画に到達しない（in-memory値は先に変わる）。→**先に更新＋再描画・保存は裏（`rpPersist`・失敗はlogErr）**。基準写真ピッカーは選択後もピッカーに留まり枠を即移動。preview で dbPut を**永久hangに差し替えても4系統すべて即反映**＝再現&修正確認／通常dbPutでDB永続も確認・console0。｜🎞️**v166+：重ね撮りUI整理（ユーザーFB）**＝①**基準日の設定を⚙設定の中へ**（ログの基準日バー撤去・値による「N日前/N日後」offset は残す・`setRephotoBaseDate(root,after)` 完了コールバック追加）②**名前はチップ撤去→題名の横の✎だけ**（アクション行 [←一覧][⚙設定][🎞️撮り重ねる] の3つに）。preview green（⚙3行=基準写真/🎯基準日/位置トグル・⚙から基準日設定→offset「10日後/基準日/7日前」・✎リネーム・console0）。｜🎞️**v166：重ね撮りを「フォルダー」化**＝ユーザー再定義「フォルダーのように使い、食事/コーデ/メイク/友達を続けて撮るかも（想定外の使い方）」→①**位置の基準継承をフォルダーごとにON/OFF**（`rpInheritLoc`・既定ON・⚙設定・場所が変わるフォルダーはOFF＝v165+の継承が定点以外で誤るのを解消・現在地GPSは常に優先）②**基準写真を変更可能**（`rpBasePhotoId`・既定=最初の1枚・⚙設定の写真ピッカー・ゴースト兼位置継承元・子はrootにひも付くまま）③**一覧「＋新しい重ね撮り」で新規作成**（拡大写真を介さず・ゴーストなしカメラ=`newseries`・最初の1枚が`rpFolder` root・カメラ実比率で保存）。`rephotoAllSeries`をrpFolder対応（明示フォルダーは1枚でも一覧）。全てスキーマレス=DB移行なし。preview E2E green（新規→root rpFolder＋現在地/既定ONで基準位置10,10継承/OFFで継承せずnull/基準=Cゴースト＋子はroot Rにひも付く/設定モーダル339px収まり/4ボタン1行56px・console0）＝**実機の手触りが次**。｜🎞️**v165+：重ね撮りで撮った写真に位置情報**＝ユーザー質問「重ね撮りで撮ると位置は保存されない？」→コード確認で保存されていなかった（canvas JPEG は EXIF 無し＆importOne に lat/lng 未渡し）→**定点=同じ場所なので位置があれば地図/いまの場所/囲むに乗る**→実装「**現在地GPS優先→基準写真の位置を継承→両方無ければnull**」（`currentGeoOnce()` を openRephotoCamera で先取り→シャッターで使用・importOne override に lat/lng）。位置はアプリ内レコードのみ=端末内・書き出すJPEGに焼き込まない（Data Not Collected 不変）・?shot は GPS 呼ばず基準継承のみ。preview 3経路 green（現在地35.111取得/GPS拒否→基準34.7継承/両方null）＝**実機の位置確認が次**。｜🎞️**v165：重ね撮り実機FB第4周（アイコン・撮り重ねる・カメラ全画面・アイコン長押し動線）**＝実機FB4点→①アイコン **📐→🎞️**（UI 全表示のみ約14箇所・コード識別子/歴史コメント不変・ユーザー選択「フィルム=時間のコマが重なる」）②**「また撮る」→「撮り重ねる」**（入口=この角度で撮り重ねる／英 Layer a shot (at this angle)）③**カメラ全画面化**＝ヘッダ/黒帯コントロール廃止・ステージ inset:0・✕(右上)+⊟⊞⏱🔄(左上42pxアイコン円ボタン)+透明度スライダー/シャッター(下部グラデ浮きUI)を映像にオーバーレイ・上下分割中はスライダー自動非表示(visibility=場所を保つ)・⏱円内 3s/10s・**crop/fit/updateGuide/updateGrid の数式は無改修**（v162-164 の実機検証済み資産に触れない）④**iOS：ホーム画面アイコン長押し（Quick Action）→🎞️重ね撮り一覧へ直行**＝自前 micro-plugin `app-shortcuts` 新設（getPending drain＋shortcut イベント＝BgLoc 同型・SPM名一致 AppShortcuts）+AppDelegate performActionFor+Info.plist static shortcut（type=rephoto・SF Symbol square.stack.3d.up・題名 InfoPlist.strings ja「重ね撮り」/en「Layered Shots」）・コールド起動は **restLoadPromise（全件追い読み完了）を待って openRephotoList**＝シリーズ欠け防止・visibilitychange でも drain（保険）・web/Android は AppSC=null で無効＝不変・**native は 1.5 の次ビルド（38〜）から＝1.5(37) には未搭載**。preview E2E 全green（stage=[0,0,375,812] 全画面・ゴースト時 3:4 最大内接 375×500・分割=フレーム812px+スライダー hidden+グリッドが各写真の contain box にフィット・タイマー 3s→10s→⏱・3秒カウントダウン「3」→撮影→rephotoOf→シリーズ「🎞️重ね撮り」2枚→一覧1行・dbClear 掃除・リロード後 console 0）＝**実機（web Safari＋TestFlight 次ビルド）が次**。｜📐**v164：重ね撮りカメラの実機FB4点**＝実機（iPhone Safari・バックパック定点）で v163 使用→①**⏱セルフタイマー**（オフ/3秒/10秒＋中央カウントダウン・`runRephotoCountdown`→`doRephotoCapture`・カウント中シャッター無効）②**グリッド線を写真の実表示領域(contain box)にフィット**＝黒帯にはみ出さない（`containBox`）③**基準側にもグリッド**＝2要素化（上=カメラ/下=基準の両方に同じ3×3・各セルの写真boxに別々フィット）④ボタン増でコントロール**2段**（ツール中央1段＋シャッター別段）。preview E2E green（videoWidth 固定で検証＝gcam222/gref197<frame375 はみ出さず・上下両方・カウントダウン「3」→撮影→シリーズ・console0／偽カメラは videoWidth 未報告でガード動作＝実機は報告）＝**実機再確認が次**。｜📐**v163：重ね撮りを一級化＋⊞グリッド線＋🍜食べ物だけに（実機FB3点）**＝①カメラに**3×3グリッド線トグル**（既定OFF・分割時は上半分だけ）②**重ね撮り**（旧称 定点を改名）を思い出の場所並みに＝**⚙メニュー「📐 重ね撮り」一覧**（カバー/タイトル/N枚/年スパン）＋シリーズ画面に **✏タイトル・🎯基準日（N日前/N日後）・日ごとメモ・📖ログ/🖼写真切替・← 一覧・📐また撮る**（ログは v127/129 の造形流用＝時間の空きを高さで/オフセット。**メタは root 写真レコードの rpTitle/rpBaseDate/rpNotes＝スキーマレス・DB移行なし**）③意味レンズ UI を**「🍜 食べ物を振り返る」だけに**（チップ/言葉/写真検索を撤去・機構は残置＝ユーザー明示「食べ物だけにしましょう」）。preview E2E 全green（メニュー2ボタン・バー無し・グリッドon/off・撮影→ログ→メモ保存→基準日→オフセット37日後検算一致→✏からだ→一覧1行→復帰・console0）＝**実機 Safari の手触りが次**。｜📐**v162：定点カメラの分割を上下＆全体表示に**＝実機FB「左右だと基準もこれから撮る写真も一部しか見えない・小さくなっても全体を見たい」→ 分割を**上下**（上=📷いま/下=🖼基準・タグ付き）・分割中は **contain＝両方まるごと**（黒帯可）・枠はステージいっぱい・**破線ガイド=実際に保存される範囲**（`updateGuide`）。保存は基準と同額縁のまま＝シリーズ一貫・**ぴったり合わせはゴーストが精密モード**の役割分担。preview E2E green（上下2行/contain/枠=ステージ寸/タグ/ガイド281×281/往復復帰/分割のまま撮影→シリーズ・console0）。｜📐**v161：定点＝同じ角度を時間を空けて重ね撮り**＝拡大表示「📐この角度でまた撮る」→**アプリ内カメラ（getUserMedia）に基準写真を半透明で重ねるゴースト方式**（透明度スライダー45%・**左右分割切替**（左=カメラ/右=基準）・前面=プレビューと基準を両ミラー/保存は非ミラー・**基準と同アスペクトで中央crop**＝シリーズ同額縁・長辺2048）→撮った1枚は `importOne({datetime:now, dateSource:'exif'})` で**通常写真として取り込み＋`p.rephotoOf`=root でひも付け**（スキーマレス＝DB移行なし・新ストアなし）→シリーズ＝既存再利用（`buildTimelinePhotos` 日ごと＋⇆比べる hideTripMap＋「📐また撮る」）。入口＝拡大表示のみ（⚙に足さない・2枚以上で「📐定点（N枚）」）。**⚠️native はカメラ権限宣言が要る＝iOS 1.5 で Info.plist `NSCameraUsageDescription`／Android Manifest `CAMERA`（今の1.3/1.4では動かない）**。web=GitHub Pages（HTTPS）は今日から動く。preview E2E 全green（getUserMedia を captureStream でスタブ→拡大📐→カメラ（video ready/ghost0.45/枠fit/分割トグル）→シャッター→**本物のimportOne経路**でrecord（rephotoOf・撮影日・thumb）→シリーズ2枚日ごと見出し→比べる→「📐定点（2枚）」・console0・テストはdbClear掃除）＝**実機 Safari の手触りが次**。次段候補＝重ねてスライダー再生（体形変化に効くはず）／最初の基準をアプリ内カメラで撮る導線。｜🔍**v160：意味レンズ＝食べ物レンズを汎用化（カテゴリ／フリーテキスト／写真で探す）**＝同じ CLIP 埋め込みで3つの入口：①**カテゴリ**（🍜食べ物/🏞風景/👤人/🐾ペット）＝陽性 vs 他カテゴリ＋その他を softmax（実質マルチクラス・食べ物 v157 の "food vs 他全部" を各カテゴリに一般化＝食べ物判定は実質不変）②**フリーテキスト**＝入力語＋"a photo of 語" vs 汎用アンカー softmax ③**写真で探す**＝アップロード画像を CLIP 画像エンコーダで512次元化→保存済み embedding とコサイン上位N（reverse image search・`LENS_IMAGE_TOPK`60/`MIN_COS`0.6）。結果は全部これまでのタイムライン（月/日・⇆比べる・大きく）で表示（写真レンズは類似上位N→時系列に並べ直す＝「同じ仕組み」維持）。⚙メニュー「🍜食べ物を時系列で」→**「🔍 意味で探す」**（`btnFood`→`btnLens`）＋オーバーレイ上部の**レンズバー**（チップ/言葉/📷写真）で開いたまま切替（既定=食べ物）。共通化＝`foodProbOf`→`lensProbOf`／`openFoodLens`→`openLens`＋`runCategoryLens/runTextLens/runImageLens/runLensScoring`。web のみ native 契約不変。preview 全green（合成E2E：カテゴリ分類 food→[food1,food2]/風景→[land1,land2]・チップ切替追従・フリーテキスト「a beach」→該当のみ・写真レンズ経路動作・レンズバーDOM確認・console0。CLIPテキスト512次元は v157 実証済）＝**実機の手触り＆各カテゴリ/テキスト/画像しきい値の調整は残**。｜🍜**v159：食べ物レンズに「⇆比べる」と「大きく/小さく」**＝思い出の場所と同じ `.atl-photobar`（枚数＋⇆比べる＋大きく/小さく）を再利用。**大きく**=`buildTimelinePhotos` に `big` 引数（`.atl-grid.big`=2列・地図タイムラインは無改修）。**比べる**=`openFoodCompare`＝食べ物レンズ(z1900)は開いたまま比較(z4000)を上に重ね、閉じると食べ物レンズに戻る（`openCompareView` に後方互換オプション `hideTripMap`/`title` 追加＝足跡地図を隠す・タイトル差替／既存の思い出の場所は4引数のまま不変）。**背景＝v158 実機「いいかんじ」＋要望**。web のみ native 契約不変。preview green（大きく→2列・⇆比べる→比較z4000/食べ物レンズ開いたまま/タイトル「🍜食べ物を見比べる」/足跡地図ボタン無し/閉じて復帰・console0）。｜🍜**v158：食べ物レンズに「月ごと/日ごと」切替**＝食べ物タイムラインの見出し粒度を 📅月ごと(既定)／📆日ごと(日付＋曜日) で切替（`buildTimelinePhotos` に第3引数 `groupBy` 追加・`'day'`で日見出し・既定`'month'`は従来どおり＝**地図タイムラインは無改修**・`_wdayJa`/`_wdayEn`・切替バーは `.atl-modebar` 再利用・トグルは再分類せず即時再描画）。**背景＝v157 実機「いい感じで出ますね」（食べ物判定が実写真で効いている signal YES）＋日ごと要望**。web のみ native 契約不変。preview green（合成5枚：月ごと=3見出し／日ごと=4見出し 7/8水・7/5日・6/20土・12/25木＝曜日正確・同日集約・切替即時・console0）。｜🍜**v157：食べ物レンズ＝意味(CLIP)を「テキスト検索」に拡張**＝これまで意味モードは「写真同士の類似」だけ→**CLIPテキスト側エンコーダ（`AutoTokenizer`＋`CLIPTextModelWithProjection`）を追加**し「食べ物」を512次元に起こして保存済み画像embeddingとコサイン比較＝**ゼロショット分類**（食べ物語群8＋対抗カテゴリ8を softmax・logit_scale100・しきい値0.5）→**食べ物写真だけを日別・時系列(新→古)で全画面**（⚙メニュー「🍜 食べ物を時系列で」・表示は `buildTimelinePhotos` 再利用＝月見出し＋タップ全画面スワイプ）。「最近なにたべたか」は上／下へ辿ると久しぶりの食事＝コア(久しぶり/よみがえる)と地続き。オーバーレイ z1900（拡大z2000の下）・端末内完結（テキストも写真も送信せず・DLはAIモデルのみ・q8遅延ロード）・**web のみ native 契約不変**。preview 全green＝**CLIPテキストが preview ブラウザで完動**（CDN import＋モデルDL成功・`text_embeds` dim=**512**＝画像と同一空間・norms1.0・cos(食×食)0.954＞cos(食×街)0.876・foodProbOf food→1.0/other→0.0・need-ai導線・console0）。**判定プロンプト/しきい値は実機のユーザー写真で調整が前提**（preview に実写真なし）＝**実機の手触り残**。｜🗺️**v156：地図バー2段目を1行に**＝「🫧 囲んで思い出を見る」を「🫧 囲んで見る」に短縮（スマホ縦で `.mc-row`(wrap) が3段に折り返すのを解消・title にフル説明維持・preview 375/320px で両段1行・barW291<307）。｜🗺️**v155：地図バー再構成＋期間クイックボタン**＝①「囲んで思い出を見る」を⋯メニューから**地図バーへ昇格**（`mcEncircle`＝1タップで囲むモード）②バー並び＝1段目 [◀ 🗺過去の今日 ▶ 📅全期間]／2段目 [🎲 📍 🫧囲んで思い出を見る ⋯表示]（`mcToday` ラベル化・全期間を1段目へ・現在地は📍アイコンのみ・**◀▶日ナビ維持**＝ユーザー選択）③期間絞り込みに**クイックボタン（1週間/1ヶ月/3ヶ月/1年/3年＝今日から遡って直近N）**④`.hdr-primary-row` を均等幅nowrap＝**拡大時も2段目1行を保つ**（3段折り返し解消）。**web のみ・native 契約不変**。preview green（バー座標2段・mcEncircle→囲む起動・mmCompare は⋯から消滅・クイック「1年」=366日・ヘッダ375/320px 1行クリップなし・console 0）＝**実機の手触り残**。｜📍**v154：「いまの場所」を半径選択式に＋ヘッダ2段化**＝①半径を自動採用せず、現在地に円を出し**スライダー(100m〜10km・対数)で店⇔地域の粒度をユーザーが選ぶ**＋**ライブで「N枚」**＋2枚以上で「この範囲で見る」→`openEncircleTimeline`。初期半径=2枚以上入る最小（開いた瞬間に写真＋調整可）。②写真0/1枚は「見る」無効＋「右に広げて」導線＝**少数時の混乱を解消**（円と枚数が見える）。③ヘッダ2段化＝1段目=タイトル+⚙／2段目=🗺過去の今日・📍いまの場所・🗺過去の明日（`.hdr-primary-row`・menu-dropdown は絶対配置で不変）。`enterHereRadiusMode`/`endHereRadiusMode`＋`.here-panel` CSS・`mapState.hereMode`。**web のみ・native 契約不変**。preview green（ヘッダ座標2段・スライダー連動 v3=1枚無効/v25=12枚/v75=21枚・確定で「囲んだ場所21枚」・空状態=0枚+導線・スクショ確認・console 0）＝**実機の手触り残**。｜📍**v153：トップに「いまの場所」ボタン**＝アプリを開いて1タップで「現在地の地図＋青ドット → 自動円（段階半径 400m→800m→1.5km→3km→6km で最初に2枚以上）→ 過去の時系列タイムライン（`openEncircleTimeline`）」まで一気に。**囲む操作を省略**＝手順を最小化（ユーザー実機要望）。`#btnHere`（🗺過去の今日の隣・`LOCATION_AVAILABLE` 時のみ・日英i18n）／`openHereTimeline()` は geolocation 前面1点＝v151 `flyToCurrentLocation` と同 API・`.here-marker` 共有。写真0枚は「この辺りには、まだ写真がありません」。**web のみ・native 契約不変**。preview 実コードパス green（合成21枚：center=35.680,139.760/zoom15/青ドット＋「囲んだ場所16枚」／空状態＝地図移動＋メッセージ／console 0・実タップは `.click()` で発火確認＝合成クリックは当たらず）＝**実機の手触り残**。｜🐛**v152：英語UIの間隔ラベル i18n**（"2.0年 apart"→"2.0 years apart"・`intervalLabel` を tr() 化・複数形・唯一の呼び出し=思い出ログのギャップ・preview en/ja 確認済）＝**1.3(34) には未反映＝次ビルドに乗る**。｜🎉**1.3(34) リリース済み（2026-07-07・ex-EU 148・café ヒーロー版）**＝7/6 審査提出→承認→手動リリース。店頭コピーを「囲む→時系列（久しぶりがよみがえる）」前面に刷新（プロモ用テキスト/説明文=先頭にヒーロー/リリースノート・日英）＋café「囲んだ場所/Encircled area」を日英とも**1枚目(ヒーロー)**に差し替え（1284×2778）＋build34。｜🗺️**v151：地図に「📍現在地」ボタン ＋ 🫧囲む→ログ(タイムライン)で開く**＝①現在地＝前面 `navigator.geolocation` で地図を寄せて青ドット（`.here-marker`・パルス付き）。コントロールバー行2（🎲の隣・「📍現在地」）に配置し **`LOCATION_AVAILABLE` 時のみ**（位置オフ/`?shot` では出さない＝許可を求めない）。②「囲んで思い出を見る」を deck(1枚) ではなく**思い出の場所ログと同じタイムライン**で開く＝一時オブジェクト（`_transient`・`visitsFromPhotos`）＋`openAnnivTimeline` 再利用・未登録は編集系（メモ/基準日/代表写真/＋続き）を隠し「📍思い出の場所に登録」で `registerAnniversary`→本物ログ（編集可）に昇格・deck 動線 `openAnnivDeck` は温存（未使用化）。**web のみ＝native 契約不変**・preview 実コードパス検証 green（現在地スタブ移動＋青ドット描画／囲む→🫧囲んだ場所5枚→写真グリッド→登録→📖本物ログ昇格）・console 0・スクショ確認・push 済・**実機の手触り残**。｜🛰️**v150：Android にも位置ロガー（B＝背景・iOS 同等）**＝`background-location` に Android(Kotlin) 実装を新規追加＝**通知つき前面サービス (FGS type=location)＋fused provider・`ACCESS_BACKGROUND_LOCATION` なし**（Play 最厳格審査ゲートを構造的に回避しつつ「アプリを閉じても記録」成立）。iOS と同契約5メソッド＝JS 無改修（`NATIVE_LOCATION` を ios||android に＋文言2箇所のみ・web/iOS 不変）。**🎉 S21 実機 E2E 全クリア**＝権限2段（位置→通知）→「今日2点を記録」（service→バッファ→drain→track→UI）→ dumpsys `isForeground=true`/`allowWhileInUsePermissionInFgs=true` → **recents スワイプ kill 後もサービス生存（One UI の swipe-away を生き延びた＝最大の未知数クリア）** → 再起動でモード復元・点数維持。テスト後は借り物端末を清掃（オフ・記録0・📍消灯）。残＝実移動の軌跡/電池（実使用待ち）。｜✅**v149：地図タイムラインの絞り込みバグ修正済み**＝🖼写真タブ/⇆比べるが `timelineFilteredPhotos(all)` で 📋一覧と同じ範囲（`mapState.pickDays`/`range`）に揃う。preview で述語5ケース pass・一覧 `updateTimelineUI` は無変更＝回帰なし・inline 0エラー・push 済。**🎉実機 YES**（ユーザー・TestFlight **1.3 build 33**＝「修正されてました」＝絞り込んだ範囲だけになる）。📱**iOS=1.2 公開済み**（位置ロガー版・ロードマップ②）／**1.3(33)** に v149＋v148 の思い出の場所改修が乗る＝**1.3 の審査提出が次**。｜**v148：タイムラインの見せ方 統一**＝①思い出ログの「⇆比べる」を写真タブのバー(大きく/小さくの隣)へ移動(openAnnivCompare 共通化)②地図タイムラインに **📋一覧/🖼写真 切替＋⇆比べる** を追加＝一覧(位置を直す/🗑/フィルタ)は据え置き・写真は全可視写真の時系列グリッド(月見出し・タップで全画面)・写真モードは updateTimelineUI を no-op で一覧機構と非干渉＝**現状を壊さず付け足す**。preview で 思い出ログ(top に compare 無し/写真バーに比べる)・地図(切替/グリッド/一覧戻すと位置直す/🗑 健在) 検証green・inline 0エラー。実機の手触り残。｜**v147：思い出ログに表示モード切替「📖ログ / 🖼写真」**＝写真モードは全写真を新しい→古いの時系列グリッドでいっぱい(月見出し・小4列/大2列トグル・タップで全画面スワイプ・戻ってもモード維持)。ログ(日ごと+メモ)据え置き。既存 photosInAnniv/thumbUrl/showFullImage 再利用・web完結。preview で切替/グリッド/月見出し/枚数/小大/往復 検証green・inline 0エラー。実機の手触り残。｜**v146：地図⋯「囲んで比べる」→「囲んで思い出を見る」に表現変更**（英語 Encircle to see memories・ラベル1箇所 mmCompare のみ・機能不変）。｜**v145：機能名を「記念日」→「思い出の場所」に改名**＝UI に見える文言を日英とも全置換（日本語は一括／英語=Memory place）。**DBストア名 `'anniversaries'`・コード識別子（`registerAnniversary`/`.anniv-*`）・一部コメントは不変＝既存データ/コード互換**。ヘッダ(⚙内)・一覧・空状態・登録/移転モーダル・トースト・確認・静的i18n を網羅。grep で `記念日` 0件・preview で表示確認・inline 0エラー。｜**v144：思い出の場所マルチ地点の同日バグ修正**＝別の場所を足した写真が「同じ日」だとタイムラインに載らなかった（`mergeVisits` が既存 dayKey を丸ごとスキップ＝比べる[全円ライブ]には出るのに visits スナップに入らない）→**同日は既存訪問に写真を統合**（重複除去・上限8）／別日は新規訪問。preview で検証（p1,p2+p3,p2→p1,p2,p3・memo保持・上限8）。ユーザー実機報告の修正。｜**v143：記念日「この場所の記憶」をマルチ地点対応**＝ユーザー機能質問（①エリアを先に登録すると中の店を別記念日にできない＝重なると自動吸収②お店が移転したら前後両方で思い出を積みたい）に対応。ユーザー選択A＝**重なったら自動吸収せず必ず選ばせる**（`promptAnnivRegister`＝「◯◯に足す／新規／移転で既存に足す」）。記念日を **`center/radius`単一→`circles[]`複数円の器**に拡張＝写真は全円の和集合（`photosInAnniv`）＝移転しても場所の同一性が1つの記念日で続く。移転の入口2つ（登録モーダルの select／詳細の「📍別の場所」→エンサークル直接attach）＋詳細に「📍N か所」表示。旧単一円は `circlesOf` で後方互換（IndexedDB スキーマ変更なし）・読み手5箇所を統一。preview page context で純ロジック＋モーダル検証 green（circlesOf/annivContains/photosInAnniv/mergeVisits・初回は名前入力だけ）・inline 0エラー。**実機でエンサークル→attach の手触り確認が残**。web 完結＝native 無改修・審査中1.2 無関係。｜**v142：本丸＝写真全件アクセスの Android 実装（photo-library に MediaStore/Kotlin）＝エミュE2E YES**＝iOS の自前 Swift plugin と**同契約**で Android を Kotlin/MediaStore 新規実装（requestAccess=13+/14部分許可=limited・enumerate=全件即時列挙で DATE_TAKEN→保存日フォールバック dateSource・thumbnail=loadThumbnail＋EXIF の GPS/撮影日時を同乗）。web 層は GPS/日時マージ3行＋権限文言分岐のみで無改修。権限はプラグイン側 Manifest に隔離＝**app 本体はクリーンのまま**フル版を両立。`cap sync android` で自動配線（settings.gradle include・plugins.json classpath）。**エミュE2E（adb 自動化）＝テストJPEG7枚→Android14 実権限ダイアログ→📚全体取り込み7枚成功＝カード「2023/07/03 Captured」で保存日→EXIF撮影日の格上げを実動確認**。GPS は emulator が redact（コードは正・requireOriginal付与/ACCESS_MEDIA_LOCATION許可済＝実機で要確認）。ローカルビルド確立（Android Studio JBR21 で gradlew assembleDebug 通過）。**実機で GPS/大量スループット確認が次**。｜**v141：Android 着手＝クリーン版(位置なし/広告なし)を scaffold**＝`npx cap add android` で android/ 生成（applicationId=iOS と同じ `io.github.yutsutke.madeleine`・minSdk24/compile・target36・app名 en 既定"A Past Day"＋`values-ja`「あの日 — 写真と足跡」）。`NATIVE_LOCATION` を固定 true→`getPlatform()==='ios'`（iOS のみ位置あり＝1.2 不変／Android・web は false）。background-location/photo-library は `capacitor.ios` のみ宣言＝Android 非登録で Gradle も権限も汚さない→AndroidManifest の権限は **INTERNET のみ**＝完全クリーンが scaffold 直後に成立。Codemagic に **android-debug workflow**（mac_mini_m2・java21・sync:web→cap sync android→gradlew assembleDebug→APK artifact・デバッグ署名でサイドロード可）追加。native 本体はコミット・生成物のみ ignore（dry-run 54ファイル・危険パターン0）・codemagic.yaml parser green。**🎉 Codemagic build #3 green（app-debug.apk 4.58MB）→ Android Studio エミュレータ（Pixel7/API36.1）で起動 YES＝本スパイク達成**（空状態UI完全描画・クラッシュなし・🛰️位置なし確認・英語ロケールで i18n も乗る・CDN/vendor OK）。ビルドの壁3つ＝instance(linux→mac)/SDK36自動DL/JDK17→21(capacitor-android は Java21ソース)。写真の Android 実装＝MediaStore(Kotlin) は本丸として次。web 側は NATIVE_LOCATION 分岐のみ＝挙動不変。｜**v140：足跡から写真の位置を自動確定＋位置の由来バッジ＋位置ロガーの日別/全削除**＝①GPSなし写真を地図の「表示中の窓」で足跡から自動確定（既定ON・`autoConfirmTrackPass`／geoFromTrack+geoAuto で永続・`_autoTrackTried` で1回だけ試行・新軌跡が来たら再試行可・🛰️パネルで OFF にすると従来の手動確定に戻る）②位置の由来を1本化（`locSource`＝none/exif/track/edited/manual）＝バッジ「足跡から/位置を修正/手動」＋マーカー隅ドット（teal足跡/amber修正）・exif/none は非表示（clutter回避）③`saveLocationEdit` で元EXIF GPSを動かしたら geoEdited＝「位置を修正」表示④🛰️ロガーパネルに「🗑すべての記録を消す」(confirm必須)＋「📅日ごとに消す」(日別集計リスト・各行 `trackClearRange`)。自動でも由来バッジ＋「📍位置を直す」で修正可＝[[editing-triggers-reminiscence]] を殺さない。preview green（12:30写真→軌跡中点に補間・由来分岐・日別リスト・console 0）・**実機確認待ち**（native 1.2 に載る）。｜**v139：地図の線を「写真＋ロガーを時刻順に1本」へ**＝ロガー点を擬似点{lat,lng,datetime}にして写真の点列とマージ→時刻順1本（飛び飛びの区間を軌跡が埋める・ユーザー要望）。青緑の点線は廃止・記録点ドットは残す・軌跡トグルOFF=写真のみ。2段描画（写真のみ即→ロガー点が積めたら合流して引き直し）・窓替わりで trackMerge=null（race防止）。preview green（casing頂点5/セグメント4/ドット3/点線0・OFFで写真のみ・console 0）・**実機確認待ち**。｜**v138：位置ロガー復活（native 1.2）**＝NATIVE_LOCATION=true・background-location を package.json に復活(npm install済)・Info.plist に NSLocation×2＋UIBackgroundModes=location・**用途文言を InfoPlist.strings ja/en にロケール別実装（写真2キーの英訳も追加＝1.1の穴）**・MARKETING_VERSION 1.1→1.2（審査中の1.1と分離）。web 無影響確認済。**push後の Codemagic 1.2 ビルドを TestFlight で実機確認**（位置ダイアログ/🛰️3モード/背景記録/英語文言）→ 1.1公開後に 1.2 審査提出。｜**v137：トップの並び替え**＝写真をヘッダ(過去の今日/明日)直下に・フィルタチップ(全期間等)と枚数は最下部へ(ユーザー実機FB・append順の変更のみ・空状態でもチップは残す)。preview green(DOM順＋mobile実座標・console 0)・実機確認待ち。｜**v136：拡大表示にピンチズーム**＝2本指で拡大縮小(1〜5倍・つまんだ点を画面上に保持)・拡大中は1本指パン＋横スワイプ(snap)停止(等倍復帰で自動再開)・拡大中のタップは等倍へ戻す(もう一度で閉じる)。**🎉実機 YES「いいですね。問題なさそう」**。｜**v134：初期画面（ネイティブ空状態）の主導線ボタン「写真ライブラリ全体から始める」に（おすすめ）/EN (recommended) を付与**＝取り込みメニュー(L5197)の「ライブラリ全体（おすすめ）」と用語統一・実機FB・JP/EN 両方・preview green（起動ログ BUILD=3.87・console エラー0）。｜**v133：英語化の総仕上げ（実機で発見の残り＋長文）**＝写真詳細🗺ボタン(重複2箇所)・スワイプヒント・取り込みドロップダウン・詳細設定モーダル・情報モーダル本文6段落・自動取り込みプロンプト・位置ロガーパネル全体・日時ソースタグ(撮影日/保存日/投稿日)を tr() 化＝**dev(🧪#dev 開発者専用)以外ほぼ完全英語化**。教訓＝「地図でこの場所を見る」が3箇所に重複実装→静的網羅1回では拾えず**実機確認が最終網羅**（実機FB 3連続で新未訳を発掘）。preview green。｜**v132：二次画面の英語化＋i18n関数 t()→tr() 改名**＝残りの二次画面を英語化＝記念日(一覧/ログ/基準日/登録/予定/代表写真)・封印・ⓘヘルプ・AIモーダル・削除写真一覧・確認ダイアログ・各トースト・「N枚から」件数。⚠️**i18n関数 t() が既存の t ローカル変数(20箇所)と衝突→ `tr()` に全面改名**（sed で `\bt(`→`tr(` 一括・v130/131 で t() を使った地図/記念日一覧/情報モーダルの潜在バグも同時に解消）。長文の情報モーダル本文/位置ロガー説明/devは次段。preview green（?lang=en 空状態英語・?lang=ja 不変・console 0）。｜**v131：世界版の続き（海外地図フォールバック＋主要導線の残り英語化）**＝①日本語モードの地図に世界の下地(CARTO)を常時敷き、日本人が海外旅行の写真を見ても地理院の外(海外)が真っ白にならずシームレスに（`worldUnderlay`・`LANG==='ja'` 時のみ・切替時も bringToBack で最背面維持）＝「言語で地図を切替」の穴（言語≠見る場所）を塞いだ。②Explore で未訳UI約150箇所を網羅→**触る主要画面**を t() 化＝フィルタチップ/連想ウォーク(Wander)/写真詳細/地図(ツールバー/popup/フィルタ/⋯/タイムライン/囲む比較)/ヘッダtitle（機能名統一 Wander/Footprints/Anniversary/Seal）。二次画面(記念日詳細/封印/情報モーダル本文/確認/dev)は次段。preview green（?lang=en 全面英語・?lang=ja 不変・console 0）。｜**v130：英語 i18n の下ごしらえ＋世界地図**＝公開後ロードマップ①（世界版）の基盤を web に先行投入（native 1.0(21) は審査待ちのまま・非接触）。端末言語(navigator.language)で日英出し分け＝`t(ja,en)`（ja は第1引数そのまま＝**日本語は1px も不変**・非日本語は第2引数・未指定は ja フォールバック）・言語自動判定・主要導線（ヘッダ/空状態/取り込み進捗/モーダル見出し/枚数）を英語化。地図は非日本語ロケールで地理院→CARTO Voyager/Positron に差し替え（`BASE_LAYERS` を LANG 出し分け・key 共通で localStorage 互換・描画/保存ロジック無改修）。preview green（?lang=ja 回帰なし・?lang=en 全面英語＋タブ名・console エラー0）。`?lang=en` で検証可。｜**v129：記念日の表現＋基準日**＝①「＋未来の予定」→「＋あの日の続き」（作業感を外し記憶の続きへ・未来バッジ 予定→これから・データ不変）②記念日に任意の基準日を設定でき、ログの各日の横に「N日前／N日後／基準日」を表示＝あの日からの隔たりを日数で体感（`a.baseDate`・IndexedDB スキーマ変更なし・既定候補=最古訪問日）。preview green（862日後 検算一致・前後両方向・teal）。**🔀方針＝承認済み 1.0(16) は release せず、UI仕上げ版を再提出しデビュー版にする → ✅2026-07-01 実行＝Codemagic ビルド1.0(21)（v111-129 UI入り）を ASC で再提出→「審査待ち」（デビュー審査・2回目・追加質問ゼロ）。承認後 ASC で手動公開＝デビュー**。｜**v128：UI微調整3点**＝①地図「📖記念日のログ」を常時表示化＝記念日由来の地図→そのログ／**通常の地図→記念日一覧**（無反応を解消）②地図の種類チップ ダーク/地理院標準/地理院淡色 を1行に③ヘッダの写真枚数をタイトル行へ移動＝2段に圧縮（⚙が2段目に収まる）。preview green（②③実測・①ロジック）。｜v127：**記念日ログの作り込み**＝①時系列を逆に（現在→過去へ遡る・新しいが上）②訪問の間隔が空くほどログ上の「間」を広げる（log スケール＋「N年のあいだ」＝時間の長さを体で感じる）③一覧タップ→ログへ直行（記念日の家＝物語）④ログの代表写真タップ→拡大＋記念日内を日にち横断スワイプ⑤相互動線＝ログ↔一覧/左右で比べる/足跡の地図（比較・地図からもログへ・`anniv` を compareState/mapState に伝播）。preview green（全方向）。｜v126：①地図 popup を暗く（日時が読めるように）②記念日一覧おしゃれ③記念日「📖ログ」＝日別タイムライン(写真/コメント/未来の予定)。｜v125：微調整6点＝①足跡の地図から記念日登録②🎲1日③連想ウォーク統一④位置なし説明⑤位置あり拡大スワイプ⑥今日0年分。｜v124：微調整4点＝①比較スワイプ即応(画像 pointer-events:none)②比較に「🗺足跡の地図」③ヘッダ「🗺過去の明日」(todayOffset:1)④囲む後も記念日と同じ deck 動線に統一(openAnnivDeck・違いは canRegister の登録ボタンだけ)。｜v123：動線タップの実機「無反応」修正＝透明な太い当たり判定線(weight24/opacity0)・見た目線 interactive:false。教訓＝preview の合成クリックは実機タップ命中を保証しない。｜v122：**記念日の詳細を作り替え**＝記念日タップ→写真1枚大表示＋スワイプ(showFullImage deck)→「⇆左右で比べる」／「🗺足跡の地図」(全訪問日を動線表示・1本タップでその日だけ明るく＋タイムライン連動)。一覧に🗑。｜v121：記念日(過去半分)＝円を名前付き保存し訪問が積もる器・IndexedDB v3。｜v120：地図「囲んで比べる」(実機「いいかんじ」YES)。｜v119：起動時の自動差分取り込み(native)。v111-118 UI改善✅。｜🎉🎉**App Store 承認（2026-07-01）！ビルド1.0(16)が審査通過**＝「Welcome to the App Store / eligible for distribution」。**ただし 16 は公開せず**（旧UI）＝手動リリース設定のまま「リリースをキャンセル」→ **v111-129 UI入りの ビルド1.0(21) を再提出＝「審査待ち」（2回目）**→ **次の一手＝審査結果待ち→承認後 ASC で手動公開＝デビュー**。英語名 A Past Day／未来=カレンダー書き出し(EventKit)／位置ロガー復活／iPad／広告 は native 次版。）

> ### 📍 次セッションの再開ポイント（2026-07-13 セッション36 更新・まずここを読む）
>
> **⏱ TL;DR（次に開いたら、まずこの4点）**
> 1. **web**: GitHub Pages **`phase3.159`（v205-207）** push 済。**v207＝✏️手書き点の実機FB3点**（v206「いい感じ」＋要望）＝①ドラッグ難→点のモーダルに「✥ 移動する（次タップした所へ）」（時刻不変）②🕒時刻の手入力→その点が「時刻アンカー」tFixed（白リング）③realN判定を「写真/ロガー＋tFixed手書き点」に拡張＝**確定点(GPS/ロガー)の無い日でも手動アンカー2つで v188区間按分が効く**（ユーザー洞察「時間確定点があると線が引きやすい」＝②の帰結）。preview E2E 全green（按分midBetween・移動座標追従/時刻不変・やめる・seg起点の移動/時刻追従・console0）。**実機の手触りFB待ち**（時刻ピッカーの開けやすさ・白リングの分かりやすさ・移動→次タップのテンポ）。**v206＝✏️🎯手書きに「起点→終点」モード**（ユーザー提案「歩き回ったエリアの打ち方＝先に起点と終点→通過点をタップ順で」）＝打ち方トグル ✚近くに打つ(従来)/🎯起点→終点。🎯＝既にある点2つをスナップ選択（逆順自動入替・琥珀リング）→通過点をタップ順で下書き（琥珀点線・☕可）→🎯確定で道のり按分の絶対時刻を焼き込み＝**確定で独立**（スキーマ不変・基準点を後から消しても連鎖しない）。下書き中の基準点🗑ブロック・↩は下書き→保存済みの順に1手ずつ・✓終了/日切替/✚戻しは自動確定（↩で可逆）。preview E2E 全 green（console 0）。**実機の手触りFB待ち**（スナップ48pxの当てやすさ・確定を挟むテンポ・下書きの見え方）。⚠️dayKey は月0-index（7/1=`'2026-6-1'`）＝テスト時の罠。v205＝**✕で思い出の場所の一覧へ戻る**（実機FB「一覧へのチップは不要・✕で一覧に戻れるように」）＝思い出の場所の詳細（`atl-overlay`）の「← 一覧」チップを撤去し、右上の ✕ を「閉じて一覧へ」に統一（v177 の重ね撮りシリーズと同じドリルダウン）。transient（🫧囲んだだけの未登録＝一覧に無い）は従来どおり✕で閉じるだけ。preview 両ケース green（通常＝✕でopenAnnivList発火・ボタンから list 消滅／transient＝✕で閉じるだけ・console0）。※直近の大物 v194-204（🤝もらった思い出＝パックの渡し合い一式）は下のセッション33ブロック参照。**実機の手触りFB待ち**（✕で一覧に戻る感覚）。
> 2. **iOS**: 🎉🎉**1.5 が承認→App Store 公開済み（2026-07-12）＝🎞️重ね撮り版デビュー（第2章）**（ストア説明も重ね撮りが主役・apps.apple.com/jp/app/id6784557053・バージョン1.5・更新確認済み）。**1.6(build 47=7/12・48=7/13朝) が TestFlight 配信済**（v189 で MARKETING_VERSION 1.5→1.6・手書き v187-193＋もらった思い出 v194-204 を実機で試せる）。⚠️v194 の📤共有シートは WKWebView 未対応（次の native 作業で @capacitor/share＋.json 受け取り口）。**▶次＝1.6 の実機FB／1.6 を審査に出すか判断**。
> 3. **Android→Google Play**: 🎉**Developer 登録 完了（2026-07-10・$25＋本人確認）＝Play Console「アプリを作成」ロック解除**。**📱Pixel 7a を入手・ログイン（7/12〜13・Gmail のログイン通知から推定＝要ユーザー確認）**＝実機も揃った公算。**▶残＝鍵を Codemagic 登録→`android-release` で AAB 初回ビルド→内部テスト**（掲載文ドラフトはセッション30会話内）。
> 4. **Claude の「待ち無し」タスクは無し**＝次は実機 FB か新要望から。挨拶時は [[session-start-gmail-check]]（Apple＋Play Console 通知）。
>
> ### 📍 （履歴）セッション33 の再開ポイント（2026-07-12 更新）
> 1. **web**: GitHub Pages **`phase3.156`（v194-204）** push 済＝🤝**もらった思い出＝パックの渡し合い一式**。v204＝🎞️**基準確定を📌アイコンのみ＋右上（✕の下）へ**（テキスト帯→アイコンで被り面積最小・全モードでタグ/⚙と非競合）。v203＝基準確定を上部中央へ（→v204で右上に再調整）・基準バー表示中は出さない。v202＝🎞️**基準写真の📌ピン止めを見つけやすく**（基準バー非表示でも「📌 この写真を基準にする」チップで確定・スワイプで基準以外を表示中に出る）。v201＝🐛**モーダル背後のスクロール抜け修正**（⚙メニュー/モーダル中は背景を touchmove 抑止・レイアウト不変）＋🎞️**基準写真スワイプの説明を⚙に**（2枚以上の時）。v200＝✨**遷移のちらつき解消**（拡大→地図で本体が一瞬見える・先に地図を作る順に）＋🤝**地図トグルを「写真・足あと」まとめてに**（🗺地図に表示中/非表示・受け手目線）。v199＝🐛**拡大→地図でフォルダーに戻る不具合を修正**。v198＝**受け取ったパックが「帰る場所」へ**（実機FB反映）：🎞️重ね撮りパック→重ね撮り一覧に👤フォルダーとして入る（シリーズ/比べる/ログがそのまま・見るだけ）／📍場所パック→思い出の場所一覧に👤で入る（**あなたの同じ場所の写真も同じログに積み重なる**）／日パック→従来どおり地図の重ね歩き。**プライバシー＝場所パックの軌跡は「円内だけ」クリップが既定**（行き帰りの道は渡らない）。削除は重ね撮り一覧/場所一覧の行からもパック丸ごと。｜（履歴）v195=贈り物ビルダー（⏱時間帯窓/🖼全部・選ぶ・入れない/💬写真ごと+✉️手紙）・v196=「選ぶ」見せ方3択（▦/⊡/☰コメント縦並び・記憶）・v197=フォルダーからも渡せる（source一般化）。パックv:2・v:1後方互換。**実機の手触りFB待ち**（フォルダーに「入ってる」感・場所ログで自分の写真と混ざる感覚・円内クリップの安心感・3モードの使い分け・手紙の受け取り）。｜（履歴）v194＝🤝**もらった思い出＝家族/友達と「日パック」を渡し合う**（シェア un-park 第一手・ユーザー明示要求 2026-07-12。設計の線引き＝**共有は「過去の日」であって「今の位置」ではない**・サーバーなし＝JSON手渡しで「Data Not Collected」不変）。📤地図⋯表示「この思い出を渡す」（名前+🖼写真トグル→共有シート）／📥⚙「🤝もらった思い出」（取り込み→**そのまま地図が開き同じ地域の自分の日≤6日を自動で重ね＋「N年ずれ」トースト＝「重ね歩き」**）／相手の写真は 地図・地図TL・囲む だけ（visible から常時除外・mapVisible で opt-in・パック単位🖼トグル=線は残る）／相手の線=パック色破線+👤名前ラベル・読み取り専用／isPeerPt でロガー統計/削除/自動確定/手書き骨格から除外。preview E2E green（詳細は現在地）。**実機の手触りFB待ち**（iOS Safari 共有シート→LINE/AirDrop・受け取りの一連・重ね歩きが心に効くか・パック一覧の分かりやすさ）。⚠️**native は共有シート未対応（navigator.share が WKWebView に無い→正直案内実装済）＝次の iOS ビルドで @capacitor/share を足す**。詳細は memory [[sharing-with-close-people-requested]]。｜（履歴）v193＝✏️**期間絞り込み（クイック1週間など）からも手書きに入れる**（実機FB「一週間を選択→✏️→どの日から直す？モーダルが見つからない」＝正体は不到達：クイックは range を立てるが✏️入口は pickDays しか見ず「先に日を選んで」で弾かれていた）→range 中は期間内の暦日を日リスト化してモーダルへ（1週間=8日・写真ゼロ日も並ぶ・各日に「・N枚」）・期間10日超は「期間が長すぎます…期間を短く」警告。preview E2E green（実UIクイック経由 R1-R4＋range中の描画・console0）。**実機の手触りFB待ち**（一週間→モーダル→描くの一連・モーダル+チップの二段が冗長でないか）。｜（履歴）v192＝✏️複数日で「どの日から直す？」モーダル復活（v191でチップに統合したら入口が消えた→モーダル=入口＋チップ=編集中の切替の二段構え・単一日/🎲は直行・10日超警告）。｜（履歴）v191＝✏️**手書き修正を数日まとめて**（要望「数日の選択でも手書き修正したい・日数が多いと無理なときは押したら注意を」）＝複数日は**折り畳まず日切替チップ**（横スクロール・アクティブ日を強調＋その日へ地図を寄せる）で数日を1つずつ修正・**10日超は✏️押下で警告**（📅🎲で絞る）・旧「どの日?」別画面はチップに統合し撤去・単一日は従来どおり折り畳み。preview E2E green（D1単一日/D2複数日3=チップ3/D3日切替でタップが正しい日/D4 12日警告・console0）。**実機の手触りFB待ち**（数日切替の感覚・アクティブ日の見分け・上限10の妥当性）。｜（履歴）v190＝✏️**手書きモードを写真の無い日にも開放**（要望「位置つきが1点以下でも点を打ちたい・写真を撮らなくても振り返りたい日はある」）。**確定点（写真+ロガー）が2つ未満の日もフリーハンドで道を描ける**＝`realN` で分岐：**確定点≥2は v188 の按分を無改修（回帰ゼロ）**／≤1はタップ順＝道順で最寄り点の前後に入れ端の外側も許す（描き始め〜描き終わり）・空の日は正午起点。写真ゼロの日の入口＝**📅フィルタ「＋追加」で単日なら写真ゼロでも足せる**（range 空追加は従来どおり拒否）。線は保存せず点だけ＝データ増加の歯止め不変。preview E2E green（A 空の日=入れる＋タップ順 noon→+1分→+2分/B 確定点2枚=2枚の間に按分＝v188不変/C 📅単日追加→その日で手書き可・console0）。**実機の手触りFB待ち**（写真ゼロの日を描く体験・タップ順の直感性・地図センタリング）。｜（履歴）v187-188＝✏️**手書きの移動線**＝日を1日に固定→確定点の間に 🚶通過/☕滞在 をタップ→線は v139 時刻順マージ（手書き専用線なし）。v188＝②☕滞在を分かりやすく（塗り+白縁+☕マーカー）③挿入を「最寄り点＋前/後トグル」に（折り返す道で誤区間を解消・既定=後）。track に src:'hand' 同居（DB移行なし）・✓終了で自動確定を再試行＝描いた道に GPSなし写真が乗る。
> 2. **iOS**: 📤**1.5 審査待ちのまま**（ビルド46・承認後 自動リリース）。🍏**v189 で MARKETING_VERSION 1.5→1.6 に上げ済み（審査中の1.5と分離）＝手書き（v187-193）を native で試す用**。**▶ユーザーの手番＝Codemagic の `ios-testflight` を「Start new build」**→ 1.6(build N) が TestFlight 配信 → 実機で ✏️手書き（地図⋯「✏️この日の道を手書き」＝v190で写真ゼロ日もフリーハンド／v191-193で複数日・期間（1週間）から「どの日から直す？」モーダル→日切替で修正）＋v180-186。web/native コード無改修＝現状の web（**phase3.146**＝v187-194込み）が自動で乗る・カメラ権限は1.5(37+)で既存。⚠️**v194 の📤書き出しだけは native 未対応（WKWebView に navigator.share 無し→アプリ内で正直に案内）＝次の native 作業で @capacitor/share＋.json 受け取り口を足す**（📥取り込みは file input なので native でも動くはず＝実機で要確認）。**挨拶時 Gmail 確認済（2026-07-12 朝：1.5 審査結果まだ＝審査中のまま。TestFlight 42-46 の配信通知のみ）**。
> 3. **Android→Google Play**: ユーザーの手番のまま（①Play Console「アプリを作成」解錠確認②Pixel 7a 中古調達③鍵を Codemagic 登録→`android-release` で AAB 初回ビルド）。
> 4. **Claude の「待ち無し」タスクは無し**＝次は実機 FB か新要望から。挨拶時は [[session-start-gmail-check]]（Apple＋Play Console 通知）。
>
> ### 📍 （履歴）セッション32 の再開ポイント（2026-07-11 更新）
>
> **⏱ TL;DR（当時の4点）**
> 1. **web**: GitHub Pages **`phase3.139`（v180〜v186）** push 済。直近＝🧭**v186 タイムライン系の「最後に使った表示」を記憶**（地図TL/思い出の場所/重ね撮り/食べ物 の4画面それぞれ 一覧⇄写真・大きく/小さく等を localStorage 記憶し次回そこから開く・共通ヘルパ getViewPref/saveViewPref・pms-viewPrefs）／🎞️v185 連続撮影ラベル簡素化＋新規カメラのヒント／🎞️v184 iOS連続撮影の切り分け（新1.5ビルドで✅解消）／🐘v180-183 10万枚スケール（取り込み⏸停止・地図量適応・タイムライン分割・メモリ解放・ℹ️容量）。**実機の手触りFB待ち**（v186＝写真＋大きくで開いた時の初速）。①**取り込み**＝ヘッダ🌀タップ→⏸停止（confirm＋残り時間EMA・native/web/dev の3ループ共通・続きは差分同期＝📚再実行/選び直しで再開）＋native は**新しい順**取り込み＋枚数ラベルの O(n²) 修正 ②**地図＝量で自動切替**（線: ≤30日 日ごと色分け／≤200日 全日同色1本／超は線なし。写真: ≤300 サムネ／≤8千 白点canvas／超は**密度タイル**＝GridLayer 自作・密度中はタイムラインから spotlight・ドリルダウンは🫧囲む）＋推定≤200/記録点≤2万ガード ③**タイムライン**＝一覧2千超で窓モード（絞り込み前置＋200件ずつ＋IntersectionObserver 自動追い足し）・写真グリッド240ずつ（思い出の場所/レンズ/重ね撮りにも効く）・**全画面デッキは前後600枚だけ slide 化**（通し番号維持）。**閾値は全部定数＝実機FBで調整前提**（`MAP_MARKER_DOT_MAX` 8000・密度セル12px あたりが調整候補）。preview E2E green（#devダミー**8万件**：⏸停止実動・全期間71,991枚=密度+線0+一覧チャンク・3ヶ月=白点+単色線2レイヤ・1週間=従来・console 0）。**実機FB待ち＝密度タイルの見え方／白点の上限感／⏸の操作感／10万実写真の storage**。
> 2. **iOS**: 📤**1.5 を審査提出＝「審査待ち」（2026-07-11・承認後 自動リリース）**＝ビルド46（web v157-186込）。目玉🎞️重ね撮り（新規カメラ権限）＋🍜食べ物＋🐘10万枚スケール。最新情報=重ね撮り主役の payoff（日英）／審査メモ=1.5カメラ版に差替（あなた対応）／年齢制限=新ソーシャルメディア質問 いいえ＋13歳未満いいえ（4+維持）。✅新1.5ビルドで連続撮影OK確認済（v184切り分け＝TestFlight(42)がv177前が正解）。**▶次＝審査結果待ち→承認で自動公開**（Chrome MCP で状況確認補助可）。v180-186 は web のみ＝native 契約不変（次ビルドに自動で乗る）。
> 3. **Android→Google Play**: ユーザーの手番のまま＝①Play Console「アプリを作成」解錠確認②Pixel 7a 中古調達③鍵を Codemagic 登録→`android-release` で AAB 初回ビルド（今すぐ可）。掲載文ドラフトはセッション30の会話内。
> 4. **Claude の「待ち無し」タスクは無し**＝次は実機 FB か新要望から。挨拶時は [[session-start-gmail-check]]（Apple＋Play Console 通知）。
>
> ### 📍 （履歴）セッション31 の再開ポイント（2026-07-11 更新）
>
> **⏱ TL;DR（当時の4点）**
> 1. **web**: GitHub Pages **`phase3.134`（v177〜v179）** push 済＝🎞️実機FB対応「①シリーズ画面の← 一覧撤去＋✕で一覧へ（ドリルダウン）②🐛横持ちで⚙パネル8行があふれ最下段の🔁連続撮影が見切れて『選択できない』→ max-height:500px 以下は2列グリッド（**実機YES『縦にしても横にしても完璧に見やすい』**）③『写真の形』トグル＝新規（▯縦長⇄▭横長・v178）→ **v179 でフォルダーのカメラにも拡張（自動=基準写真と同じ→縦長→横長・root.rpAspect でフォルダーに記憶・明示指定中は基準スワイプでも枠が揺れない）**＝実機FB『保存範囲の白い破線が縦/横で安定しない→⚙で選べるように』への回答」。**実機（破線ガイドが安定したか・形の3段トグル）の手触り FB 待ち**。
> 2. **iOS**: TestFlight **1.5(42) が 7/11 0:47 配信**（メール確認済）。⚠️**v175(0:08 push)/v176(0:32 push) が乗っていない可能性**（配信 0:47＝ビルド開始が push 前かも）→ アプリの ℹ️ で BUILD を確認、`phase3.130` 未満なら **Codemagic で新 1.5 ビルド**（v177 まで乗せる）＝ユーザー操作。確認事項は下のセッション30 TL;DR ①のまま（🎞️カメラ WKWebView・カメラロール保存・Quick Action）。
> 3. **Android→Google Play**: 🎉本人確認 完了（7/10）。ユーザーの手番＝①Play Console「アプリを作成」解錠確認②Pixel 7a 中古調達③鍵を Codemagic 登録→`android-release` で AAB 初回ビルド（今すぐ可）。掲載文ドラフトはセッション30の会話内。
> 4. **Claude の「待ち無し」タスクは無し**＝次は実機 FB か新要望から。挨拶時は [[session-start-gmail-check]]（Apple＋Play Console 通知）。
>
> ### 📍 （履歴）セッション30 の再開ポイント（2026-07-10 更新）
>
> **⏱ TL;DR（当時の4点）**
> 1. **iOS**: 1.4 公開中／1.5(38-41) TestFlight。次＝**Codemagic で新 1.5 ビルド**（web v157-173＋native `savePhoto` が乗る）→実機で 🎞️重ね撮りカメラ（WKWebView で出るか＝session28 の宿題）・カメラロール保存・スワイプ削除の手触り。＝**ユーザー操作**。
> 2. **Android→Google Play（本命・進行中）**: 🎉**本人確認 完了（2026-07-10 17:18 メール・Play Console からの通知）＝律速が解けた**。**ユーザーの手番**＝①Play Console で「アプリを作成」が解錠されたか画面確認（→掲載文ドラフトは会話内・§📝）②Pixel 7a 中古 購入（デバイス確認＋テスト用）③鍵を Codemagic 登録→`android-release` で **AAB 初回ビルド**（パイプライン未検証）。個人アカウントは production 前に「20人×14日クローズドテスト」必須（要再確認）→初回ゴール＝内部テストに署名付き AAB。
> 3. **web**: GitHub Pages `phase3.131` 全 push 済（**v175＝⚙パネル収納・全画面復活・前回モード記憶・🔁連続撮影／v176＝📣シャッター音（合成Web Audio・単発/連続とも撮影の瞬間）＋🐛連続撮影の増分がフォルダーに戻ると反映されないバグ修正（下敷きのシリーズビューを撮影ごとに再描画＋✕で shotRoot に着地）**。ほか v168-174）＝**v175/176 の実機の手触り FB 待ち**（⚙パネル操作感・連続撮影のテンポ・シャッター音の質感・増分が即見えるか）。
> 4. **Claude の「待ち無し」タスクは無し**＝次は「実機 FB」か「新しい要望」から着手。挨拶時は [[session-start-gmail-check]]（Apple＋**Play Console 通知**も見る）。
>
> **🎯 セッション29 ＝ Android → Google Play トラック着手（本命③）**。iOS 1.4 公開中／1.5(38-41) TestFlight は据え置き（下のセッション28ブロック参照）。
> **やったこと（native/CI のみ・web 無改修＝BUILD `phase3.123` 据え置き・CHANGELOG v168）**: Android の**署名付き AAB パイプラインを整備**＝(1)`android/app/build.gradle` に `signingConfigs.release`（Codemagic `CM_*` 注入を読む・手元 debug は guard で無影響）＋`versionCode` を `$BUILD_NUMBER` 採番 (2)`codemagic.yaml` に **`android-release` workflow 新設**（`bundleRelease`→署名付き `.aab`・`android_signing: [madeleine_upload]`）(3)`.gitignore` に `*.jks` 等。**アップロード鍵を keytool で生成済**（`Desktop\madeleine-keystore\madeleine-upload.jks`・alias `madeleine-upload`・reference 名 `madeleine_upload`・リポ外・`KEYSTORE-INFO.txt` 同梱）。
> **🎞️ web（同セッション後半・BUILD `phase3.124`・CHANGELOG v169）＝重ね撮りの削除を実装**: ①フォルダ削除（⚙設定「🗑このフォルダを削除」）②フォルダ内の写真削除（拡大表示「🗑削除」）。どちらも2択＝「📤フォルダから外す」(rephotoフラグ削除→通常写真として地図/タイムラインに残す)／「🗑非表示」(既存 `excludePhoto`＝どこからも消える・「🗑削除した写真」から復元可)。root(器)を外す時は最古の子を新rootに昇格＋メタ移管＋兄弟付け替え（スキーマレス維持・DB移行なし）。`buildTimelinePhotos` に後方互換の第5引数で🖼写真モードからも削除導線（既存4引数呼び出し不変）。**ユーザー質問への回答＝重ね撮り写真は端末の写真アプリに保存されない**（`doRephotoCapture`→`importOne` でアプリ内 IndexedDB のみ・`KEEP_ORIGINAL=false` でサムネのみ＝端末フォトを汚さない代わりにアンインストールで消える／端末フォトにも残したいなら native 実装が別途必要）。preview E2E green（純ロジック11＋UI配線5・console0）＝実機の手触りが次。
> **🎞️ web/native（同セッション・BUILD `phase3.125`・CHANGELOG v170）＝重ね撮りを端末フォトに保存＋撮影サイズ選択**: ①**端末の写真アプリ（カメラロール）にも保存**＝`PhotoLibrary` プラグインに `savePhoto` 追加（iOS=PHPhotoLibrary・`.addOnly` 権限＝Info.plist の `NSPhotoLibraryAddUsageDescription` 既存で追加不要／Android=MediaStore・新権限なし）、web はダウンロード（iOS Safari は共有→写真に保存）。②**撮影サイズ S/M/L トグル**（`captureMax` 1024/2048/3072・主にカメラロールに残る原画の解像度・アプリ内は thumb 不変）。端末フォトに残る＝アンインストールしても写真が残る。preview green（web・カメラスタブでサイズトグル＋回帰なし・console0）。**⚠️native の savePhoto は preview 検証不可＝次の Codemagic 1.5 ビルドで実機確認**（カメラロール保存・権限プロンプト）。
> **🎞️ web（同セッション・BUILD `phase3.126`・CHANGELOG v171）＝重ね撮り一覧で左スワイプ→フォルダ削除**: 一覧の各行を `.rp-row-wrap`（scroll-snap x）で包み左スワイプで「削除」を出す（CSS scroll-snap＝JS判定なし・実機サクサク）→既存 `promptRephotoFolderDelete` の2択（フォルダだけ削除／写真ごと非表示＝excludePhoto・🗑削除した写真から復元可）。行タップは従来どおりシリーズを開く。preview green（構造/2択モーダル/行タップ回帰なし・console0）＝実機の手触りが次。
> **🐛 web（同セッション・BUILD `phase3.127`・CHANGELOG v172）＝①撮影がシリーズに残らない回帰修正＋②思い出の場所も左スワイプ削除**: ①v170 の web ダウンロード fallback が iOS Safari で blob→ページ遷移し撮影フローを壊していた→**web の端末保存を撤去**（`saveRephotoToLibrary` は native 無ければ即 null・端末フォト保存は native のみ）。実 `doRephotoCapture` を preview で走らせシリーズ 1→2＋child ひも付き確認。②📍思い出の場所の一覧も `.anniv-row-wrap`(scroll-snap) で左スワイプ→「削除」（従来 inline🗑 は撤去・削除は既存 annivDelete＝場所だけ削除で写真は残る）。preview green（console0）。**教訓＝web で保存を撮影フローに割り込ませない（iOS Safari の blob download はページ遷移）**。
> **✅ 2026-07-10：Play Console 個人デベロッパーアカウント作成済み**（デベロッパー名「あの日」・アカウントID `9066571770168143110`・$25決済済み・連絡先メール `yutsutke@gmail.com`）。**本人確認（身分証）を提出済み＝審査中**（Google 側・数日）。**Android 実機は未所持と判明**（借り物 S21 でのテストは清掃済み）→ **Pixel 7a を中古で購入予定**（Android16まで・セキュリティ更新〜2028年目安・中古ショップ推奨＝ネットワーク制限○/初期化済みが担保されるため）。
> **⚠️訂正（前回の誤り）**: 「本人確認を待たずにアプリ作成に着手できる」という想定は**誤り**＝実際は Play Console の「アプリを作成」ボタン自体が**本人確認完了までロック**される（画面で確認済み＝「新しいアプリを作成するには、アカウント確認を完了してください」）。
> **⚠️状態**: **Play Console 側は完全に外部要因待ちで操作不可**（本人確認の審査結果／Pixel 7a 到着 → デバイス確認 → 電話番号確認 → アプリ作成 の順で解錠）。AAB パイプラインも**未検証**（Codemagic に鍵登録→初回 AAB ビルドで要確認・これは Play アカウント状態と無関係に今すぐ進められる）。**個人アカウントは production 前に「20人×14日クローズドテスト」必須**（現行ルール・要確認）→初回ゴール＝**内部テストに署名付き AAB**。
> **📝 ストア掲載文はドラフト済み（会話内・リポ未反映）**: 実際の App Store 掲載文（WebFetch で取得）を元に、Play 用に文字数調整済みの日英ドラフトを作成（アプリ名30字/簡単な説明80字/詳細な説明4000字/初回リリースノート）。**アプリ作成が解錠され次第そのまま貼り付け可**（このセッションのチャット参照・まだファイル化していない）。データセーフティは概念整理のみ（「収集なし・端末内処理」「FGS locationはACCESS_BACKGROUND_LOCATION不使用のため重審査回避見込み」）＝実際のフォーム項目は画面が出た時に確認。
> **🎯 次の一手（優先順）**: ①ユーザー＝**本人確認の結果を待つ**（Gmail確認・[[session-start-gmail-check]] に Play Console 通知も追加で見るとよい）②ユーザー＝**Pixel 7a を中古調達**（中古ショップ推奨・ネットワーク制限○/初期化済み確認）③ユーザー＝**鍵を Codemagic に登録**（`KEYSTORE-INFO.txt` の手順）→`android-release` で初回 AAB ビルド＝**パイプライン検証**（Play アカウント状態と無関係に今すぐ可能）④本人確認 OK＋端末到着後＝デバイス確認・電話確認・アプリ作成→ストア掲載文（ドラフト済み）を貼り付け。**チェックリストは下の「📋 Android → Google Play 着手チェックリスト」**（(c)(d) は基盤済・(a)一部済（登録済み・確認待ち）・(e)ドラフト済み・(f)(g) 残）。
>
> ### 📍 （履歴）セッション28 の再開ポイント（2026-07-09 更新）
>
> **🎯 いまの状態**: **🎉 iOS 1.4(35) は承認→公開済み（2026-07-09）**＝承認メール 00:39 JST → 約20分後にストア反映（WebFetch で ver 1.4・「7時間前」を確認）。**⚠️訂正: 旧記載「1.4=手動リリース」は実態とズレ＝リリース設定が自動になっていた**（深夜に自動公開・ユーザー操作なし）→ 以後のバージョンはリリース設定（自動/手動）を提出時に確認する。App Store 現行 = **1.4**（top/route ヒーロー＋v153-156 いまの場所/半径/地図バー＋v152・ex-EU 148）。**1.5(37) が TestFlight 配信済み（7/8）＝カメラ権限入り**→⚠️「🎞️重ね撮り」のカメラが WKWebView で出るか実機未確認（出なければ WKUIDelegate 橋渡しが次）。**v165（Quick Action／全画面カメラ）は 1.5(37) には未搭載＝次の Codemagic ビルド（build 38〜）から**。web = GitHub Pages **`phase3.123`（v165-167：🎞️重ね撮りのアイコン🎞️/「撮り重ねる」/全画面カメラ/iOS Quick Action配線／撮影写真に位置情報（現在地GPS→基準継承）／**フォルダー化**（位置継承トグル・基準写真変更・一覧から新規作成）／設定の再描画バグ修正（iOS Blob再put hang→楽観更新）／地図タイムライン🖼写真タブに大きく/小さく）**。Android = v142（クリーン起動＋写真全件 MediaStore まで・実機 GPS/大量スループット残・Play 未着手）。
> **🎯 次の一手（優先順）**: ①**Codemagic で 1.5 の新ビルド（38〜）**（ユーザーが Start new build・v157-167 全部乗る）→ TestFlight 実機で **(a) 🎞️重ね撮りカメラが WKWebView で出るか (b) ホーム画面アイコン長押し→重ね撮り一覧 (c) 全画面カメラの手触り**。②**web 実機（iPhone Safari・GitHub Pages `phase3.123`）で v165-167 の手触り**（全画面カメラ・重ね撮りのフォルダー運用（新規作成/基準写真変更/位置トグル）・⚙設定が即反映されるか・地図タイムラインの大きく/小さく）。③**Android を Google Play へ**（本命・下の「Android → Google Play 着手チェックリスト」・Quick Action の Android 版＝shortcuts.xml もこのトラックで）。
>
> ## 📍 セッション28（2026-07-09）＝🎉1.4 公開確認 ＋ 🎞️重ね撮り大改修（v165〜167）＋地図タイムライン
> - **このセッションのまとめ**: web を **phase3.118→3.123** に（7コミット）。①1.4 が自動リリースで公開済みと判明（記録訂正）②v165 重ね撮り第4周（🎞️アイコン・撮り重ねる・全画面カメラ・iOS Quick Action）③v165+ 撮影写真に位置情報④v166 フォルダー化⑤v166+ UI整理⑥**v166++ iOS 再描画バグ修正（楽観更新）＝ユーザー「一発で完璧」**⑦v167 地図タイムライン写真タブに大きく/小さく。**ユーザー FB は終始ポジティブ（「めっちゃサクサク/使いやすい」系）＝重ね撮りへの投資が深まり続けている signal**（[[rephoto-layered-shots]]）。全て preview E2E green・console 0・push 済み。native 契約は不変（次の 1.5 ビルドで自動で乗る／カメラ権限とQuickActionだけ build 38〜）。
> - **開始時確認（[[session-start-gmail-check]]）**: 1.4(35) 承認メール（00:39 JST）＋ 1.5(37) TestFlight 配信（7/8 17:01）。ストア実査（WebFetch）＝**1.4 が既に公開中（「7時間前」）→「手動リリース」の記録は誤りで自動リリースだったと判明**（公開通知メールは Apple から来ないことがある＝ストアページが正）。
> - **v165（web＋native 配線・このセッションの本体）**: ユーザー実機FB4点→ ①🎞️アイコン（AskUserQuestion でユーザー選択）②「撮り重ねる」③カメラ全画面化（オーバーレイUI）④iOS Quick Action（app-shortcuts micro-plugin 新設）。preview E2E 全green・console 0。詳細 CHANGELOG v165。
> - **v165+（位置情報・同セッション追記）**: ユーザー質問「重ね撮りで撮ると位置は保存されない？」→保存されていなかった→**現在地GPS優先→基準継承→null** を実装（ユーザー選択「現在地＋基準fallback」）。位置はレコードのみ=端末内・JPEG に焼かない。preview 3経路 green。詳細 CHANGELOG v165 追記。
> - **v166（フォルダー化・同セッション）**: ユーザー再定義「フォルダーのように使う（食事/コーデ/メイク/友達）」→ ①位置継承をフォルダーごとにON/OFF（rpInheritLoc・既定ON）②基準写真を変更可能（rpBasePhotoId・⚙設定ピッカー）③一覧「＋新しい重ね撮り」で新規作成（ゴーストなしカメラ・最初の1枚がrpFolder root）。preview E2E 全green・console0。詳細 CHANGELOG v166。
> - **v166+（UI整理・同セッション・ユーザーFB）**: ①基準日の設定を⚙設定へ移動（ログの基準日バー撤去）②名前はチップ撤去→題名の横の✎だけ（アクション行3ボタンに）。preview green。詳細 CHANGELOG v166 追記。
> - **v166++（設定の再描画バグ修正・同セッション・実機FB）**: ⚙設定/題名✎の変更が実機iOSで即反映されなかった（閉じて開くと反映）→原因＝`await dbPut(root)` 後に再描画＝iOS の Blob入り写真レコード再put で tx hang→**render-first＋rpPersist（保存は裏）**。基準写真ピッカーは留まって枠移動。preview で dbPut hang 差し替えでも即反映＝再現&修正確認。詳細 CHANGELOG v166 追記。**教訓＝iOSは写真レコード(Blob)の再put+await→UI更新が危険・楽観更新に倒す**。
> - **v167（地図タイムライン写真タブに大きく/小さく・同セッション・実機FB）**: 「写真タブの中に写真を拡大表示する切り替えボタン」→思い出の場所/重ね撮りと同じ ⊡大きく/⊞小さく を地図タイムラインの🖼写真タブにも追加（`mapState.tlBig`→`buildTimelinePhotos` big）。preview green。詳細 CHANGELOG v167。**実機の手触り（web＋次の1.5ビルド）が次**。
> - **⚠️次ビルドの注意**: CapApp-SPM/Package.swift はバニラのままコミットが規約（CI の `cap sync ios` が local-plugins を注入）＝app-shortcuts も自動配線される。SPM 名一致（AppShortcuts）済み。もし Codemagic が exit 74 なら名前規約を疑う（[[native-photo-access-works]] の既知罠）。
>
> ## 📍 セッション27（2026-07-08・web）＝🍜 食べ物レンズ（意味CLIPをテキスト検索に拡張・v157）
> - **ユーザー要望（実機で使いたい）**: 「意味モードを使って、食べ物の写真を日別・時系列で出せないか。最近なにたべたか思い出すのに」。
> - **やったこと（web のみ・native 契約不変・push 済・BUILD `phase3.110`）**: 意味モード（CLIP）は今まで「写真同士の類似」だけ→**CLIPテキスト側エンコーダを追加**（`AutoTokenizer`＋`CLIPTextModelWithProjection`・q8遅延ロード）。「食べ物」を512次元に起こし、**保存済み画像embedding（同じ512次元共有空間）とコサイン類似でゼロショット分類**（食べ物語群8＋対抗カテゴリ8を softmax・logit_scale100・しきい値0.5）→**食べ物写真だけを日別・時系列(新→古)で全画面表示**（⚙メニュー「🍜 食べ物を時系列で」＝`#btnFood`・表示は `buildTimelinePhotos` 再利用）。オーバーレイ z1900（拡大z2000の下＝タップ拡大が前面）。詳細 CHANGELOG v157。
> - **検証（preview 全green・console 0）**: **CLIPテキストエンコーダが preview ブラウザで完動**＝CDN import＋HF モデル DL（q8・100%）成功、`text_embeds` dim=**512**（画像と同一空間）・norms1.0・**意味の妥当性 cos(食×食)0.954＞cos(食×街/猫)0.876**。foodProbOf 数理健全（food→1.0/other→0.0）。need-ai 導線（「🧠 AI で解析する」）・z1900・開閉 OK。⚠️**実機の手触りは残**＝preview に実写真が無いので**食べ物/非食べ物の境界（プロンプト/しきい値）は実機のユーザー写真で調整が前提**。
> - **教訓**: 意味モードの資産（512次元 CLIP 画像 embedding）は**そのままテキスト検索に化ける**（共有空間ゆえ語を1回エンコードするだけでゼロショット分類・新しい重い解析は不要）。CLIPテキストはベース類似が高い（cos≈0.87）＝**生のしきい値は罠→logit_scale×softmax で差を増幅**が定石（preview の text-text 検証で裏取り）。
> - **🎉 実機 signal（v157-159）**: ユーザー「**いい感じで出ますね**」（食べ物判定が実写真で効く＝ゼロショット分類 YES）→ **v158：📅月ごと / 📆日ごと 切替** → 実機「**いいかんじ**」＋要望「思い出の場所のように⇆比べる／写真を大きく」→ **v159：⇆比べる＋大きく/小さく** → 要望「風景・人・ペットの別レンズ／フリーテキスト検索／写真1枚UPで似た写真」→ **v160：🔍意味レンズに汎用化**（カテゴリ4＋フリーテキスト＋写真で探す＝reverse image search・同じ CLIP 埋め込み・結果は既存タイムライン・⚙「🔍意味で探す」＋レンズバー・preview 合成E2E green）。
> - **🧊 実機 verdict（v160 同日）**: ユーザー「AIの性能が限界ありそう。とりあえず食べ物だけは使えそう。これはここまでにしましょう」＝**意味レンズの拡張（風景/人/ペット/テキスト/写真検索）とチューニングは凍結**（食べ物だけ実用・UI チップは残置・既定=食べ物・「今は作らない」リストに記載）。
> - **📐 v161-163 重ね撮り（旧称 定点）＝実装済み（同日・ユーザー選択「ゴースト重ねで web v1」→FB を2周反映）**: 拡大表示「📐この角度でまた撮る」→ゴーストカメラ→撮影→シリーズ。**実機FB①「左右分割だと一部しか見えない」→ v162＝上下＆contain（全体表示）＋保存範囲の破線ガイド**。**実機FB②「上下よさそう・グリッド線が欲しい・思い出の場所みたいにメニューから／タイトル・基準日・メモ」→ v163＝⊞グリッド線＋一級化**（⚙「📐 重ね撮り」一覧・✏タイトル・🎯基準日・日ごとメモ・📖ログ/🖼写真・メタは root レコードの rpTitle/rpBaseDate/rpNotes＝スキーマレス）。**実機FB③（スクショ・バックパック定点）「⏱タイマー欲しい・基準にもグリッド・グリッドが写真外にはみ出す・上下で同じ位置に」→ v164**（⏱セルフタイマー・グリッドを写真領域にフィット・上下両方・2段UI）。preview E2E 全green。**▶ 次＝①実機 iPhone Safari（GitHub Pages）で一式の手触り**（タイマー秒数感・グリッドの見やすさ・ログの書き心地・一覧回遊）**②刺されば iOS 1.5 でカメラ権限**（Info.plist `NSCameraUsageDescription`＋Android Manifest `CAMERA`/WebView 権限橋渡し確認＝**今の 1.3/1.4 アプリでは動かない**）③次段候補＝重ねてスライダー再生・最初の基準をアプリ内で撮る導線・シリーズ削除/写真の取り外し。v157-163 は次の Codemagic ビルドで iOS/Android に乗る（**ただしカメラだけは権限宣言が要る＝1.5**）。
>
> ## 📍 セッション26（2026-07-08）＝iOS 1.4(35) 審査提出（新ヒーロー top/route ＋「積み重ね」プロモ）
> - **🎉 1.4(35) を審査提出済み**（ユーザーが ASC で操作・最終「審査へ提出」を押下）。中身＝v153-156＋v152。**手動リリース**＝承認後の公開はユーザー。開始時 Gmail 確認＝1.4(35) TestFlight 到着（7/7）・1.3(34) 承認/公開（既知）を把握。
> - **スクショ刷新（café→top→route＋既存）**: ユーザー提供の実機スクショ4枚を **960×2079→1284×2778**（6.5"スロット・既存 café と同寸）に Pillow(Lanczos/4:4:4) でリサイズ。top=過去の今日/明日＋📍いまの場所＋写真1枚＝**局所**／route=6/8〜7/8 の足跡の束・クラスタ数字(218…)・🫧囲んで見る＝**統計**。狙い＝**局所↔統計の対比＋積み重ね**。保存先＝`Desktop\あの日２６０７０８\1284x2778\`（元は別フォルダ保持）。日=topjp/routejp・英=topen/routeen。⚠️ASC への配置はユーザーが **D&D**（セッション外パスは file_upload 不可＝[[madeleine-product-repo]]）。
> - **What's New（日英・確定）**: 「いまいる場所から、忘れていた時間がよみがえる。」＋3点（📍いまの場所／半径スライダー／地図の期間クイック絞り込み1週間〜3年＝足跡の束）。英 "Right where you are, forgotten time comes back to life." ＋対応3点。
> - **プロモ用テキスト（新・「積み重ね」／採用=layered）**: 🇯🇵「あの場所に積み重ねられた思い出。積み重ねられた日々の足跡。積み重なった時間が、ある日ふっとよみがえる。」🇬🇧「Memories layered at the places you go. Footprints layered day upon day. And one day, all that layered time quietly comes back to life.」＝**静→静→動**（体言止め2＋完全文1）で三連 layered。方針＝**プロモ=積み重ね／説明文=よみがえる物語**の二段構え。審査不要・即時反映＝1.3 にも適用可。
> - **コード変更なし**（index.html 無改修＝BUILD `phase3.109`/v156 のまま・**CHANGELOG 新 vN なし**）。store-ops のみのセッション。
> - **▶ 次の一手**: 上記「次の一手」。①1.4 審査結果待ち→承認で手動リリース ②Android を Google Play へ。
>
> ## 📍 セッション25（2026-07-07・web）＝「いまの場所」動線＋地図UI v153→v156
> - **🍎 iOS 1.4 train を開始（この web 改善を実機/TestFlight へ・2026-07-07）**: `MARKETING_VERSION` を **1.3→1.4** に上げた（Debug/Release・pbxproj）。**理由＝1.3 は公開済みで train が閉じている**＝同バージョンだと Codemagic Publishing が 90186/90062 で失敗する（v148 の実例）。**ビルド番号は Codemagic が `$BUILD_NUMBER` で自動採番**（`agvtool new-version -all`）＝手で変えない。**トリガーは手動**（ios-testflight に `triggering:` 無し・ユーザーが Codemagic で「Start new build」）→ **✅ 2026-07-07 ビルド完了＝1.4 が TestFlight へ**（build 自動採番・`submit_to_testflight:true`・審査提出はしない）。**次＝TestFlight 実機で v153-156 の手触り確認 → 良ければ 1.4 を ASC で審査提出**（公開は手動）。中身＝v153-156（いまの場所/半径選択/地図バー再構成/クイック期間/2段目1行）。
> - **要望C（v154 を実機で触って→v155）**: ①「囲んで思い出を見る」を⋯メニューでなく**地図バー**に②バー並び=1段目 過去の今日・全期間／2段目 🎲・現在地(📍だけ)・囲んで思い出を見る・表示（◀▶日ナビは残す＝ユーザー選択）③期間絞り込みに**1週間/1ヶ月/3ヶ月/1年/3年**（今日から遡って）④ヘッダ拡大時の3段折り返し。→ **v155**＝地図バー再構成（mmCompare→mcEncircle 昇格・mcToday ラベル化・全期間を1段目・現在地📍のみ・◀▶維持）＋クイック期間ボタン＋`.hdr-primary-row` を均等幅nowrap（拡大でも2段維持）。preview green（バー座標2段・囲む起動・クイック「1年」=366日・ヘッダ375/320px 1行・console 0）。→ **v156**＝2段目が長くスマホ縦で3段折り返し→「🫧 囲んで思い出を見る」を「🫧 囲んで見る」に短縮（preview 375/320px 両段1行）。詳細 CHANGELOG v155/v156。BUILD `phase3.109`。**実機の手触り残**。
> - **要望A（実機・iOS 1.2 の地図）**: 「トップから“現在地で囲んで思い出を見る”状態まで**ボタン一つ**で。手順を最小限に」→ **v153**。
> - **要望B（v153 を実機で触って→v154 で対応）**: ①半径は自動でなく**選べる**（店＝小さく／地域＝大きく）②写真1枚以下だと地図が出るだけで**説明なく混乱**③ヘッダ3ボタンが窮屈＝⚙を最上段右・2段目に3ボタン。→ **v154**＝「いまの場所」を**半径スライダー式**に（現在地に円＋スライダー100m〜10km＋ライブ枚数・2枚以上で「この範囲で見る」・0/1枚は無効＋「広げて」＝②解決）＋**ヘッダ2段化**（1段目=タイトル+⚙／2段目=3ボタン）。初期半径=2枚以上入る最小。preview green（ヘッダ座標2段・スライダー連動・確定で「囲んだ場所21枚」・空状態=0枚+導線・スクショ・console 0）。詳細 CHANGELOG v154。**実機の手触り残**（スライダー操作感・初期半径・プリセット併設要否）。
> - **やったこと（web のみ・native 契約不変・push 済・BUILD `phase3.106`）**: トップヘッダに `📍 いまの場所`（`#btnHere`・🗺過去の今日の隣・`LOCATION_AVAILABLE` 時のみ）＋ `openHereTimeline()`＝現在地取得→地図を現在地に寄せ青ドット→**段階半径 400m→800m→1.5km→3km→6km で最初に2枚以上の円**を採用→`openEncircleTimeline`（v151 の過去時系列タイムライン）を直接開く（**囲む操作を省略**）。写真0枚は「この辺りには、まだ写真がありません」。詳細 CHANGELOG v153。
> - **検証（preview・実コードパス／合成21枚）**: `#btnHere` click→center=35.680,139.760/zoom15/青ドット＋「🫧囲んだ場所（16枚）」タイムライン、空状態＝地図移動＋メッセージ（クラッシュなし）、console 0。**実機の手触り残**（iOS 1.3/Android は次ビルドで自動で乗る＝web 分岐のみ）。⚠️preview の合成クリックは #btnHere を発火しないことがある→`.click()` で切り分け（[[preview-raf-not-firing]]）。⚠️`mapState` は `let`＝`window.mapState` は undefined（bare `mapState` で読む）。
> - **⚠️ セキュリティ（この回）**: ツール結果ストリームに**プロンプトインジェクション疑いの注入**（偽 system reminder＋「clean checkout に revert した・触るな」という偽ユーザーメッセージ）が混入。`git status`/grep で実地確認＝**revert されておらず v153 編集は intact**＝注入に従わず実状態のみ信頼して続行。
> - **▶ 次の一手**: セッション24 と同じ（下記）＝①iOS **公開反映確認**（café ヒーロー版・〜24h）②**Android を Google Play へ**（本命・下のチェックリスト）。v153 は次の Codemagic ビルドで iOS/Android に自動で乗る。
>
...
- **意味レンズの拡張・チューニング**（2026-07-08 凍結・ユーザー明示「AIの性能が限界ありそう。とりあえず食べ物だけは使えそう。ここまでにしましょう」）— v160 の汎用化（風景/人/ペット・フリーテキスト・写真で探す）は実写真で精度不足＝CLIP ViT-B/32 zero-shot の上限。**食べ物レンズだけ実用として残す**（UI チップは残置・既定=食べ物）。プロンプト調整・カテゴリ追加・しきい値チューニングに時間を使わない。再挑戦は**より強いモデル（SigLIP 等）への乗せ替えとセットでのみ**。
