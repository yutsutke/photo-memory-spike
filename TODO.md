...
> - **▶ 次セッションの一手**: ①この回の web ブラッシュアップ（v128〜）を実機（GitHub Pages）で確認 → 続きの直しがあれば対応 ②固まったら **ASC を開いて実際のボタンで再提出手順を確定**（承認済み16→新ビルドへ差し替え。Pending Developer Release 状態からの差し替えは挙動が変わるので憶測で操作せず現物を見る）。TestFlight は 1.0(20) まで自動蓄積（v111-127 系）。
...
> - 検証＝inline script vm.Script **0エラー**／preview で②③実測 green・①はロジック（分岐＋既存 openAnnivList・空表示対応・anniv-z z5000 で地図の上）で担保。**実機（GitHub Pages phase3.81）で3点確認待ち**。
>
> ### 📍 次セッションの再開ポイント（2026-06-30〜07-01 セッション9 更新）
>
> ## 🎉🎉 App Store 承認！（2026-07-01）＝**最初の一手は「公開（手動リリース）」**
...
> - **▶ 次セッションの最初の一手**: セッション開始 Gmail 確認（[[session-start-gmail-check]]）の後、**ユーザーに「公開しますか？」を確認 → ASC（Chrome 拡張）でステータスを見る**。①「Pending Developer Release（デベロッパによるリリース待ち）」なら「このバージョンをリリース」を押す ②既に「販売準備完了/処理中」なら自動公開進行中。契約（Agreements/Tax/Banking）が有効か念のため確認（メール注記）。
