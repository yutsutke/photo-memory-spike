...
> - **📤 v237（phase3.184）＝🛰️パネルに「足あとを書き出す」**。きっかけ＝「取り込みテストのために全削除すべきか」→ **🗑 全削除は `trackClear()` も呼ぶ＝軌跡も消える**と判明。**ゆうの判断＝「削除はいまのまま全削除で大丈夫。開発者以外、足跡のみを残すなんて使わない。新規で足跡の情報をエクスポートできる窓口を作って」**＝削除の作り分けはせず、**別窓口で逃げ道を用意**。自分の足あと（🛰️ロガー＋✏️手書き・🤝相手は除外）を JSON 1ファイルに→共有シート/ダウンロード（`sharePackFile` 再利用・`anohi-足あと-YYYYMMDD-HHmmss.json`）。preview E2E ✅ console 0。**⚠ 読み込み（復元）の口は無い＝いまは一方通行**（作るかは要判断・`trackPutMany` があるので実装は小さい）。
