...
## 現在地 — BUILD: phase3.8 (線=旅行ごと1本 + 地図UI整理)
...
> **🗺 直近の周回 (v49-v57, 2026-06-14〜15) = 地図ビュー一式 + 取り込み簡素化 + 記憶の封印 + 線/UI整理。** 軌跡+タイムライン / **線=旅行ごと1本（旅行の区切りを日数で選択・既定2日、v57）** / 日付・期間フィルタ（地図の📅パネル↔タイムラインの日付タップを双方向同期、絞ると一覧もその期間だけ）/ 連想ウォーク↔地図ピンの双方向リンク / 取り込み1本化（📷取り込む + ⚙️詳細設定）+ 日時の出所表示（撮影日/保存日/投稿日）/ **記憶の封印（地図🔒 で「見えている範囲×期間」を条件保存して非表示・いつでも解除、v56）** / **地図上部を「📅 期間 + ⋯ 表示（線/封印）」に整理（v57）**。**preview で全 E2E green、次の一手 = 実機 (iPhone Safari) で手触り確認**（[[map-view-unparked]]）。
...
### ✅ 線=旅行ごと1本 + 地図UI整理 (v57, 2026-06-15)
- ユーザー「つなぐ線も**1日単位で選びたい**。2日の旅も5日の旅も**動静をすべて一本でつなぎたい**」+「マップの**日ごとと🔒は基本使わない**。上部は**期間ともう一つのアイコン**（タップで日ごとと🔒を選べる）に」。
- **線モードを「旅行の区切り＝日数」に再設計**：`1日/2日/3日/5日/1週間/ぜんぶ/日ごと`。gapDays 以上あいたら別の線（既存 `segmentByGap` 流用）。**既定=2日**（連続した旅行を1本に。毎日撮る旅行は内部 gap<2日で連結、別の旅行とは数日空くので分離）。`日ごと`（暦日分割＝避けたい過分割）は末尾に退避。選んだ区切りは localStorage 永続（`pms-lineMode`）。
- **地図上部を「📅 期間」+「⋯ 表示」の2つに整理**。⋯ → メニュー（〰 線のつなぎ方 / 🔒 記憶の封印）。常時トップは主役の期間だけ（[[ui-minimalism-works]]）。
- preview E2E green（2日旅(4枚)+5日旅(5枚) が 1〜1週間どの区切りでも `[4,5]`=各1本 / 日ごと=7本 / ぜんぶ=1本。UI=📅+⋯のみ・メニュー2項目・線7チップ・区切り永続・⋯→🔒）。詳細 CHANGELOG v57。
- **残: 実機で既定2日の旅行体感 / 中日に写真ゼロだと割れる点 / ⋯ メニューの発見性。**
