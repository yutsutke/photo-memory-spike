...
- **純正写真アプリへの動線**(v24) — フル表示 (長押し) に **日時を大きく表示** (純正アプリで辿る鍵)、タップでコピー、「📷 写真アプリで開く」(`photos-redirect://`)。**Web は写真アプリの特定写真を直接開けない**ので「アプリを開く+日時で自分で辿る」が上限
...
- **(実機計測待ち) 1万枚の実挙動** — 取り込み所要・発熱・実 quota・dedup の遅さ (10k で 10k トランザクション)。重ければ: dedup を一括 `getAllKeys` で Set 化して in-memory 判定 / CLIP 自動抽出に低優先間隔 / thumb 品質 (512 で十分か)
