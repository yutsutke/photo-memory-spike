...
## 現在地 — BUILD: phase3.20 (折りたたんだ日は地図の線も非表示)

> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.20`。**v64 入口モデル → v65 TL対応 → v66 飛び飛び+クラッシュ根治 → v67 非連続の置き場所整理 → v68 タイムライン折りたたみ → v69 折りたたみと地図の線を連動**（preview E2E green）。**v56〜v69 の周回は全て preview green だが、まとめて実機 (iPhone Safari) 未確認**。
> - **v69 で何が変わったか**: **タイムラインで折りたたんだ日は、地図の軌跡（線）も非表示**（マーカーは残す＝「線」だけ消す）。`drawTrips` が `foldedDays` の日の線を描かず、`toggleFold` は `applyFilter({skipFit:true})` で地図とタイムラインを一括更新（ビュー維持）。
> - **v68 で何が変わったか**: もう一度引く/写真入口のタイムラインで日見出しタップ＝その日の写真を折りたたむ/開く（▾/▸）。`mapState.foldedDays`、再構築でリセット。browse/days は従来どおり日タップ＝選択。
...
- **残: 実機で「線だけ消しマーカーは残す」が直感に合うか（マーカーも消したくなるか）。**
