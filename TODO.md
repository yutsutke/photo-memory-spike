...
> - **📐 v161-162 定点＝実装済み（同日・ユーザー選択「ゴースト重ねで web v1」）**: 拡大表示「📐この角度でまた撮る」→ゴーストカメラ（半透明の基準＋分割切替＋前後カメラ）→撮影→定点シリーズ（日ごとタイムライン＋⇆比べる＋また撮る）。**実機FB①（同日）＝「左右分割だと両方一部しか見えない・小さくてもいいから全体を」→ v162＝分割を上下＆contain（全体表示）＋ステージいっぱい＋保存範囲の破線ガイド**。preview E2E 全green。**▶ 次＝①実機 iPhone Safari（GitHub Pages）で手触り再確認**（ゴーストの合わせやすさ・上下分割の見え方・前面ミラー）**②刺されば iOS 1.5 でカメラ権限**（Info.plist `NSCameraUsageDescription`＋Android Manifest `CAMERA`/WebView 権限橋渡し確認＝**今の 1.3/1.4 アプリでは動かない**）③次段候補＝重ねてスライダー再生・最初の基準をアプリ内で撮る導線。v157-162 は次の Codemagic ビルドで iOS/Android に乗る（**ただしカメラだけは権限宣言が要る＝1.5**）。
>
