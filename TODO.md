...
> ### 📍 次セッションの再開ポイント（2026-07-07 セッション25 更新・まずここを読む）
>
> **🎯 いまの状態**: iOS **1.4（TestFlight ビルド完了・2026-07-07）**＝今日の web 改善 v153-156 入り（いまの場所/半径選択/地図バー再構成/クイック期間/2段目1行）。web = GitHub Pages `phase3.109`。iOS **1.3 は公開中**（ex-EU 148）。Android = v142（クリーン起動＋写真全件 MediaStore まで・実機 GPS/大量スループット残）。
> **🎯 次の一手（優先順）**: ①**1.4 を TestFlight 実機で確認**（今日の改善の手触り＝いまの場所の半径スライダー/地図バー/クイック期間）→ 良ければ **ASC で 1.4 を審査提出**（What's New 日英・手動リリース・最終ボタンはユーザー）。②**Android を Google Play へ**（本命・下の「Android → Google Play 着手チェックリスト」）。③（習慣）開始時に **Gmail で Apple/App Store 系メール確認** [[session-start-gmail-check]]。
>
