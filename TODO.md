...
>
> **🎯 いまの状態**: iOS **1.4（35）を審査提出済み（2026-07-08・審査中）**＝v153-156＋v152。**手動リリース＝承認後の公開ボタンはユーザー**。開始時 Gmail 確認＝**1.4(35) の審査完了メールはまだ無し**（提出当日＝結果待ちは想定内。最新の「審査完了」メールは 1.3(34) 承認＝既知）。web = GitHub Pages **`phase3.113`（🔍意味レンズ＝v157 食べ物→テキスト検索／v158 月・日切替／v159 ⇆比べる＋大きく/小さく／**v160 汎用化＝カテゴリ(食/風景/人/ペット)＋フリーテキスト＋写真で探す**）**。iOS **1.3 は公開中**（ex-EU 148）。Android = v142（クリーン起動＋写真全件 MediaStore まで・実機 GPS/大量スループット残）。
...
> - **▶ 次の一手**: 📐**定点観測（同じ角度を時間を空けて重ね撮り）**＝ユーザー要望（体形・町の風景。基準写真を見ながらアプリ内カメラで撮る）。方式＝ゴースト重ね（基準写真を半透明でカメラに重ねる）を軸に検討→スパイクへ。v157-160 は次の Codemagic ビルドで iOS/Android に自動で乗る（web 完結）。
>
