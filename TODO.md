...
## 現在地 — BUILD: phase3.66（v113：ヘッダ整理＝🗺 過去の今日を主役ボタンに、残り(取り込み/AI/復元/削除/情報)を ⚙️ ドロップダウンへ集約。preview green。｜v111-112：トップ1枚大表示＋横スワイプ＋微調整＝**実機で「完璧」YES**（スワイプ・この日の地図へ・トップに戻る）。**TestFlight 次ビルドで v111-113 を実機確認・審査中 1.0(16) とは別トラック＝承認後の次版用**。｜v110：✅ 初回リジェクト対応＝再提出済み＝「審査待ち（2回目）」（2.3.8 名前→1.0(16)／1.5 Support URL→support.html・連絡先 anohiapp@gmail.com／審査メモ）。**次＝審査結果待ち→承認後 ASC で手動公開**。英語名 A Past Day — Photos and Footprints・v1.1。｜📝次段＝起動時の自動差分取り込み(native)。）
