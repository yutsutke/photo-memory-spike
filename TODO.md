...
>
> **🎯 いまの状態**: iOS **1.4（35）を審査提出済み（2026-07-08・審査中）**＝v153-156＋v152。**手動リリース＝承認後の公開ボタンはユーザー**。開始時 Gmail 確認＝**1.4(35) の審査完了メールはまだ無し**（提出当日＝結果待ちは想定内。最新の「審査完了」メールは 1.3(34) 承認＝既知）。web = GitHub Pages **`phase3.110`（🍜v157 食べ物レンズ＝意味CLIPをテキスト検索に拡張）**。iOS **1.3 は公開中**（ex-EU 148）。Android = v142（クリーン起動＋写真全件 MediaStore まで・実機 GPS/大量スループット残）。
...
> - **▶ 次の一手**: 上記「次の一手」②＝**実機で食べ物レンズを試す＆チューニング**。v157 は次の Codemagic ビルドで iOS/Android に自動で乗る（web 完結）。
>
