...
> 3. **web**: GitHub Pages `phase3.130` 全 push 済（**v173＝基準写真スワイプ切替＋📌確定＋横持ち左右分割→「完璧」／v174＝stage inset で重ねない→実機FB「画面が小さくて醜い」で撤回／v175＝アイコン整理で最大化と両立＝画面上は⚙とシャッターだけ・ツール類は⚙パネルに収納（基準バー/スライダーは表示切替）・モード等を localStorage rpCamPrefs で記憶（前回のゴースト/2画面で開く）・🔁連続撮影（撮ってもカメラに残る・新規は1枚目で器に変換）**。ほか v168-172）＝**v175 の実機の手触り FB 待ち**（⚙パネル操作感・連続撮影のテンポ・復元の体感）。
