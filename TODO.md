...
## 現在地 — BUILD: phase3.65（v112：トップ微調整＝🎲もう一度引き廃止→スワイプ説明／写真に「この日の地図へ」導線／連想ウォークの戻るを「← トップに戻る」に。preview green。｜v111：トップ1枚大表示＋横スワイプ化（実機で「ストレスなく引けていい」＝YES）。**TestFlight 次ビルドで v111+v112 を実機確認・審査中の 1.0(16) とは別トラック＝承認後の次版用**。｜v110：✅ 初回リジェクト対応＝再提出済み＝「審査待ち（2回目）」（2.3.8 名前→ビルド1.0(16)／1.5 Support URL→support.html・連絡先 anohiapp@gmail.com／審査メモ）。**次＝審査結果待ち→承認後 ASC で手動公開**。英語名 A Past Day — Photos and Footprints・v1.1。）
