...
## 現在地 — BUILD: phase3.19 (もう一度引く: 日見出しタップで折りたたみ)

> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.19`。**v64 入口モデル → v65 TL対応 → v66 飛び飛び複数日+クラッシュ根治 → v67 非連続の置き場所整理 → v68 入口モードのタイムライン折りたたみ**（preview E2E green）。**v56〜v68 の周回は全て preview green だが、まとめて実機 (iPhone Safari) 未確認**。
> - **v68 で何が変わったか**: **もう一度引く/写真入口のタイムラインで日見出しタップ＝その日の写真を折りたたむ/開く**（▾/▸）。各日に写真が多くても見出しだけにできる。地図は変えない（タイムラインだけ）。`mapState.foldedDays`＋`applyFold`、再構築でリセット。browse/days は従来どおり日タップ＝選択。
