...
## 現在地 — BUILD: phase3.14 (一日のみ表示: 線を時刻→空の色に)

> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.14`。**v62 = 地図の表示基準を「全件」→「入口で決まる seed」へ転換 / v63 = 一日のみ表示の線を時刻→空の色に**（どちらも preview E2E green）。これを含む **v56〜v63 の周回は全て preview green だが、まとめて実機 (iPhone Safari) 未確認**。
> - **v63 で何が変わったか**: **「一日のみ表示」のときだけ、線を `skyColor(時刻)` のグラデーション**に（夜明け藍→朝焼けオレンジ→青空→夕焼け茜→夜濃紺）。単一日判定＝`lineGps` の暦日数==1 で、seed 単一日 / 📅単日 / 日付タップ の全入口に自動で効く。**複数日は v58 パレット（昔→今）据え置き**。矢印は薄い白に。
> - **v62 で何が変わったか**: 地図は「全 GPS を撒く」のをやめ、**入口で seed を1つ持つ**。ヘッダ🗺＝**偶然の1スライス**（ランダム写真の時空近傍）/ 写真からの入口＝その写真の**時空近傍＋意味echo（時空無視で似た場所）**。開幕は時空にタイト、`🌍 似た場所 N` で意味echoを地図に広げる。線は時空近傍だけ（線あり=足跡 / 破線紫の単独ピン=意味echo）。seed 時は 📅⋯ を隠し `🌍`/`↩︎全部を見る` を出す。→「位置写真が増えるほど破綻する」問いを **seed で有界化**して解いた。
