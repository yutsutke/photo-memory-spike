...
> 2. **iOS**: ✅**新1.5ビルドで連続撮影OKをユーザー確認（「やったー」）＝v184の切り分け（TestFlight(42)がv177より前）が正解**。残る確認＝カメラロール保存・Quick Action（メモ: v185 のヒント/ラベルは次のビルドから）。v180-183/185 は web のみ＝native 契約不変（次ビルドに自動で乗る）。
