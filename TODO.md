...
> **🔧 セッション46（2026-08-23 朝）＝🗺 あの日本体を久しぶりに改修＝v236「全期間の地図から写真が消えていた」を直した。phase3.183。web は GitHub Pages 即反映・native は次ビルド（iOS 1.7 / Android vc5）で乗る。**
>
> - **実機FB**＝「位置情報を ON にしてから2〜3か月。全期間にすると移動線のみで、写真が出なくなってしまった」。→ **真因＝v181 の見せ方の自動切替（≤300 サムネ / ≤8千 白点 / 超 密度）の母数が「期間の総枚数」だったこと**。v140「足跡から写真の位置を自動確定」が効き続けて GPS 付き写真が単調増加するので、**一度 300 を超えたら全期間では二度と戻らない**＝写真が永久に白点のまま。位置ロガーを入れた時点で仕込まれた時限式の劣化（v181 単体でも v140 単体でも起きない）。
> - **直し方**＝①母数を **「いま画面に写っている範囲の枚数」** に変更（閾値の数字は不変・意味だけ変えた）＝引けば点/密度、寄ればサムネ、が連続する ②**白点/密度でも期間全体から等間隔の代表サムネ60枚を常に重ねる**（`MAP_SAMPLE_THUMB`）＝どのモードでも「写真がある」ことを写真で示す ③`markerById` の意味が「画面内か」に変わったので、それを述語に使っていた3か所を `isOnMapNow()`＋`focusPhotoOnMap()` に統一（タイムラインの写真タップが全画像へ逃げる回帰を防ぐ）。
> - **ハマり**＝`cluster.zoomToShowLayer` は張り替えと共存不可（moveend/animationend に自前ハンドラを残す→`clearLayers()` 後に `_leaflet_id in undefined`＝v181 のコメントが警告していた地雷を確実に踏む）→ 使用をやめて `map.openPopup(popup, latlng)` に。`setView` の moveend は「すでにその位置」だと来ない → popup の指示は `wantPopupId` で持ち越し＋450ms 保険。
> - **preview E2E 全 green・console 0**（1200枚＝白点1140＋サムネ60 / z9↔z17 で dot↔photo 往復 / 画面外フォーカスで popup 実オープン / 12000枚＝density＋サムネ60 / 1日絞りは従来どおり / 閉じる→開き直しクリーン）。
> - **🎉 実機 YES**（web・GitHub Pages）＝ゆう「**いいですね、全期間でも写真がでるようになりました**」。
> - **📤 iOS 1.7 (52) を審査提出（2026-08-23）＝「審査待ち」**。1.6(50) 公開中からの差分＝**v224〜v232 ＋ v236**（🚀取り込み速度パック／🖼拡大表示3件／🗺全期間の地図）。**新権限ゼロ**（Info.plist は v165 から未変更＝写真・カメラ・位置は 1.5/1.6 で申告済み）・プライバシー/年齢レーティング/スクショは 1.6 引き継ぎ。審査メモは「表示と速度の改善だけ・サーバーなし・端末内完結」で作成（全文は CHANGELOG v236）。
> - **⚡ push が Codemagic を自動で回す**（あの日 iOS）＝v236 を push した 11 分後に TestFlight 1.7(52) が届いていた。`codemagic.yaml` に `triggering:` が無いので手動起動だと思い込みかけた＝**トリガは Codemagic の Web UI 側の設定**。ビルドの中身の確認は **⚙情報のいちばん下の `phaseN.M`**（52＝`phase3.183`＝v236 入りを本人が確認）。
> - **▶ 審査中に見ておきたい2点**: ① **📚 ライブラリ全体の取り込みが iOS 実機で止まらず進むか**＝v228 の Swift バッチ取得は **iOS 実機では一度も動いていない**（Android エミュ E2E ＋ preview のモックのみ）。壊れていれば審査中でも新ビルドに差し替えられる ② **自宅周辺のように写真が密な所は寄っても白点のまま**（狭い範囲に数百枚あると z15 でも 300 超）＝代表60枚で足りるか、`MAP_SAMPLE_THUMB` を上げ下げするかは手触りで決める。
> - **✅ v228 の iOS 実機確認 完了**＝ゆうが 1.7(52) で写真20枚を撮って起動 → **⚙情報に 📊 行が点灯**＝Swift のバッチサムネ経路が実機で最後まで通った。**提出時の唯一の不安要素が解消**（実測値は未回収＝Android の 47ms/枚 と比べたい）。
> - **📤 v237（phase3.184）＝🛰️パネルに「足あとを書き出す」**。きっかけ＝「取り込みテストのために全削除すべきか」→ **🗑 全削除は `trackClear()` も呼ぶ＝軌跡も消える**と判明。**ゆうの判断＝「削除はいまのまま全削除で大丈夫。開発者以外、足跡のみを残すなんて使わない。新規で足跡の情報をエクスポートできる窓口を作って」**＝削除の作り分けはせず、**別窓口で逃げ道を用意**。自分の足あと（🛰️ロガー＋✏️手書き・🤝相手は除外）を JSON 1ファイルに→共有シート/ダウンロード（`sharePackFile` 再利用・`anohi-足あと-YYYYMMDD-HHmmss.json`）。preview E2E ✅ console 0。**→ v238 で復元口も作った**（下記）。
> - **📥 v238（phase3.185）＝🛰️パネルに「足あとを読み込む」**。v237 の 📤 は**一方通行**で「消えたら終わり」の不安を実際には解けていなかった → ゆう「復元口を作ってください」。📤 の隣に配置（🗺 / 📤 / 📥 / 📅日ごとに消す / 🗑すべて消す）。**冪等**＝track ストアは id が keyPath なので同じファイルを何回入れても増えない・確認ダイアログで「新しく増えるのは N 点／残り M 点は既にあります」と押す前に結果を出す。🤝相手の点は読み込まない（パック管理下＝孤児になる）。壊れた JSON／別種のファイル／新しい版 をそれぞれ別文言で弾き、欠損行は捨てた数を言う。2000点ずつ刻んで `yieldFrame()`。preview E2E ✅ console 0（5ケース＝全削除→255点復元/冪等/パック拒否/壊れJSON拒否/混在ファイル）。
> - **🤖 vc5 ＝ 手元署名 AAB 完成（2026-08-23 12:19・clean 込み 1分6秒）**。`npm run sync:web`→`npx cap sync android`→`gradlew -p android clean bundleRelease --no-daemon`（資格情報のパースと gradle は**同じ1コール**・`BUILD_NUMBER=5`）。**アップロード前の検証3点 ✅**＝署名済み（`META-INF/MADELEIN.SF`/`.RSA`・`:app:signReleaseBundle` 実行）／versionCode **5**・versionName 1.0／バンドル内 `base/assets/public/index.html` が **phase3.185**（web が本当に載った証拠）。控え＝`Documents/madeleine-signing/madeleine-1.0-vc5.aab`（4030KB）。**中身＝v215〜v238 全部入り**（vc4 は phase3.180＝v229 まで。今回 v230/v231/v232 の拡大表示＋v236 の地図＋v237/v238 の足あと書き出し・読み込みが Android に初上陸）。手順メモ [[android-local-signing-build]] のとおり一発で通った。
> - **🔑 Play のアップロード鍵登録は済んでいる**（2026-08-19 に あの日・レシート・ボイスカレンダーの3アプリとも登録して解決）。**今回やることは無い**（索引の記述が「未登録だった」で終わっていて一度読み違えた）。
> - **▶ 次の一手**: ① **ゆうが Play Console → 内部テストへ vc5 をアップロード → 「内部テスターに公開」**（公開系の操作＝本人）② Pixel 7a で実機確認＝**⚙情報末尾が `phase3.185`**（vc5 が乗った目印）／🗺 全期間の地図に写真が出るか（v236）／🛰️パネルの 📤📥 足あとの書き出し・読み込み（v237/v238・**Android のファイル選択で保存先から選べるか**）③ iOS 1.7 の審査結果待ち（承認後のリリースはゆう）。
>
> **🔧 セッション45（2026-08-21）＝🧭 メタ回（コード無改修）：全判断記録（あの日 v1-v233・レシート r1-r91・TODO・Notion）から「ゆうの判断モデル」を蒸留し `DECISION-FRAMEWORK.md` を新設**。憲法7ヶ条／文脈別プレイブック／チェックリスト／アンチパターン集／転換の力学、全主張に vN/rN 出典付き。DOCMAP と CLAUDE.md から参照を張った（Claude は判断が絡む提案の前に参照する運用）。**▶▶ 次回の一手は下記セッション44から変わらず＝🍎 レシートを Apple 審査に出す（まず審査専用 API キー発行）**。
>
...
> - **（同日追記）ライフログ repo（owntracks-supabase-notion）を読解** → レシート機能が既に本格稼働（receipt関数・transactions・支出マップ）。実測教訓を receipt-spike **r2** に反映（1枚に複数レシート・印字住所=2576px送信・収入対応・カテゴリ統一）。ライフログとの関係整理・移植教訓の一覧は `receipt-spike/TODO.md`。
...
## 🔑 Play に次を出す前に：アップロード鍵を「Android デベロッパーの確認」に足す（2026-08-19）

⚠ **これをやらないと、次の Play リリースは弾かれる**（レシート側で 2026-08-19 に実際に踏んで、足したら通った）。

> このリリースを進めるには、Android デベロッパーの確認要件を満たすようすべての鍵を登録してください。

- 🔎 原因＝**Play App Signing を使っていても、アップロード鍵は自動登録されない**（自動で入るのは Google 側の鍵だけ）
- あの日は「Android デベロッパーの確認」に **鍵が1つだけ**登録されていて、**それは手元のアップロード鍵ではない**（突き合わせ済み）
- ▶ https://play.google.com/console/android-developer-verification → **`io.github.yutsutke.madeleine`** → **「鍵の追加」**で↓を足す

```
55:70:A0:A4:23:90:5B:AB:2C:FE:C1:C6:71:01:51:C7:CE:49:54:98:08:F5:BE:DA:23:42:8C:45:2F:3E:B1:58
```

- 鍵の実物＝`~\Documents\madeleine-signing\madeleine-upload.jks`（別名は同フォルダの CREDENTIALS.txt）
- ⚠ **期限 2026年9月30日**（未登録のアプリは Play から削除される・Google のリマインダー 2026-08-07）
- ⚠ **ボイスカレンダーも同じ状態**（別リポ）＝`57:73:C3:D8:3A:A2:DC:37:9F:4C:20:61:EA:AD:00:6F:2D:82:E5:57:26:F9:29:28:F0:61:C8:A2:9C:D6:EE:18`
- 詳しい経緯・指紋の出し方は [receipt-spike/BUILD-android.md](receipt-spike/BUILD-android.md) の「🚧 Android デベロッパーの確認」
