...
### ✅ タップ履歴「足跡」(v39→v41, 2026-05-31 クローズ)
- random→tap→explore→tap… で**辿った写真の足跡**を中心カードの下にサムネ帯で表示。足跡タップで過去地点へ戻る。**v40 でモード切替と同じ開閉式** (既定畳み・opener に枚数表示)。**v41 で永続化**: ウォークをまたいで貯まり**リロードしても残る** (id 列を localStorage 保存・直前の中心も復元)、**30枚リングバッファ** (`MAX_TRAIL=30`、超えたら古い順に消える)。🗑 全削除でのみクリア。詳細は CHANGELOG v39/v40/v41。実機の手触り観察は次セッション
