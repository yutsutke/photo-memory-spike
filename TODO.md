...
> - **v166++（設定の再描画バグ修正・同セッション・実機FB）**: ⚙設定/題名✎の変更が実機iOSで即反映されなかった（閉じて開くと反映）→原因＝`await dbPut(root)` 後に再描画＝iOS の Blob入り写真レコード再put で tx hang→**render-first＋rpPersist（保存は裏）**。基準写真ピッカーは留まって枠移動。preview で dbPut hang 差し替えでも即反映＝再現&修正確認。詳細 CHANGELOG v166 追記。**教訓＝iOSは写真レコード(Blob)の再put+await→UI更新が危険・楽観更新に倒す**。**実機の手触り（web＋次の1.5ビルド）が次**。
