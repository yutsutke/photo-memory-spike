...
## 現在地 — BUILD: phase3.21 (折りたたんだ日は地図の線も写真も非表示)

> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.21`。**v64 入口モデル → … → v68 タイムライン折りたたみ → v69 折りたたみで線も消す → v70 折りたたみで写真(マーカー)も消す**（preview E2E green）。**v56〜v70 の周回は全て preview green だが、まとめて実機 (iPhone Safari) 未確認**。
> - **v70 で何が変わったか**: タイムラインで折りたたんだ日は、地図から**線もマーカー（写真）も丸ごと非表示**（v69 の「線だけ」から拡張）。マーカーは `filteredGps` から畳んだ日を除外、ただし**線の色 index は `lineGps`＝全日のまま**にして残った日の色が入れ替わらない/空色化しないようにした。`toggleFold`＝`applyFilter({skipFit:true})`。
