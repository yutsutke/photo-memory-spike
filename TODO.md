...
## 現在地 — BUILD: phase3.16 (タイムラインも表示中の日に対応)

> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.16`。**v64 = 地図の入口モデルを「日ベースの軌跡」へ / v65 = タイムラインも表示中の日に対応**（preview E2E green、ヘッダ/写真入口とも「いい感じ」のユーザー評価）。これを含む **v56〜v65 の周回は全て preview green だが、まとめて実機 (iPhone Safari) 未確認**。
> - **v65 で何が変わったか**: pickDays モードで**タイムラインも選ばれた日（ヘッダ=3日 / 写真=その日）だけ**を表示（v64 は全件のままだった）。その日の GPS 無し写真も一覧に出る（地図=GPSのみ / タイムライン=その日の全部の役割分担）。fitBounds を try/catch で握り潰し（markercluster moveend クラッシュの保険）。
