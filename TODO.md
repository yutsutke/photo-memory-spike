...
> - **v165+（位置情報・同セッション追記）**: ユーザー質問「重ね撮りで撮ると位置は保存されない？」→保存されていなかった→**現在地GPS優先→基準継承→null** を実装（ユーザー選択「現在地＋基準fallback」）。位置はレコードのみ=端末内・JPEG に焼かない。preview 3経路 green。詳細 CHANGELOG v165 追記。**実機の位置確認（web は現在地プロンプト・native はロガー許可済）が次**。
