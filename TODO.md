...
> - **🎉 実機 signal（v157）**: ユーザー「**いい感じで出ますね**」＝食べ物判定が実写真で効いている（ゼロショット分類 YES）。→ 追加要望で **v158：📅月ごと / 📆日ごと 切替**を追加（既定=月ごと・日ごとは日付＋曜日で「その日に食べたもの」がまとまる・`buildTimelinePhotos` に `groupBy` 引数・地図タイムラインは無改修・preview green）。
> - **▶ 次の一手**: ①実機で v158 の日ごと/月ごとを触る②食べ物判定のチューニング（取りこぼし/誤検出→`FOOD_PROMPTS`/`FOOD_MIN_PROB`）③刺されば他カテゴリ/フリーテキストへ一般化。v157/v158 は次の Codemagic ビルドで iOS/Android に自動で乗る（web 完結）。
>
