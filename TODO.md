...
### ▶ 次の候補 (未着手)
- **タップ履歴 (連想ウォークの足跡を見返す)** (ユーザー要望 2026-05-31) — random→tap→explore→tap… で**辿ってきた写真の履歴**を見返せるようにしたい。Phase 1 で意図的に「履歴スタックは持たない」とした判断の見直し。reminiscence の「昔→今→未来の流れ」([[reminiscence-at-scale-works]]) と地続き = 歩いた経路自体が小さな物語。実装は **UI 最小主義** ([[ui-minimalism-works]]) に照らし「履歴は使う時だけ広がる」形に (常時表示で写真から意識を逸らさない)。実装先は当面このプロトタイプ (spike)、madeleine への反映は着手時に判断
