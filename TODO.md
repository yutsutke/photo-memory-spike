...
- **HEIC変換を OS に任せる**(v37) — メモリ天井の本命は **heic2any (JS の HEIC→JPEG 変換)**。→ `<input accept>` から `.heic` を外し iOS に JPEG 変換させる (OS 側=軽い、GPS 保持) + `isHeic` を type 優先に (JPEG に heic2any を誤って呼ばない)。サマリに「(うち HEIC変換 N)」を出して実際に呼ばれた枚数を可視化 (N=0 が理想)。**要確認: iPad で 47枚 + 「HEIC変換 N」が 0 になるか**
