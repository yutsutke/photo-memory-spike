...
## 現在地 — BUILD: phase3.37 (地図 📆今日を◀▶で前後の日にずらす＝過去の昨日/明日/明後日＝v87)

> ### 📍 次セッションの再開ポイント（2026-06-22 更新・まずここを読む）
> **🧭 最短サマリ（次に開いたらまずこれ）**: spike の web 体験を一段強化して区切った（**v79–v87 / BUILD `phase3.37` / 全て push 済み・git clean**）。今セッションの軸＝**「過去の今日（On This Day）」を毎日ひらく習慣の入口に育てた一連**（地図 🗺=今日／トップに「過去の今日」チップ／位置なし写真も日時でその日の地図へ／地図入口をアイコン整理／地図タイル記憶／**v87=📆を◀▶で前後の日にずらす＝過去の昨日・明日・明後日…で予定立て/振り返り**）。**実機で強い YES**＝v83「線が無くてもタイムラインに数年分出るだけで時間の経過を感じ心が動く」（[[on-this-day-daily-entry]]）。**Apple 側＝支払い済みなのに登録「保留中」で詰まり→2026-06-22 サポート問い合わせ済み（お問い合わせ番号 102921793205・返信待ち）。詳細は下の「唯一のブロッカー」欄**。
> **▶ 次の3択（どれでもOK・前提情報は全部このすぐ下にある）**:
> - 〔A〕**実機（iPhone Safari）で v79–v87 をまとめて触る** ＝唯一の未確認。観察: 過去の今日チップ／🗺今日（数年分の重なり・写真が少ない日の寂しさ）／**📆◀▶で前後の日にずらす＝過去の昨日/明日/明後日（矢印の発見しやすさ・相対ラベルで「いつを見てるか」伝わるか・予定立て/振り返りに効くか・写真ゼロの日「0年分」の手触り）**／位置なし写真→その日の地図→タイムラインから位置を直す＝想起（[[editing-triggers-reminiscence]]）／地図入口アイコンの並びバランス／タイル記憶。`?v=337` でキャッシュ回避。
> - 〔B〕**web 機能の続き** ＝下の「将来案」や新しい思いつき（毎日の習慣まわりが当たっているので、その周辺＝今日±数日／偶然通知との接続／「N年前の今日」見出し 等が有力）。
> - 〔C〕**native / Apple** ＝**サポート返信待ち**（支払い済みなのに「保留中」で詰まり→2026-06-22 にサポート問い合わせ済み・詳細は下の「唯一のブロッカー」欄）。次セッション開始の挨拶で **Gmail でサポート返信を確認**＝[[session-start-gmail-check]]。有効化されていれば下の「有効化が来たら…(1)〜(5)」へ／まだなら今できること (A)codemagic.yaml ドラフト・(C)i18n 文字列抽出）。
>
...
> **唯一のブロッカー＝Apple メンバーシップが「保留中」のまま有効化されない**（有効化されると確認メール → [App Store Connect](https://appstoreconnect.apple.com) が使えるようになる）。**2026-06-22 時点で状況が判明＝支払いは完了しているのに登録が止まっている**: ①カード明細で **6/17 に ¥12,980 引き落とし済み**を確認 ②それでも Developer アカウントは「保留中」、かつ「Apple Developer Program 登録」ページが「購入手続きを完了してください／お支払いに進む」と**再度の支払いを要求**（押すと二重課金＝押さない）③登録ID **F6U34X83VU**（注文 W1884878174 とは別物）。→ **2026-06-22 11時頃、Apple Developer サポートへ問い合わせ済み（受付確認の自動返信あり・お問い合わせ番号 `102921793205`・人からの返信待ち）**。内容＝「支払い済み（注文 W1884878174・請求 AEY4562521・6/17 引き落とし確認）なのに保留中・登録ページが再支払いを要求。二重課金せず既存支払いを紐付けて有効化してほしい」。督促・返信時はこのお問い合わせ番号を参照。**次セッション開始時はサポート返信の有無を Gmail で確認**（来ていなければ 6/24 頃に再督促を検討）。
