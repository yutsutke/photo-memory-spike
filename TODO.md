...
## 現在地 — BUILD: phase3.24 (位置ロガー: 開いている間だけ軌跡を記録)

> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.24`。git clean & push 済み。**v49〜v73 は全て preview E2E green。ただし v56〜v73 はまとめて実機 (iPhone Safari) 未確認**。
> - **🛰️ 位置ロガー（v73, 新規）**: あとからその日の軌跡を振り返るロガー。**ヘッダ 🛰️ パネルで3モード**＝オフ(既定)/重要な移動のみ(500m以上で1点・疎)/こまめに(25m or 20秒・密)。記録は **`track` ストア（DB v2、座標+時刻のみ＝写真キー非依存＝機種変更で移せる＝Notion §⑥）**。**振り返り＝地図に青緑の点線で重ねる**（⋯「🛰️ 自分の軌跡」トグル・既定ON・表示中の日/期間に追従・畳んだ日は消える）。**web の制約＝前面（アプリを開いている間）でしか記録できない**（背景常時は native の宿題＝Notion §④）。詳細 CHANGELOG v73。
> - **最有力の次手 = 実機 (iPhone Safari) で①ロガー（v73）②地図周回（v56〜v72）を触る**（手元作業）。ロガー観察: 散歩中アプリを開いて軌跡が貯まるか／前面復帰の1点が効くか／重要vsこまめの粒度・電池感／その日の軌跡を地図で振り返ると想起が呼ばれるか（[[editing-triggers-reminiscence]] と同型で効くか）。
...
- **残: 実機（iPhone Safari）で①散歩中アプリを開いて軌跡が貯まるか・前面復帰1点②重要vsこまめの粒度/電池③その日の軌跡の振り返りが想起を呼ぶか。将来: 滞在地クラスタ／再訪検出／GPSなし写真の位置補完（§③b）／ロガー軌跡→連想ウォーク／軌跡を空色に。本命の背景常時は native。**
