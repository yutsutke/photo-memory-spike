...
## 現在地 — BUILD: phase3.17 (飛び飛びの複数日を重ねる)

> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.17`。**v64 入口モデル(日ベース軌跡) → v65 タイムライン対応 → v66 飛び飛び複数日を重ねる + markercluster クラッシュ根治**（preview E2E green、ヘッダ/写真入口とも「いい感じ」評価）。これを含む **v56〜v66 の周回は全て preview green だが、まとめて実機 (iPhone Safari) 未確認**。
> - **v66 で何が変わったか**: **タイムラインの日付見出しタップ＝その日を表示集合(pickDays)に出し入れ（トグル）**。離れた2日を足すと**各日が別色の軌跡で地図に重なる**（2旅行の重ね合わせ）。選んだ日は ✓ 強調＋写真表示、他の日は見出しだけ残し畳む（＝足す候補として見える）。連続範囲は📅パネルに残置。**markercluster の `_leaflet_id in undefined` クラッシュを `removeOutsideVisibleBounds:false` で根治**（近接マーカー＋fitBounds/zoomToShowLayer の moveend で出る・非同期なので try/catch 無力。詳細は CHANGELOG v66 の「効かなかった対処」）。
> - **地図の周回はこれで一区切り。次の最有力 = 実機 (iPhone Safari) でこの一連（v56〜v66）を触る。** あるいはプロダクト化 = HOW Phase 0-1（Capacitor 実機 + ネイティブ写真ライブラリ全件アクセスの de-risk）。
