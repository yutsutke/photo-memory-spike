...
> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.21`。git clean & push 済み（最新 `8061684`）。**v49〜v70 の地図周回は全て preview E2E green。ただし v56〜v70 はまとめて実機 (iPhone Safari) 未確認**。
> - **いまの地図モデル（v64〜v70 で確定。旧 v62 の seed/意味echo は撤去済み）**:
>   - 表示は**入口で決まる**。状態は `mapState.pickDays`（表示する日の集合）。**🗺ヘッダ＝ランダム3日**（各日の全GPSを別色の軌跡に・🎲もう一度引く・軌跡1本タップで他を暗く）/ **📷写真入口＝その写真の日**。
>   - **非連続の複数日は「全期間（ブラウズ）」で**：✕全部を見る→ブラウズ→タイムラインの日付タップで飛び飛びの日を出し入れ（各日が別色で重なる＝2旅行の重ね合わせ）。📅 は「📅 N日」で残り、📅パネルの連続範囲とは排他。
...
>   - 設計判断・ハマり詳細は CHANGELOG v49〜v70 と memory [[map-view-unparked]] に集約。
> - **最有力の次手 = 実機 (iPhone Safari) で地図周回（v56〜v70）を触る**（手元作業）。観察: ①ランダム3日の軌跡から想起できるか／3日が離れすぎないか②全期間での飛び飛び日選択の発見性（📅パネルのヒントで気づくか）③折りたたみの手触り④（v56-61 積み残し）封印の範囲×期間・位置/日付編集の操作感。
> - **大きく動くなら**: プロダクト化 = HOW Phase 0-1（Capacitor 実機 + ネイティブ写真ライブラリ全件アクセスの de-risk）。足場リポ `madeleine` は parked。
> - **将来案（要望が固まれば）**: 📅パネルで「期間を追加」して非連続な複数期間（trip 単位）／畳んだ日に枚数「(N枚)」表示／days・browse でも折りたたみ。
