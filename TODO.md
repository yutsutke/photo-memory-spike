...
## 現在地 — BUILD: phase2 (mix 合流)

**「完成の定義」(reminiscence walk が手で動かせる) は達成済み。** spike の主目的=手触り確認は強い手応え:
「めっちゃいい」(v4) →「色似たやつもいい / 夜祭と夜景のセット」(v5) →「より歩きたくなる」(v12)。
...
- **意味軸を mix プールに合流**(v19) — `mixedNeighbors` を 時空+色+意味 の純シャッフルに拡張。embedding 無しは自然 degrade。pilot で分離していた意味を既定 explore に統合。検証問い「AI が完璧すぎて標準アプリ既視感が出ないか → 時空/色のノイズで連想がドライブするか」を mix そのもので試す段階に到達

### ▶ 次の候補 (未着手)
- **mix 合流の実機手触り判定** (v19 投入後の最重要次手) — 既定 explore (🎲 ミックス) を歩いて、意味だけ単独 (🧠) との違いを観察。判定問い: 時空/色のノイズが混ざることで「標準アプリの AI アルバム」既視感が薄れ連想がドライブするか / それとも意味単独の方が刺さるか (= mix 配分の再検討)
- **新規取り込みに embedding 相乗り** — 今は手動 🧠 backfill のみ。v7 フェーズB で色と一緒に自動抽出 (ただしモデル DL 済み = AI opt-in 済みの人だけ。未 opt-in に 100MB DL を強制しない設計が要)
- **触り込み + 検証問い観察** — 実装ゼロで手触り深堀り (やめにくいか / 旅のヒントになるか / 除外が馴染むか)
...
- [x] mix プールに meaning を合流 (時空 / 色 / 意味 をシャッフル混合) — pilot は比較のため分離、刺さり確認済なので統合 (v19)
- [ ] v7 背景パイプラインに embedding 抽出を相乗り (新規取り込み時に色と一緒に自動抽出)
