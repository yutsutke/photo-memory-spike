...
> - **🎉 実機 signal（v157-159）**: ユーザー「**いい感じで出ますね**」（食べ物判定が実写真で効く＝ゼロショット分類 YES）→ **v158：📅月ごと / 📆日ごと 切替**（既定=月ごと・日ごとは日付＋曜日）→ 実機「**いいかんじ**」＋要望「思い出の場所のように⇆比べる／写真を大きく」→ **v159：⇆比べる＋大きく/小さく**（思い出の場所と同じ `.atl-photobar` 再利用・比較は食べ物レンズを開いたまま z4000 で重ね閉じると戻る・`openCompareView` に後方互換オプション追加・preview green）。
> - **▶ 次の一手**: ①実機で v159 の比べる/大きく＋v158 日ごと/月ごとを触る②食べ物判定のチューニング（取りこぼし/誤検出→`FOOD_PROMPTS`/`NONFOOD_PROMPTS`/`FOOD_MIN_PROB`）③刺されば他カテゴリ/フリーテキストへ一般化。v157-159 は次の Codemagic ビルドで iOS/Android に自動で乗る（web 完結）。
>
