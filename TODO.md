...
> - **📐 v161-163 重ね撮り（旧称 定点）＝実装済み（同日・ユーザー選択「ゴースト重ねで web v1」→FB を2周反映）**: 拡大表示「📐この角度でまた撮る」→ゴーストカメラ→撮影→シリーズ。**実機FB①「左右分割だと一部しか見えない」→ v162＝上下＆contain（全体表示）＋保存範囲の破線ガイド**。**実機FB②「上下よさそう・グリッド線が欲しい・思い出の場所みたいにメニューから／タイトル・基準日・メモ」→ v163＝⊞グリッド線＋一級化**（⚙「📐 重ね撮り」一覧・✏タイトル・🎯基準日・日ごとメモ・📖ログ/🖼写真・メタは root レコードの rpTitle/rpBaseDate/rpNotes＝スキーマレス）。preview E2E 全green。**▶ 次＝①実機 iPhone Safari（GitHub Pages）で一式の手触り**（グリッド線・ログの書き心地・一覧回遊）**②刺されば iOS 1.5 でカメラ権限**（Info.plist `NSCameraUsageDescription`＋Android Manifest `CAMERA`/WebView 権限橋渡し確認＝**今の 1.3/1.4 アプリでは動かない**）③次段候補＝重ねてスライダー再生・最初の基準をアプリ内で撮る導線・シリーズ削除/写真の取り外し。v157-163 は次の Codemagic ビルドで iOS/Android に乗る（**ただしカメラだけは権限宣言が要る＝1.5**）。
>
