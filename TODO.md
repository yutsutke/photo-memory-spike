...
- **起動の段階読み込み**(v28、ユーザー案) — 起動時の全件読み込みを撤廃。**Phase 1: 先頭 `INITIAL_LOAD=3000` (UUID順=全期間ランダム代表) で即 UI / Phase 2: 残りを背景チャンク (`REST_CHUNK=2000`) 追い読み**。id が UUID なのでキー順=ランダム → 明示グルーピング不要でユーザー案「グループからランダム数千」を実現。`addPhotos`/`loadedIds` で取り込み並行でも二重化なし。取り込みも全件 dbGetAll をやめ追記式に
