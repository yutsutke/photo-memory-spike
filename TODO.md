...
### ▶ 次の候補 (未着手)
- **Phase 2 pilot の手触り判定** (v17 投入後の最重要次手) — 50 枚解析して「意味」モードを explore で試し、色ジャンプ (v5) より刺さるか比較。判定問い: 「夜祭と夜景」級の発見が起きるか / 「標準フォトアプリの AI アルバム」と既視感が出ないか / mix に合流させたい衝動があるか
  - 刺されば → Phase 2 本実装 (全件 backfill、Progressive 取り込みに相乗り、mix への合流)
  - 刺さらなければ → 撤退 (🗑 全削除 or `excluded` の embedding を持つ写真を消す手順を作る)
...
### 本実装 (pilot で刺されば)
- [ ] v7 背景パイプラインに embedding 抽出を相乗り (新規取り込み時に色と一緒に)
- [ ] 既存写真も全件 backfill (色 backfill 同等の低優先ループ、ただし時間は CLIP の方が桁違いに重い)
- [ ] mix プールに meaning を合流 (時空 / 色 / 意味 をシャッフル混合)
- [ ] WebGPU/WASM の自動切替明示 (iOS 18+ で WebGPU、それ未満は WASM)
