...
> **🧭 最短サマリ（次に開いたらまずこれ）**: spike の web 体験を一段強化して区切った（**v79–v87 / BUILD `phase3.37` / 全て push 済み・git clean**）。今セッションの軸＝**「過去の今日（On This Day）」を毎日ひらく習慣の入口に育てた一連**（地図 🗺=今日／トップに「過去の今日」チップ／位置なし写真も日時でその日の地図へ／地図入口をアイコン整理／地図タイル記憶／**v87=📆を◀▶で前後の日にずらす＝過去の昨日・明日・明後日…で予定立て/振り返り**）。**実機で強い YES**＝v83「線が無くてもタイムラインに数年分出るだけで時間の経過を感じ心が動く」（[[on-this-day-daily-entry]]）。**✅ Apple メンバーシップ有効化済み（2026-06-25・ケース 102921793205）＝唯一のブロッカー解消。App Store Connect 利用可能・有効期限 2027-06-25。次は native 着手（下の「▶ 次の一手 (1)〜(5)」）**。
...
> - 〔C〕**native / Apple** ＝**✅ 有効化済み（2026-06-25）でブロッカー解消・ここが本命の前進先**。App Store Connect 利用可能。下の「▶ 次の一手 (1)〜(5)」を順に：(1) App Store Connect サインイン確認 → (2) Small Business Program 申請（手数料15%）→ (3) Codemagic 接続（API キー発行）→ (4) `cap add ios`＋Info.plist 用途文言 → (5) 最初の1ビルドを TestFlight へ。🔴 最優先 de-risk＝**ネイティブ写真全件アクセスの最小スパイク**（核「久しぶり=全ライブラリ」の生命線）。
>
...
> **▶ 次の一手**: ✅(1) App Store Connect サインイン＝済 → ✅(2) **ASBP 申請＝送信済（承認待ち・数営業日／承認後 翌会計月末+15日で15%）** → (3) **Codemagic 接続**（App Store Connect API キー Issuer ID / Key ID 発行 → 証明書を自動生成・Mac 不要）→ (4) **`cap add ios`（CI/Mac で `pod install`）＋ Info.plist 用途文言**（写真/位置）→ (5) **最初の1ビルドを TestFlight へ**（今のコードで・パイプライン de-risk＝「整理の順番」①）。🔴 並行で最優先 de-risk＝**ネイティブ写真全件アクセスの最小スパイク**。⚠️ **EU 配信＝DSA トレーダー申告が別途未対応**（自宅住所が EU の App Store ページに公開される→私書箱で回避 or 当面 EU 除外。有料契約/ASBP/申請は止めない・配信テリトリー設定時に判断）。
