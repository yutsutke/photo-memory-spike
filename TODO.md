...
>
> **⏱ TL;DR（次に開いたら、まずこの4点）**
> 1. **web**: GitHub Pages **`phase3.157`（v205）** push 済＝**✕で思い出の場所の一覧へ戻る**（実機FB「一覧へのチップは不要・✕で一覧に戻れるように」）＝思い出の場所の詳細（`atl-overlay`）の「← 一覧」チップを撤去し、右上の ✕ を「閉じて一覧へ」に統一（v177 の重ね撮りシリーズと同じドリルダウン）。transient（🫧囲んだだけの未登録＝一覧に無い）は従来どおり✕で閉じるだけ。preview 両ケース green（通常＝✕でopenAnnivList発火・ボタンから list 消滅／transient＝✕で閉じるだけ・console0）。※直近の大物 v194-204（🤝もらった思い出＝パックの渡し合い一式）は下のセッション33ブロック参照。**実機の手触りFB待ち**（✕で一覧に戻る感覚）。
