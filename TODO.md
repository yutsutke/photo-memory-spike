...
> **🎯 次の一手（優先順）**: ①**1.4 の審査結果を待つ**（開始時 Gmail [[session-start-gmail-check]]）→ 承認されたら **ASC で手動リリース**（公開ボタンはユーザー・〜24h 反映）。②**🍜v157 食べ物レンズを実機で試す＆チューニング**（TestFlight/GitHub Pages 実機で ⚙→🍜 → 食べ物判定の取りこぼし/誤検出を見て `FOOD_PROMPTS`/`NONFOOD_PROMPTS`/`FOOD_MIN_PROB` を調整。刺されば他カテゴリ/フリーテキストへ一般化）。③**Android を Google Play へ**（本命・下の「Android → Google Play 着手チェックリスト」）。④（任意）プロモ「積み重ね」は審査不要・即時反映＝**公開中の 1.3 にも今日から出せる**。
>
...
> - **▶ 次の一手**: ①実機で v160 の各レンズ（風景/人/ペット・言葉・写真で探す）を触る②判定のチューニング（`LENS_CATEGORIES` のプロンプト・`LENS_MIN_PROB`・写真は `LENS_IMAGE_MIN_COS`）③（任意）画像レンズを類似順表示にするか・フリーテキストのサジェスト。v157-160 は次の Codemagic ビルドで iOS/Android に自動で乗る（web 完結）。
>
