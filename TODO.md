...
>
> **🎯 いまの状態**: **🎉 iOS 1.4(35) は承認→公開済み（2026-07-09）**＝承認メール 00:39 JST → 約20分後にストア反映（WebFetch で ver 1.4・「7時間前」を確認）。**⚠️訂正: 旧記載「1.4=手動リリース」は実態とズレ＝リリース設定が自動になっていた**（深夜に自動公開・ユーザー操作なし）→ 以後のバージョンはリリース設定（自動/手動）を提出時に確認する。App Store 現行 = **1.4**（top/route ヒーロー＋v153-156 いまの場所/半径/地図バー＋v152・ex-EU 148）。**1.5(37) が TestFlight 配信済み（7/8）＝カメラ権限入り**→⚠️「🎞️重ね撮り」のカメラが WKWebView で出るか実機未確認（出なければ WKUIDelegate 橋渡しが次）。**v165（Quick Action／全画面カメラ）は 1.5(37) には未搭載＝次の Codemagic ビルド（build 38〜）から**。web = GitHub Pages **`phase3.118`（v165：🎞️アイコン・撮り重ねる・カメラ全画面・Quick Action 配線）**。Android = v142（クリーン起動＋写真全件 MediaStore まで・実機 GPS/大量スループット残・Play 未着手）。
> **🎯 次の一手（優先順）**: ①**Codemagic で 1.5 の新ビルド（38〜）**（ユーザーが Start new build・v157-165 全部乗る）→ TestFlight 実機で **(a) 🎞️重ね撮りカメラが WKWebView で出るか (b) ホーム画面アイコン長押し→重ね撮り一覧 (c) 全画面カメラの手触り**。②**web 実機（iPhone Safari・GitHub Pages）で v165 の手触り**（全画面カメラ・アイコン円ボタン・スライダー位置・「撮り重ねる」の語感）。③**Android を Google Play へ**（本命・下の「Android → Google Play 着手チェックリスト」・Quick Action の Android 版＝shortcuts.xml もこのトラックで）。
>
> ## 📍 セッション28（2026-07-09）＝🎉1.4 公開確認 ＋ 🎞️v165 重ね撮り実機FB第4周
