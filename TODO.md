...
> - **▶ 次の一手（英語版を実際に出す時＝ロードマップ①の本番。今は審査待ち優先）**: ①**残りの長文だけ tr() 化**＝情報モーダルの使い方6段落・位置ロガーパネルの説明文・dev(🧪 #dev)（主要＋二次のUIは v130-132 で済・枠組み `tr()` は入っているので英語を足すだけ）②ASC 英語ローカライズ名（A Past Day — Photos and Footprints）＋`InfoPlist.strings` en③**配信地域を広げる際 EU27 は除外**（DSA 業者住所公開回避・[[post-launch-roadmap]]）。native はこの web を包むので次版で自動的に i18n が乗る。
>
