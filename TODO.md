...
> - **残**: 実機（TestFlight 次ビルド）で、地図タイムラインを絞り込んだ状態→🖼写真/⇆比べる がその範囲だけになるか手触り確認。
...
> - **iOS** = **1.2 公開済み**（位置ロガー版・ロードマップ②／配信は 1.1 を継承＝ex-EU 148地域）。1.3(32) は TestFlight 配信済み＝**v149 の web 修正を載せた次ビルド（1.3 系・build 33+）を Codemagic で作り、実機で手触り確認後に 1.3 を審査提出**が次の候補（1.3 train はまだ開いている＝MARKETING_VERSION 据え置きで可）。
