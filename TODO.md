...
## 現在地 — BUILD: phase3.68（v115：足跡・タイムラインをタップで拡大→左右スワイプでめくれる(showFullImage を list 対応)＋足跡の拡大に「🚶ここから歩く」で辿り直し両立。preview green。｜🎨UI改善ロードマップ（実機フィードバック起点）：**塊A 地図線=✅／塊B 足跡・タイムラインのスワイプ拡大=✅** ／塊C 連想ウォーク6枚→1枚スワイプ（deck/makeBigCard 流用）=▶次。｜v111-113＝トップ1枚スワイプ＋微調整＋ヘッダ整理（実機YES「完璧」「すっきり」）。｜v110：✅再提出済み＝「審査待ち（2回目）」→**審査結果待ち**→承認後 ASC で手動公開。英語名 A Past Day — Photos and Footprints・v1.1。📝次段＝起動時の自動差分取り込み(native)。）
