...
> **🚀 次の周回 = プロダクト化 (App Store / Google Play)。** spike は手触り検証を終え卒業。大枠の方針は Notion メモ「写真思い出アプリ — プロダクト化方針 & フェーズ」に集約。確定: **Capacitor で web をラップ / iOS 先行 / v1 ローカル完結のみ (同期なし)**。次の一歩 = Phase A (Capacitor で実機 + ネイティブ写真アクセス de-risk)。この repo の web コードが資産として乗る。
