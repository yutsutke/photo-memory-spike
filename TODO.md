...
## 現在地 — BUILD: phase3.12 (地図/タイムラインから削除)

> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.12`。git clean & push 済み（最新 `9b8ee3e`）。**v56〜v61 の周回（封印 / 線=1日色替え / 位置の後付け / 日付直し / 削除動線）は全て preview E2E green だが、まとめて実機 (iPhone Safari) 未確認**。
> - **最有力の次手 = 実機でこの周回を触る**（手元作業）: ①🔒封印の「範囲×期間」が実 GPS で直感に合うか（preview は地図 0×0 で範囲封印を実検証できていない）②線の日替わり色が「昔→今」として読めるか③位置ドラッグ／④日付直しの操作感⑤タイムライン項目のボタン過密（位置を直す＋🗑）が窮屈でないか。
> - **実装で続けるなら（有力候補）**: **「直す → そのまま連想ウォークへ」**（位置/日付を直した勢いで 🔮 へ。[[editing-triggers-reminiscence]] = 「編集＝想起の入口」の次の芽。ユーザー反応が一番強かった筋）。他: 名前/キャプション編集、複数選択して一括削除、フル画像にも削除導線。
