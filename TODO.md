...
> - **▶ 次の一手（英語版を実際に出す時＝ロードマップ①の本番。今は審査待ち優先）**: ①情報モーダルの長文本文・二次画面（封印/囲んで比べる/記念日/タイムライン各所）の文字列を `t()` 化（今回は主要導線のみ・枠組みは入ったので英語を足すだけ）②ASC 英語ローカライズ名（A Past Day — Photos and Footprints）＋`InfoPlist.strings` en③**配信地域を広げる際 EU27 は除外**（DSA 業者住所公開回避・[[post-launch-roadmap]]）。native はこの web を包むので次版で自動的に i18n が乗る。
>
