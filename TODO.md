...
### ✅ タップ履歴「足跡」(v39, 2026-05-31 クローズ)
- random→tap→explore→tap… で**辿った写真の足跡**を中心カードの下にサムネ帯で表示。1歩目は出さず2歩目から (使う時だけ広がる)。足跡タップで過去地点へ戻り先の枝は畳む (パンくず的後戻り)。引き直しでリセット。詳細は CHANGELOG v39。実機の手触り観察は次セッション

### ▶ 次の候補 (未着手)
- **(足跡の実機観察)** — iPhone Safari で「辿り直したくなるか / 帯が写真集中を邪魔しないか」。邪魔ならモード切替と同じ開閉トグル化も選択肢
