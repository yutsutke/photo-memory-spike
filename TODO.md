...
- **取り込みのメモリ改善**(v36) — wedge を直したら次は **iPad で ~47枚取り込み中にタブ再読込 (メモリ不足クラッシュ)**。9・20枚は耐え 47 で超え (3GB 端末の天井)。→ `createThumbnail` finally で img/canvas を明示解放 + `BG_IMPORT_DELAY` 120→220。根本は端末限界なので**確実策は 20〜30枚バッチ** (dedup で安全、クラッシュしても取込済みは残る)。**要確認: これで 47 が通るか / 通らなければ HEIC 判定見直し (heic2any 回避) が次の大きい手**
