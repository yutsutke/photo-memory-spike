...
> 3. **web**: GitHub Pages `phase3.129` 全 push 済（**v173＝🎞️基準写真スワイプ切替＋📌確定＋横持ち左右分割 → 実機FB「完璧」／v174＝実機FB対応「写真にアイコンを重ねない」＝fit() が stage の inset を計算（縦=上下帯・横=上帯+右シャッター帯+下スライダー帯）＋📌をアイコン円ボタン化（日付表示は維持）**。ほか v168-172）＝**v174 の実機の手触り FB 待ち**（写真がボタンに隠れないか・横持ち右シャッターの押しやすさ）。
