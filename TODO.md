...
> ### 📍 次セッションの再開ポイント（2026-06-19 セッション終了時・まずここを読む）
> **🧭 最短サマリ（次に開いたらまずこれ）**: spike の web 体験を一段強化して区切った（**v79–v86 / BUILD `phase3.36` / 全て push 済み・git clean**）。今セッションの軸＝**「過去の今日（On This Day）」を毎日ひらく習慣の入口に育てた一連**（地図 🗺=今日／トップに「過去の今日」チップ／位置なし写真も日時でその日の地図へ／地図入口をアイコン整理／地図タイル記憶）。**実機で強い YES**＝v83「線が無くてもタイムラインに数年分出るだけで時間の経過を感じ心が動く」（[[on-this-day-daily-entry]]）。
> **▶ 次の3択（どれでもOK・前提情報は全部このすぐ下にある）**:
> - 〔A〕**実機（iPhone Safari）で v79–v86 をまとめて触る** ＝唯一の未確認。観察: 過去の今日チップ／🗺今日（数年分の重なり・写真が少ない日の寂しさ）／位置なし写真→その日の地図→タイムラインから位置を直す＝想起（[[editing-triggers-reminiscence]]）／地図入口アイコン3つの並びバランス／タイル記憶。`?v=336` でキャッシュ回避。
