...
> ### 📍 次セッションの再開ポイント（2026-07-09 セッション29 更新・まずここを読む）
>
...
> **⚠️状態**: パイプラインは**未検証**（Codemagic に鍵登録→初回 AAB ビルドで要確認）。**律速＝Play アカウント未登録**（$25・本人確認に数日）＋**個人アカウントは production 前に「20人×14日クローズドテスト」必須**（現行ルール・要確認）→初回ゴール＝**内部テストに署名付き AAB**。
> **🎯 次の一手**: ①ユーザー＝**Play Console 登録（$25・最優先＝本人確認が長い）** ②ユーザー＝生成済み鍵を Codemagic に登録→`android-release` で初回 AAB ビルド＝**パイプライン検証** ③Claude＝データセーフティ「収集なし」＋FGS location 申告＋掲載文（日英・iOS 流用）＋スクショ Android サイズの**下書き** ④実機で GPS/大量スループット（エミュは位置 redact）。**チェックリストは下の「📋 Android → Google Play 着手チェックリスト」**（(c)(d) は基盤済・(a)(e)(f)(g) 残）。
>
