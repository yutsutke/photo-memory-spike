...
## 現在地 — BUILD: phase3.50 (v98：地図に「地図だけ全画面」＝⋯表示→⛶ で動線と写真だけの没入表示〔ツールバー/タイムライン/コントロールを隠し解除アイコン1つ・desktopは地図全幅〕。v97＝地図ツールバーを固定2段に統一。ブラウザ検証 green・実機確認待ち)
...
> 2. ✅ **「地図だけ全画面（没入表示）」を追加（v98）**＝ユーザー要望「地図と動線と写真だけ／タイムラインの出ない全画面／解除アイコン1つ」。**入口=⋯表示メニュー「⛶ 地図だけ表示（全画面）」**（v97 の2段ツールバーを汚さない）／**没入中は左上に解除 `⛶` 1つだけ**・ツールバー/タイムライン/ズーム/レイヤを隠す（地図の出典 attribution はライセンス上 残す）／desktop は `#mapEl` を 320px→0 に広げ `invalidateSize`。overlay に `.immersive` クラス1枚＝CSS で実装（状態集約）。ブラウザ検証 green（没入ON/OFF で各 chrome の display 切替・menu entry でメニュー閉じ・desktop 地図再描画 320↔0）。詳細 CHANGELOG v98。
> 3. 📝 **App Store 提出は案B＝コア先行→後で Always 追加（決定・記録のみ／実行はあと）**。初回審査は位置を外して写真コアを先に通し、Always 背景ロガー（v96 実装済）は実績後のアップデートで追加。フォールバックは `if(BgLoc)`/track 隔離で綺麗。「常に許可で通る前例」＝Arc/OwnTracks（端末内ライフログ）等。memory [[location-logger]] に記録。
