...
> - **✅ v1 配信地域＝EU27除外に変更・保存済み（2026-07-01）**: ASC「価格および配信状況」を確認＝**全175地域（EU含む）**だった → 「配信状況の設定」→「特定の国または地域」→**EU27のみ外して148地域**に。目的＝**DSA 業者情報の住所公開を回避**（[[post-launch-roadmap]] の⚠️確認事項を解消）。ASC上の検証＝**配信不可27／配信可能148**。EEA の🇳🇴ノルウェー・🇮🇸アイスランドと🇬🇧イギリスは**残置**（v1はデータ収集ゼロ＝GDPR無関係）。「今後の新規地域を自動配信」トグルは**オフ**（手動管理）。審査待ち中でも地域変更可・**審査はリセットされない**。UI の癖＝「特定」選択→「次へ」で国リスト（全未チェック開始）→全チェックしてEU27だけ外す方式。
