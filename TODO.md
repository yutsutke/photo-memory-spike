...
> **唯一のブロッカー＝Apple メンバーシップが「保留中」のまま有効化されない**（有効化されると確認メール → [App Store Connect](https://appstoreconnect.apple.com) が使えるようになる）。**2026-06-22 時点で状況が判明＝支払いは完了しているのに登録が止まっている**: ①カード明細で **6/17 に ¥12,980 引き落とし済み**を確認 ②それでも Developer アカウントは「保留中」、かつ「Apple Developer Program 登録」ページが「購入手続きを完了してください／お支払いに進む」と**再度の支払いを要求**（押すと二重課金＝押さない）③登録ID **F6U34X83VU**（注文 W1884878174 とは別物）。→ **2026-06-22 11時頃、Apple Developer サポートへメールで問い合わせ済み（返信待ち）**。内容＝「支払い済み（注文 W1884878174・請求 AEY4562521・6/17 引き落とし確認）なのに保留中・登録ページが再支払いを要求。二重課金せず既存支払いを紐付けて有効化してほしい」。**次セッション開始時はサポート返信の有無を Gmail で確認**（来ていなければ 6/24 頃に再督促を検討）。
