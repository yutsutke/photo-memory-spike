...
### ✅ タップ履歴「足跡」(v39→v40, 2026-05-31 クローズ)
- random→tap→explore→tap… で**辿った写真の足跡**を中心カードの下にサムネ帯で表示。足跡タップで過去地点へ戻り先の枝は畳む (パンくず的後戻り)。引き直しでリセット。**v40 でモード切替と同じ開閉式に** (既定畳み・opener に枚数表示・localStorage 永続)。詳細は CHANGELOG v39/v40。実機の手触り観察は次セッション
