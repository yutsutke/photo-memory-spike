...
## 現在地 — BUILD: phase3.64（v111：トップを **1枚大表示＋横スワイプ(ランダム順・タップで連想ウォーク)** に変更＝3枚縦リストから。preview green・**TestFlight ビルド17 で実機スワイプ確認待ち**。審査中の 1.0(16) とは別トラック＝承認後の次版用（審査中ビルドは差し替えない）。｜v110：✅ **初回リジェクト対応＝再提出済み＝「審査待ち（2回目）」**（2.3.8 名前→ビルド1.0(16)／1.5 Support URL→support.html・連絡先 anohiapp@gmail.com／審査メモ添付）。**次＝審査結果待ち→承認後 ASC で手動公開**。英語名は A Past Day — Photos and Footprints に決定・v1.1。）
