...
> 2. **iOS**: 📤**1.5 審査待ちのまま**（ビルド46・承認後 自動リリース）。🍏**v189 で MARKETING_VERSION 1.5→1.6 に上げ済み（審査中の1.5と分離）＝手書き（v187-188）を native で試す用**。**▶ユーザーの手番＝Codemagic の `ios-testflight` を「Start new build」**→ 1.6(build N) が TestFlight 配信 → 実機で ✏️手書き（地図⋯「✏️この日の道を手書き」）＋v180-186。web/native コード無改修＝現状の web（phase3.141）が自動で乗る・カメラ権限は1.5(37+)で既存。挨拶時 Gmail 確認済（7/11 23時時点で審査結果まだ）。
