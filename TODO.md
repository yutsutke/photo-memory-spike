...
## 現在地 — BUILD: phase3.70（v117：拡大表示のスワイプを**サクサク**に＝overlay/img 再利用・paint 方式で画像だけ差し替え（v115 の「サクサクいかない」対処）。preview green(imgReused=true)。｜🎨UI改善＝塊A 地図線／塊B 足跡・タイムラインのスワイプ拡大(+v117 高速化)／塊C 連想ウォーク1枚化 すべて✅（v111-117・**実機/web で要確認**）。｜v110：✅再提出済み＝「審査待ち（2回目）」→**審査結果待ち**→承認後 ASC で手動公開。英語名 A Past Day — Photos and Footprints・v1.1。📝次段＝起動時の自動差分取り込み(native)。）
