...
> - **v166（フォルダー化・同セッション）**: ユーザー再定義「フォルダーのように使う（食事/コーデ/メイク/友達）」→ ①位置継承をフォルダーごとにON/OFF（rpInheritLoc・既定ON）②基準写真を変更可能（rpBasePhotoId・⚙設定ピッカー）③一覧「＋新しい重ね撮り」で新規作成（ゴーストなしカメラ・最初の1枚がrpFolder root）。preview E2E 全green・console0。詳細 CHANGELOG v166。**実機の手触り（web＋次の1.5ビルド）が次**。
