...
## 現在地 — BUILD: phase3.49 (v97：地図ツールバーを1種類に統一＝どのモードでも ◀📆▶／🎲 📅全期間 ⋯表示 の固定2段。✕のみ・ⓘは⋯表示メニューへ・↩︎廃止。ブラウザ検証 green・実機確認待ち)
...
> 2. 📝 **App Store 提出は案B＝コア先行→後で Always 追加（決定・記録のみ／実行はあと）**。初回審査は位置を外して写真コアを先に通し、Always 背景ロガー（v96 実装済）は実績後のアップデートで追加。フォールバックは `if(BgLoc)`/track 隔離で綺麗。「常に許可で通る前例」＝Arc/OwnTracks（端末内ライフログ）等。memory [[location-logger]] に記録。
