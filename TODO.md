...
>
> **🎯 いまの状態**: iOS **1.4（35）を審査提出済み（2026-07-08・審査中）**＝v153-156＋v152。**手動リリース＝承認後の公開ボタンはユーザー**。開始時 Gmail 確認＝**1.4(35) の審査完了メールはまだ無し**（提出当日＝結果待ちは想定内。最新の「審査完了」メールは 1.3(34) 承認＝既知）。web = GitHub Pages **`phase3.117`（🍜 意味レンズ＝「食べ物を振り返る」だけに確定／**📐v161-164 重ね撮り**＝ゴーストカメラ（上下分割・⏱タイマー・写真にフィットするグリッド線 上下両方）＋一級化（⚙一覧・タイトル・基準日・メモ）・実機Safari で FB を反映しながら検証中）**。iOS **1.3 は公開中**（ex-EU 148）。Android = v142（クリーン起動＋写真全件 MediaStore まで・実機 GPS/大量スループット残）。
