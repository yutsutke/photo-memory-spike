...
## 現在地 — BUILD: phase3.13 (入り口で変わる地図: 偶然の1スライス / 時空+意味)

> ### ▶ 次セッションの入口（まずここ）
> - **状態**: BUILD `phase3.13`。**v62 = 地図の表示基準を「全件」から「入口で決まる seed」へ転換**（preview E2E green）。これを含む **v56〜v62 の周回は全て preview green だが、まとめて実機 (iPhone Safari) 未確認**。
