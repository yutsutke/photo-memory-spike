...
> - **v222（実機FB③・完了）＝✏️手書き 🎯起点→終点で「任意の場所」も端点に**。近くに点があればスナップ／無ければ ✚と同じ自動時刻（`handAutoTime` に抽出・共通化）で実点を作り端点に（🕒で後から直せる）。preview で実データ注入 E2E 検証済（任意起点10:00/終点10:09・通過点按分10:05・track焼き込み・スナップも確認・console 0）。web 即反映／iOS/Android native は次ビルド。
