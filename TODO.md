...
> - **▶ 次の一手**: ① **vc4 を作って Pixel 7a 実機確認**（v220-228 全部入り。⚙情報末尾=phase3.179 で確認・📚取り込みの体感と📊行・地図上部コントロール崩れの watch は継続）② iOS も次ビルド（1.6 審査結果次第で 1.7 train）③ 古い端末FBが来たら「⚙情報の📊行」を回収→v228 の効果を数字で確認。エミュレータ E2E は boot 不調で保留（JS 側は preview モックで検証済み・Kotlin 実機は vc4 で）。
>
